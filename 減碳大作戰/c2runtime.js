// Generated by Construct 2, the HTML5 game and app creator :: https://www.construct.net
"use strict";
var aa,
  ca,
  da,
  w,
  ea,
  fa,
  ga,
  L,
  ia,
  ja,
  ka,
  la,
  ma,
  pa,
  O,
  qa,
  ra,
  U,
  sa,
  va,
  wa,
  xa,
  V,
  ya,
  za,
  Aa,
  Ba,
  Ca,
  W,
  Da,
  Ea,
  X,
  Fa,
  Ga,
  Ha,
  Ia,
  Ja,
  Ka,
  La,
  Ma,
  Na,
  Oa,
  Pa,
  Qa,
  Ra,
  Sa,
  Ta,
  Ua,
  Va,
  Wa,
  Xa,
  Ya,
  $a,
  ab,
  bb,
  cb,
  db,
  eb,
  fb,
  gb,
  hb,
  ib,
  jb,
  kb,
  lb,
  mb,
  nb,
  ob,
  pb,
  qb,
  rb,
  sb,
  tb,
  ub,
  vb,
  wb,
  xb,
  yb,
  zb,
  Ab,
  Bb,
  Cb,
  Db,
  Eb,
  Fb,
  Gb,
  Hb,
  Ib,
  Jb,
  Kb,
  Lb,
  Mb,
  Nb,
  Ob,
  Pb,
  Qb,
  Rb,
  Sb,
  Tb = {},
  Ub = {};
"function" !== typeof Object.getPrototypeOf &&
  (Object.getPrototypeOf =
    "object" === typeof "test".__proto__
      ? function (d) {
          return d.__proto__;
        }
      : function (d) {
          return d.constructor.prototype;
        });
(function () {
  function d(a, e, g, b) {
    this.set(a, e, g, b);
  }
  function m() {
    this.pb =
      this.ob =
      this.rb =
      this.qb =
      this.yb =
      this.xb =
      this.Ya =
      this.Xa =
        0;
  }
  function q(a, e, g, b) {
    a < e
      ? g < b
        ? ((x = a < g ? a : g), (A = e > b ? e : b))
        : ((x = a < b ? a : b), (A = e > g ? e : g))
      : g < b
      ? ((x = e < g ? e : g), (A = a > b ? a : b))
      : ((x = e < b ? e : b), (A = a > g ? a : g));
  }
  function h() {
    this.items = this.hd = null;
    this.Vg = 0;
    P && (this.hd = new Set());
    this.gg = [];
    this.Td = !0;
  }
  function n(a) {
    E[T++] = a;
  }
  function t() {
    this.wa = this.sj = this.y = this.Qj = 0;
  }
  function c(a) {
    this.Wa = [];
    this.Rh = this.Th = this.Uh = this.Sh = 0;
    this.Bh(a);
  }
  function a(a, e) {
    this.Uj = a;
    this.Tj = e;
    this.cells = {};
  }
  function f(a, e) {
    this.Uj = a;
    this.Tj = e;
    this.cells = {};
  }
  function r(a, e, g) {
    var b;
    return F.length
      ? ((b = F.pop()), (b.vk = a), (b.x = e), (b.y = g), b)
      : new aa(a, e, g);
  }
  function e(a, e, g) {
    this.vk = a;
    this.x = e;
    this.y = g;
    this.bb = new ca();
  }
  function b(a, e, g) {
    var b;
    return C.length
      ? ((b = C.pop()), (b.vk = a), (b.x = e), (b.y = g), b)
      : new da(a, e, g);
  }
  function p(a, e, g) {
    this.vk = a;
    this.x = e;
    this.y = g;
    this.bb = [];
    this.If = !0;
    this.ed = new ca();
    this.mg = !1;
  }
  function l(a, e) {
    return a.Wc - e.Wc;
  }
  w = function (a) {
    window.console && window.console.log && window.console.log(a);
  };
  ea = function (a) {
    window.console && window.console.error && window.console.error(a);
  };
  fa = function (a) {
    return "undefined" === typeof a;
  };
  ga = function (a) {
    return "number" === typeof a;
  };
  L = function (a) {
    return "string" === typeof a;
  };
  ia = function (a) {
    return 0 < a && 0 === ((a - 1) & a);
  };
  ja = function (a) {
    --a;
    for (var e = 1; 32 > e; e <<= 1) a = a | (a >> e);
    return a + 1;
  };
  ka = function (a) {
    return 0 > a ? -a : a;
  };
  la = function (a, e) {
    return a > e ? a : e;
  };
  ma = function (a, e) {
    return a < e ? a : e;
  };
  pa = Math.PI;
  O = function (a) {
    return 0 <= a ? a | 0 : (a | 0) - 1;
  };
  qa = function (a) {
    var e = a | 0;
    return e === a ? e : e + 1;
  };
  ra = function (a, e, g, b, f, l, k, p) {
    var r, v, y, B;
    a < g ? ((v = a), (r = g)) : ((v = g), (r = a));
    f < k ? ((B = f), (y = k)) : ((B = k), (y = f));
    if (r < B || v > y) return !1;
    e < b ? ((v = e), (r = b)) : ((v = b), (r = e));
    l < p ? ((B = l), (y = p)) : ((B = p), (y = l));
    if (r < B || v > y) return !1;
    r = f - a + k - g;
    v = l - e + p - b;
    a = g - a;
    e = b - e;
    f = k - f;
    l = p - l;
    p = ka(e * f - l * a);
    return ka(f * v - l * r) > p ? !1 : ka(a * v - e * r) <= p;
  };
  d.prototype.set = function (a, e, g, b) {
    this.left = a;
    this.top = e;
    this.right = g;
    this.bottom = b;
  };
  d.prototype.tg = function (a) {
    this.left = a.left;
    this.top = a.top;
    this.right = a.right;
    this.bottom = a.bottom;
  };
  d.prototype.width = function () {
    return this.right - this.left;
  };
  d.prototype.height = function () {
    return this.bottom - this.top;
  };
  d.prototype.offset = function (a, e) {
    this.left += a;
    this.top += e;
    this.right += a;
    this.bottom += e;
    return this;
  };
  d.prototype.normalize = function () {
    var a = 0;
    this.left > this.right &&
      ((a = this.left), (this.left = this.right), (this.right = a));
    this.top > this.bottom &&
      ((a = this.top), (this.top = this.bottom), (this.bottom = a));
  };
  d.prototype.Ds = function (a) {
    return !(
      a.right < this.left ||
      a.bottom < this.top ||
      a.left > this.right ||
      a.top > this.bottom
    );
  };
  d.prototype.Es = function (a, e, g) {
    return !(
      a.right + e < this.left ||
      a.bottom + g < this.top ||
      a.left + e > this.right ||
      a.top + g > this.bottom
    );
  };
  d.prototype.Rb = function (a, e) {
    return (
      a >= this.left && a <= this.right && e >= this.top && e <= this.bottom
    );
  };
  d.prototype.yg = function (a) {
    return (
      this.left === a.left &&
      this.top === a.top &&
      this.right === a.right &&
      this.bottom === a.bottom
    );
  };
  U = d;
  m.prototype.Ah = function (a) {
    this.Xa = a.left;
    this.Ya = a.top;
    this.xb = a.right;
    this.yb = a.top;
    this.qb = a.right;
    this.rb = a.bottom;
    this.ob = a.left;
    this.pb = a.bottom;
  };
  m.prototype.Ko = function (a, e) {
    if (0 === e) this.Ah(a);
    else {
      var g = Math.sin(e),
        b = Math.cos(e),
        f = a.left * g,
        l = a.top * g,
        k = a.right * g,
        g = a.bottom * g,
        p = a.left * b,
        r = a.top * b,
        v = a.right * b,
        b = a.bottom * b;
      this.Xa = p - l;
      this.Ya = r + f;
      this.xb = v - l;
      this.yb = r + k;
      this.qb = v - g;
      this.rb = b + k;
      this.ob = p - g;
      this.pb = b + f;
    }
  };
  m.prototype.offset = function (a, e) {
    this.Xa += a;
    this.Ya += e;
    this.xb += a;
    this.yb += e;
    this.qb += a;
    this.rb += e;
    this.ob += a;
    this.pb += e;
    return this;
  };
  var x = 0,
    A = 0;
  m.prototype.vm = function (a) {
    q(this.Xa, this.xb, this.qb, this.ob);
    a.left = x;
    a.right = A;
    q(this.Ya, this.yb, this.rb, this.pb);
    a.top = x;
    a.bottom = A;
  };
  m.prototype.Rb = function (a, e) {
    var g = this.Xa,
      b = this.Ya,
      f = this.xb - g,
      l = this.yb - b,
      k = this.qb - g,
      p = this.rb - b,
      r = a - g,
      v = e - b,
      y = f * f + l * l,
      B = f * k + l * p,
      l = f * r + l * v,
      c = k * k + p * p,
      d = k * r + p * v,
      u = 1 / (y * c - B * B),
      f = (c * l - B * d) * u,
      y = (y * d - B * l) * u;
    if (0 <= f && 0 < y && 1 > f + y) return !0;
    f = this.ob - g;
    l = this.pb - b;
    y = f * f + l * l;
    B = f * k + l * p;
    l = f * r + l * v;
    u = 1 / (y * c - B * B);
    f = (c * l - B * d) * u;
    y = (y * d - B * l) * u;
    return 0 <= f && 0 < y && 1 > f + y;
  };
  m.prototype.Rd = function (a, e) {
    if (e)
      switch (a) {
        case 0:
          return this.Xa;
        case 1:
          return this.xb;
        case 2:
          return this.qb;
        case 3:
          return this.ob;
        case 4:
          return this.Xa;
        default:
          return this.Xa;
      }
    else
      switch (a) {
        case 0:
          return this.Ya;
        case 1:
          return this.yb;
        case 2:
          return this.rb;
        case 3:
          return this.pb;
        case 4:
          return this.Ya;
        default:
          return this.Ya;
      }
  };
  m.prototype.Zn = function () {
    return (this.Xa + this.xb + this.qb + this.ob) / 4;
  };
  m.prototype.$n = function () {
    return (this.Ya + this.yb + this.rb + this.pb) / 4;
  };
  m.prototype.jn = function (a) {
    var e = a.Zn(),
      g = a.$n();
    if (this.Rb(e, g)) return !0;
    e = this.Zn();
    g = this.$n();
    if (a.Rb(e, g)) return !0;
    var b, f, l, k, p, r, v, y;
    for (v = 0; 4 > v; v++)
      for (y = 0; 4 > y; y++)
        if (
          ((e = this.Rd(v, !0)),
          (g = this.Rd(v, !1)),
          (b = this.Rd(v + 1, !0)),
          (f = this.Rd(v + 1, !1)),
          (l = a.Rd(y, !0)),
          (k = a.Rd(y, !1)),
          (p = a.Rd(y + 1, !0)),
          (r = a.Rd(y + 1, !1)),
          ra(e, g, b, f, l, k, p, r))
        )
          return !0;
    return !1;
  };
  sa = m;
  va = function (a, e) {
    for (var g in e) e.hasOwnProperty(g) && (a[g] = e[g]);
    return a;
  };
  wa = function (a, e) {
    var g, b;
    e = O(e);
    if (!(0 > e || e >= a.length)) {
      g = e;
      for (b = a.length - 1; g < b; g++) a[g] = a[g + 1];
      xa(a, b);
    }
  };
  xa = function (a, e) {
    a.length = e;
  };
  V = function (a) {
    xa(a, 0);
  };
  ya = function (a, e) {
    V(a);
    var g, b;
    g = 0;
    for (b = e.length; g < b; ++g) a[g] = e[g];
  };
  za = function (a, e) {
    a.push.apply(a, e);
  };
  Aa = function (a, e) {
    var g, b;
    g = 0;
    for (b = a.length; g < b; ++g) if (a[g] === e) return g;
    return -1;
  };
  Ba = function (a, e) {
    var g = Aa(a, e);
    -1 !== g && wa(a, g);
  };
  Ca = function (a, e, g) {
    return a < e ? e : a > g ? g : a;
  };
  W = function (a) {
    return a / (180 / pa);
  };
  Da = function (a) {
    return (180 / pa) * a;
  };
  Ea = function (a) {
    a %= 360;
    0 > a && (a += 360);
    return a;
  };
  X = function (a) {
    a %= 2 * pa;
    0 > a && (a += 2 * pa);
    return a;
  };
  Fa = function (a) {
    return Ea(Da(a));
  };
  Ga = function (a) {
    return X(W(a));
  };
  Ha = function (a, e, g, b) {
    return Math.atan2(b - e, g - a);
  };
  Ia = function (a, e) {
    if (a === e) return 0;
    var g = Math.sin(a),
      b = Math.cos(a),
      f = Math.sin(e),
      l = Math.cos(e),
      g = g * f + b * l;
    return 1 <= g ? 0 : -1 >= g ? pa : Math.acos(g);
  };
  Ja = function (a, e, g) {
    var b = Math.sin(a),
      f = Math.cos(a),
      l = Math.sin(e),
      k = Math.cos(e);
    return Math.acos(b * l + f * k) > g
      ? 0 < f * l - b * k
        ? X(a + g)
        : X(a - g)
      : X(e);
  };
  Ka = function (a, e) {
    var g = Math.sin(a),
      b = Math.cos(a),
      f = Math.sin(e),
      l = Math.cos(e);
    return 0 >= b * f - g * l;
  };
  La = function (a, e, g, b) {
    a = g - a;
    e = b - e;
    return Math.sqrt(a * a + e * e);
  };
  Ma = function (a, e) {
    return !a !== !e;
  };
  Na = function (a) {
    for (var e in a) if (a.hasOwnProperty(e)) return !0;
    return !1;
  };
  Oa = function (a) {
    for (var e in a) a.hasOwnProperty(e) && delete a[e];
  };
  var z = +new Date();
  Pa = function () {
    if ("undefined" !== typeof window.performance) {
      var a = window.performance;
      if ("undefined" !== typeof a.now) return a.now();
      if ("undefined" !== typeof a.webkitNow) return a.webkitNow();
      if ("undefined" !== typeof a.mozNow) return a.mozNow();
      if ("undefined" !== typeof a.msNow) return a.msNow();
    }
    return Date.now() - z;
  };
  var g = !1,
    k = (g = !1),
    u = !1;
  "undefined" !== typeof window &&
    ((g =
      /chrome/i.test(navigator.userAgent) ||
      /chromium/i.test(navigator.userAgent)),
    (g = !g && /safari/i.test(navigator.userAgent)),
    (k = /(iphone|ipod|ipad)/i.test(navigator.userAgent)),
    (u = window.c2ejecta));
  var P =
    !g &&
    !u &&
    !k &&
    "undefined" !== typeof Set &&
    "undefined" !== typeof Set.prototype.forEach;
  h.prototype.contains = function (a) {
    return this.ud()
      ? !1
      : P
      ? this.hd.has(a)
      : this.items && this.items.hasOwnProperty(a);
  };
  h.prototype.add = function (a) {
    if (P) this.hd.has(a) || (this.hd.add(a), (this.Td = !1));
    else {
      var e = a.toString(),
        g = this.items;
      g
        ? g.hasOwnProperty(e) || ((g[e] = a), this.Vg++, (this.Td = !1))
        : ((this.items = {}),
          (this.items[e] = a),
          (this.Vg = 1),
          (this.Td = !1));
    }
  };
  h.prototype.remove = function (a) {
    if (!this.ud())
      if (P) this.hd.has(a) && (this.hd["delete"](a), (this.Td = !1));
      else if (this.items) {
        a = a.toString();
        var e = this.items;
        e.hasOwnProperty(a) && (delete e[a], this.Vg--, (this.Td = !1));
      }
  };
  h.prototype.clear = function () {
    this.ud() ||
      (P ? this.hd.clear() : ((this.items = null), (this.Vg = 0)),
      V(this.gg),
      (this.Td = !0));
  };
  h.prototype.ud = function () {
    return 0 === this.count();
  };
  h.prototype.count = function () {
    return P ? this.hd.size : this.Vg;
  };
  var E = null,
    T = 0;
  h.prototype.Lt = function () {
    if (!this.Td) {
      if (P)
        V(this.gg),
          (E = this.gg),
          (T = 0),
          this.hd.forEach(n),
          (E = null),
          (T = 0);
      else {
        var a = this.gg;
        V(a);
        var e,
          g = 0,
          b = this.items;
        if (b) for (e in b) b.hasOwnProperty(e) && (a[g++] = b[e]);
      }
      this.Td = !0;
    }
  };
  h.prototype.Jd = function () {
    this.Lt();
    return this.gg;
  };
  ca = h;
  new ca();
  Qa = function (a, e) {
    P ? Ra(a, e.hd) : Sa(a, e.Jd());
  };
  Ra = function (a, e) {
    var g, b, f, l;
    b = g = 0;
    for (f = a.length; g < f; ++g) (l = a[g]), e.has(l) || (a[b++] = l);
    xa(a, b);
  };
  Sa = function (a, e) {
    var g, b, f, l;
    b = g = 0;
    for (f = a.length; g < f; ++g) (l = a[g]), -1 === Aa(e, l) && (a[b++] = l);
    xa(a, b);
  };
  t.prototype.add = function (a) {
    this.y = a - this.Qj;
    this.sj = this.wa + this.y;
    this.Qj = this.sj - this.wa - this.y;
    this.wa = this.sj;
  };
  t.prototype.reset = function () {
    this.wa = this.sj = this.y = this.Qj = 0;
  };
  Ta = t;
  Ua = function (a) {
    return a.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
  };
  c.prototype.Bh = function (a) {
    this.vo = a;
    this.Rc = a.length / 2;
    this.Wa.length = a.length;
    this.Wh = this.Xh = -1;
    this.ym = 0;
  };
  c.prototype.Gf = function () {
    return !this.vo.length;
  };
  c.prototype.qa = function () {
    for (
      var a = this.Wa,
        e = a[0],
        g = e,
        b = a[1],
        f = b,
        l,
        k,
        p = 1,
        r = this.Rc;
      p < r;
      ++p
    )
      (k = 2 * p),
        (l = a[k]),
        (k = a[k + 1]),
        l < e && (e = l),
        l > g && (g = l),
        k < b && (b = k),
        k > f && (f = k);
    this.Sh = e;
    this.Th = g;
    this.Uh = b;
    this.Rh = f;
  };
  c.prototype.Ah = function (a, e, g) {
    this.Wa.length = 8;
    this.Rc = 4;
    var b = this.Wa;
    b[0] = a.left - e;
    b[1] = a.top - g;
    b[2] = a.right - e;
    b[3] = a.top - g;
    b[4] = a.right - e;
    b[5] = a.bottom - g;
    b[6] = a.left - e;
    b[7] = a.bottom - g;
    this.Xh = a.right - a.left;
    this.Wh = a.bottom - a.top;
    this.qa();
  };
  c.prototype.Zf = function (a, e, g, b, f) {
    this.Wa.length = 8;
    this.Rc = 4;
    var l = this.Wa;
    l[0] = a.Xa - e;
    l[1] = a.Ya - g;
    l[2] = a.xb - e;
    l[3] = a.yb - g;
    l[4] = a.qb - e;
    l[5] = a.rb - g;
    l[6] = a.ob - e;
    l[7] = a.pb - g;
    this.Xh = b;
    this.Wh = f;
    this.qa();
  };
  c.prototype.Jo = function (a) {
    this.Rc = a.Rc;
    ya(this.Wa, a.Wa);
    this.Sh = a.Sh;
    this.Uh = a.Uh;
    this.Th = a.Th;
    this.Rh = a.Rh;
  };
  c.prototype.uf = function (a, e, g) {
    if (this.Xh !== a || this.Wh !== e || this.ym !== g) {
      this.Xh = a;
      this.Wh = e;
      this.ym = g;
      var b,
        f,
        l,
        k,
        p,
        r = 0,
        v = 1,
        y = this.vo,
        B = this.Wa;
      0 !== g && ((r = Math.sin(g)), (v = Math.cos(g)));
      g = 0;
      for (l = this.Rc; g < l; g++)
        (b = 2 * g),
          (f = b + 1),
          (k = y[b] * a),
          (p = y[f] * e),
          (B[b] = k * v - p * r),
          (B[f] = p * v + k * r);
      this.qa();
    }
  };
  c.prototype.Rb = function (a, e) {
    var g = this.Wa;
    if (a === g[0] && e === g[1]) return !0;
    var b,
      f,
      l,
      k = this.Rc,
      p = this.Sh - 110,
      r = this.Uh - 101,
      v = this.Th + 131,
      y = this.Rh + 120,
      B,
      c,
      d = 0,
      u = 0;
    for (b = 0; b < k; b++)
      (f = 2 * b),
        (l = ((b + 1) % k) * 2),
        (B = g[f]),
        (f = g[f + 1]),
        (c = g[l]),
        (l = g[l + 1]),
        ra(p, r, a, e, B, f, c, l) && d++,
        ra(v, y, a, e, B, f, c, l) && u++;
    return 1 === d % 2 || 1 === u % 2;
  };
  c.prototype.Og = function (a, e, g) {
    var b = a.Wa,
      f = this.Wa;
    if (this.Rb(b[0] + e, b[1] + g) || a.Rb(f[0] - e, f[1] - g)) return !0;
    var l, k, p, r, v, y, B, c, d, u, x, n;
    l = 0;
    for (r = this.Rc; l < r; l++)
      for (
        k = 2 * l,
          p = ((l + 1) % r) * 2,
          c = f[k],
          k = f[k + 1],
          d = f[p],
          u = f[p + 1],
          p = 0,
          B = a.Rc;
        p < B;
        p++
      )
        if (
          ((v = 2 * p),
          (y = ((p + 1) % B) * 2),
          (x = b[v] + e),
          (v = b[v + 1] + g),
          (n = b[y] + e),
          (y = b[y + 1] + g),
          ra(c, k, d, u, x, v, n, y))
        )
          return !0;
    return !1;
  };
  Va = c;
  a.prototype.ae = function (a, e, g) {
    var b;
    b = this.cells[a];
    return b
      ? (b = b[e])
        ? b
        : g
        ? ((b = r(this, a, e)), (this.cells[a][e] = b))
        : null
      : g
      ? ((b = r(this, a, e)), (this.cells[a] = {}), (this.cells[a][e] = b))
      : null;
  };
  a.prototype.Cb = function (a) {
    return O(a / this.Uj);
  };
  a.prototype.Db = function (a) {
    return O(a / this.Tj);
  };
  a.prototype.update = function (a, e, g) {
    var b, f, l, k, p;
    if (e)
      for (b = e.left, f = e.right; b <= f; ++b)
        for (l = e.top, k = e.bottom; l <= k; ++l)
          if (!g || !g.Rb(b, l))
            if ((p = this.ae(b, l, !1)))
              p.remove(a),
                p.ud() &&
                  (p.bb.clear(),
                  1e3 > F.length && F.push(p),
                  (this.cells[b][l] = null));
    if (g)
      for (b = g.left, f = g.right; b <= f; ++b)
        for (l = g.top, k = g.bottom; l <= k; ++l)
          (e && e.Rb(b, l)) || this.ae(b, l, !0).Bk(a);
  };
  a.prototype.ej = function (a, e) {
    var g, b, f, l, k, p;
    g = this.Cb(a.left);
    f = this.Db(a.top);
    b = this.Cb(a.right);
    for (k = this.Db(a.bottom); g <= b; ++g)
      for (l = f; l <= k; ++l) (p = this.ae(g, l, !1)) && p.dump(e);
  };
  Wa = a;
  f.prototype.ae = function (a, e, g) {
    var f;
    f = this.cells[a];
    return f
      ? (f = f[e])
        ? f
        : g
        ? ((f = b(this, a, e)), (this.cells[a][e] = f))
        : null
      : g
      ? ((f = b(this, a, e)), (this.cells[a] = {}), (this.cells[a][e] = f))
      : null;
  };
  f.prototype.Cb = function (a) {
    return O(a / this.Uj);
  };
  f.prototype.Db = function (a) {
    return O(a / this.Tj);
  };
  f.prototype.update = function (a, e, g) {
    var b, f, l, k, p;
    if (e)
      for (b = e.left, f = e.right; b <= f; ++b)
        for (l = e.top, k = e.bottom; l <= k; ++l)
          if (!g || !g.Rb(b, l))
            if ((p = this.ae(b, l, !1)))
              p.remove(a),
                p.ud() &&
                  (p.reset(),
                  1e3 > C.length && C.push(p),
                  (this.cells[b][l] = null));
    if (g)
      for (b = g.left, f = g.right; b <= f; ++b)
        for (l = g.top, k = g.bottom; l <= k; ++l)
          (e && e.Rb(b, l)) || this.ae(b, l, !0).Bk(a);
  };
  f.prototype.ej = function (a, e, g, b, f) {
    var l, k;
    a = this.Cb(a);
    e = this.Db(e);
    g = this.Cb(g);
    for (l = this.Db(b); a <= g; ++a)
      for (b = e; b <= l; ++b) (k = this.ae(a, b, !1)) && k.dump(f);
  };
  f.prototype.Ss = function (a) {
    var e, g, b, f, l;
    e = a.left;
    b = a.top;
    g = a.right;
    for (f = a.bottom; e <= g; ++e)
      for (a = b; a <= f; ++a) if ((l = this.ae(e, a, !1))) l.If = !1;
  };
  Xa = f;
  var F = [];
  e.prototype.ud = function () {
    return this.bb.ud();
  };
  e.prototype.Bk = function (a) {
    this.bb.add(a);
  };
  e.prototype.remove = function (a) {
    this.bb.remove(a);
  };
  e.prototype.dump = function (a) {
    za(a, this.bb.Jd());
  };
  aa = e;
  var C = [];
  p.prototype.ud = function () {
    if (!this.bb.length) return !0;
    if (this.bb.length > this.ed.count()) return !1;
    this.kk();
    return !0;
  };
  p.prototype.Bk = function (a) {
    this.ed.contains(a)
      ? (this.ed.remove(a), this.ed.ud() && (this.mg = !1))
      : this.bb.length
      ? (this.bb[this.bb.length - 1].bd() > a.bd() && (this.If = !1),
        this.bb.push(a))
      : (this.bb.push(a), (this.If = !0));
  };
  p.prototype.remove = function (a) {
    this.ed.add(a);
    this.mg = !0;
    30 <= this.ed.count() && this.kk();
  };
  p.prototype.kk = function () {
    this.mg &&
      (this.ed.count() === this.bb.length
        ? this.reset()
        : (Qa(this.bb, this.ed), this.ed.clear(), (this.mg = !1)));
  };
  p.prototype.dr = function () {
    this.If || (this.bb.sort(l), (this.If = !0));
  };
  p.prototype.reset = function () {
    V(this.bb);
    this.If = !0;
    this.ed.clear();
    this.mg = !1;
  };
  p.prototype.dump = function (a) {
    this.kk();
    this.dr();
    this.bb.length && a.push(this.bb);
  };
  da = p;
  var I =
    "lighter xor copy destination-over source-in destination-in source-out destination-out source-atop destination-atop".split(
      " "
    );
  Ya = function (a) {
    return 0 >= a || 11 <= a ? "source-over" : I[a - 1];
  };
  $a = function (a, e, g) {
    if (g)
      switch (((a.wb = g.ONE), (a.sb = g.ONE_MINUS_SRC_ALPHA), e)) {
        case 1:
          a.wb = g.ONE;
          a.sb = g.ONE;
          break;
        case 3:
          a.wb = g.ONE;
          a.sb = g.ZERO;
          break;
        case 4:
          a.wb = g.ONE_MINUS_DST_ALPHA;
          a.sb = g.ONE;
          break;
        case 5:
          a.wb = g.DST_ALPHA;
          a.sb = g.ZERO;
          break;
        case 6:
          a.wb = g.ZERO;
          a.sb = g.SRC_ALPHA;
          break;
        case 7:
          a.wb = g.ONE_MINUS_DST_ALPHA;
          a.sb = g.ZERO;
          break;
        case 8:
          a.wb = g.ZERO;
          a.sb = g.ONE_MINUS_SRC_ALPHA;
          break;
        case 9:
          a.wb = g.DST_ALPHA;
          a.sb = g.ONE_MINUS_SRC_ALPHA;
          break;
        case 10:
          (a.wb = g.ONE_MINUS_DST_ALPHA), (a.sb = g.SRC_ALPHA);
      }
  };
  ab = function (a) {
    return Math.round(1e6 * a) / 1e6;
  };
  bb = function (a, e) {
    return "string" !== typeof a ||
      "string" !== typeof e ||
      a.length !== e.length
      ? !1
      : a === e
      ? !0
      : a.toLowerCase() === e.toLowerCase();
  };
  cb = function (a) {
    a = a.target;
    return !a ||
      a === document ||
      a === window ||
      (document && document.body && a === document.body) ||
      bb(a.tagName, "canvas")
      ? !0
      : !1;
  };
})();
var Vb = "undefined" !== typeof Float32Array ? Float32Array : Array;
function Wb(d) {
  var m = new Vb(3);
  d && ((m[0] = d[0]), (m[1] = d[1]), (m[2] = d[2]));
  return m;
}
function Xb(d) {
  var m = new Vb(16);
  d &&
    ((m[0] = d[0]),
    (m[1] = d[1]),
    (m[2] = d[2]),
    (m[3] = d[3]),
    (m[4] = d[4]),
    (m[5] = d[5]),
    (m[6] = d[6]),
    (m[7] = d[7]),
    (m[8] = d[8]),
    (m[9] = d[9]),
    (m[10] = d[10]),
    (m[11] = d[11]),
    (m[12] = d[12]),
    (m[13] = d[13]),
    (m[14] = d[14]),
    (m[15] = d[15]));
  return m;
}
function Yb(d, m) {
  m[0] = d[0];
  m[1] = d[1];
  m[2] = d[2];
  m[3] = d[3];
  m[4] = d[4];
  m[5] = d[5];
  m[6] = d[6];
  m[7] = d[7];
  m[8] = d[8];
  m[9] = d[9];
  m[10] = d[10];
  m[11] = d[11];
  m[12] = d[12];
  m[13] = d[13];
  m[14] = d[14];
  m[15] = d[15];
}
function Zb(d, m) {
  var q = m[0],
    h = m[1];
  m = m[2];
  d[0] *= q;
  d[1] *= q;
  d[2] *= q;
  d[3] *= q;
  d[4] *= h;
  d[5] *= h;
  d[6] *= h;
  d[7] *= h;
  d[8] *= m;
  d[9] *= m;
  d[10] *= m;
  d[11] *= m;
}
function $b(d, m, q, h) {
  h || (h = Xb());
  var n,
    t,
    c,
    a,
    f,
    r,
    e,
    b,
    p = d[0],
    l = d[1];
  d = d[2];
  t = q[0];
  c = q[1];
  n = q[2];
  q = m[1];
  r = m[2];
  p === m[0] && l === q && d === r
    ? ((d = h),
      (d[0] = 1),
      (d[1] = 0),
      (d[2] = 0),
      (d[3] = 0),
      (d[4] = 0),
      (d[5] = 1),
      (d[6] = 0),
      (d[7] = 0),
      (d[8] = 0),
      (d[9] = 0),
      (d[10] = 1),
      (d[11] = 0),
      (d[12] = 0),
      (d[13] = 0),
      (d[14] = 0),
      (d[15] = 1))
    : ((q = p - m[0]),
      (r = l - m[1]),
      (e = d - m[2]),
      (b = 1 / Math.sqrt(q * q + r * r + e * e)),
      (q *= b),
      (r *= b),
      (e *= b),
      (m = c * e - n * r),
      (n = n * q - t * e),
      (t = t * r - c * q),
      (b = Math.sqrt(m * m + n * n + t * t))
        ? ((b = 1 / b), (m *= b), (n *= b), (t *= b))
        : (t = n = m = 0),
      (c = r * t - e * n),
      (a = e * m - q * t),
      (f = q * n - r * m),
      (b = Math.sqrt(c * c + a * a + f * f))
        ? ((b = 1 / b), (c *= b), (a *= b), (f *= b))
        : (f = a = c = 0),
      (h[0] = m),
      (h[1] = c),
      (h[2] = q),
      (h[3] = 0),
      (h[4] = n),
      (h[5] = a),
      (h[6] = r),
      (h[7] = 0),
      (h[8] = t),
      (h[9] = f),
      (h[10] = e),
      (h[11] = 0),
      (h[12] = -(m * p + n * l + t * d)),
      (h[13] = -(c * p + a * l + f * d)),
      (h[14] = -(q * p + r * l + e * d)),
      (h[15] = 1));
}
(function () {
  function d(a, f, r) {
    this.fe =
      /msie/i.test(navigator.userAgent) || /trident/i.test(navigator.userAgent);
    this.height = this.width = 0;
    this.ma = !!r;
    this.vi = this.Pg = !1;
    this.ak = 0;
    this.Hj = 1;
    this.$l = 1e3;
    this.Rt = (this.$l - this.Hj) / 32768;
    this.Rj = Wb([0, 0, 100]);
    this.In = Wb([0, 0, 0]);
    this.dp = Wb([0, 1, 0]);
    this.Lh = Wb([1, 1, 1]);
    this.Hm = !0;
    this.Qi = Xb();
    this.oc = Xb();
    this.Rk = Xb();
    this.$j = Xb();
    this.D = a;
    this.version =
      0 === this.D.getParameter(this.D.VERSION).indexOf("WebGL 2") ? 2 : 1;
    this.cn();
  }
  function m(a, f, r) {
    this.D = a;
    this.Ch = f;
    this.name = r;
    this.tc = a.getAttribLocation(f, "aPos");
    this.ne = a.getAttribLocation(f, "aTex");
    this.Gn = a.getUniformLocation(f, "matP");
    this.Mi = a.getUniformLocation(f, "matMV");
    this.Pf = a.getUniformLocation(f, "opacity");
    this.Yk = a.getUniformLocation(f, "colorFill");
    this.Hn = a.getUniformLocation(f, "samplerFront");
    this.gh = a.getUniformLocation(f, "samplerBack");
    this.Xe = a.getUniformLocation(f, "destStart");
    this.We = a.getUniformLocation(f, "destEnd");
    this.ih = a.getUniformLocation(f, "seconds");
    this.$k = a.getUniformLocation(f, "pixelWidth");
    this.Zk = a.getUniformLocation(f, "pixelHeight");
    this.fh = a.getUniformLocation(f, "layerScale");
    this.eh = a.getUniformLocation(f, "layerAngle");
    this.jh = a.getUniformLocation(f, "viewOrigin");
    this.hh = a.getUniformLocation(f, "scrollPos");
    this.zs = !!(
      this.$k ||
      this.Zk ||
      this.ih ||
      this.gh ||
      this.Xe ||
      this.We ||
      this.fh ||
      this.eh ||
      this.jh ||
      this.hh
    );
    this.Qn = this.Rn = -999;
    this.Pi = 1;
    this.Mn = this.Ln = 0;
    this.On = this.Kn = this.Jn = 1;
    this.Un = this.Tn = this.Sn = this.Wn = this.Vn = this.Nn = 0;
    this.Qk = [];
    this.Pn = Xb();
    this.Pf && a.uniform1f(this.Pf, 1);
    this.Yk && a.uniform4f(this.Yk, 1, 1, 1, 1);
    this.Hn && a.uniform1i(this.Hn, 0);
    this.gh && a.uniform1i(this.gh, 1);
    this.Xe && a.uniform2f(this.Xe, 0, 0);
    this.We && a.uniform2f(this.We, 1, 1);
    this.fh && a.uniform1f(this.fh, 1);
    this.eh && a.uniform1f(this.eh, 0);
    this.jh && a.uniform2f(this.jh, 0, 0);
    this.hh && a.uniform2f(this.hh, 0, 0);
    this.ih && a.uniform1f(this.ih, 0);
    this.Me = !1;
  }
  function q(a, f) {
    return (
      a[0] === f[0] &&
      a[1] === f[1] &&
      a[2] === f[2] &&
      a[3] === f[3] &&
      a[4] === f[4] &&
      a[5] === f[5] &&
      a[6] === f[6] &&
      a[7] === f[7] &&
      a[8] === f[8] &&
      a[9] === f[9] &&
      a[10] === f[10] &&
      a[11] === f[11] &&
      a[12] === f[12] &&
      a[13] === f[13] &&
      a[14] === f[14] &&
      a[15] === f[15]
    );
  }
  function h(a, f) {
    this.type = a;
    this.A = f;
    this.D = f.D;
    this.cd = this.Bb = this.mo = 0;
    this.P = this.zc = null;
    this.Lo = [];
  }
  var n = Xb();
  d.prototype.cn = function () {
    var a = this.D,
      f;
    this.xn = 1;
    this.Te = this.je = null;
    this.ci = 1;
    a.clearColor(0, 0, 0, 0);
    a.clear(a.COLOR_BUFFER_BIT);
    a.enable(a.BLEND);
    a.blendFunc(a.ONE, a.ONE_MINUS_SRC_ALPHA);
    a.disable(a.CULL_FACE);
    a.disable(a.STENCIL_TEST);
    a.disable(a.DITHER);
    this.ma
      ? (a.enable(a.DEPTH_TEST), a.depthFunc(a.LEQUAL))
      : a.disable(a.DEPTH_TEST);
    this.zn = a.ONE;
    this.vn = a.ONE_MINUS_SRC_ALPHA;
    this.Fj = new Float32Array(8e3 * (this.ma ? 3 : 2));
    this.tj = new Float32Array(16e3);
    this.ro = new Float32Array(32e3);
    this.jl = a.createBuffer();
    a.bindBuffer(a.ARRAY_BUFFER, this.jl);
    a.bufferData(a.ARRAY_BUFFER, this.ro.byteLength, a.DYNAMIC_DRAW);
    this.Kh = Array(4);
    this.Gh = Array(4);
    for (f = 0; 4 > f; f++)
      (this.Kh[f] = a.createBuffer()),
        a.bindBuffer(a.ARRAY_BUFFER, this.Kh[f]),
        a.bufferData(a.ARRAY_BUFFER, this.Fj.byteLength, a.DYNAMIC_DRAW),
        (this.Gh[f] = a.createBuffer()),
        a.bindBuffer(a.ARRAY_BUFFER, this.Gh[f]),
        a.bufferData(a.ARRAY_BUFFER, this.tj.byteLength, a.DYNAMIC_DRAW);
    this.od = 0;
    this.Bs = a.createBuffer();
    a.bindBuffer(a.ELEMENT_ARRAY_BUFFER, this.Bs);
    for (var r = new Uint16Array(12e3), e = (f = 0); 12e3 > f; )
      (r[f++] = e),
        (r[f++] = e + 1),
        (r[f++] = e + 2),
        (r[f++] = e),
        (r[f++] = e + 2),
        (r[f++] = e + 3),
        (e += 4);
    a.bufferData(a.ELEMENT_ARRAY_BUFFER, r, a.STATIC_DRAW);
    this.kl = this.bg = this.Kd = 0;
    this.Ga = [];
    f = this.ma
      ? "attribute highp vec3 aPos;\nattribute mediump vec2 aTex;\nvarying mediump vec2 vTex;\nuniform highp mat4 matP;\nuniform highp mat4 matMV;\nvoid main(void) {\n\tgl_Position = matP * matMV * vec4(aPos.x, aPos.y, aPos.z, 1.0);\n\tvTex = aTex;\n}"
      : "attribute highp vec2 aPos;\nattribute mediump vec2 aTex;\nvarying mediump vec2 vTex;\nuniform highp mat4 matP;\nuniform highp mat4 matMV;\nvoid main(void) {\n\tgl_Position = matP * matMV * vec4(aPos.x, aPos.y, 0.0, 1.0);\n\tvTex = aTex;\n}";
    r = this.ug(
      {
        src: "varying mediump vec2 vTex;\nuniform lowp float opacity;\nuniform lowp sampler2D samplerFront;\nvoid main(void) {\n\tgl_FragColor = texture2D(samplerFront, vTex);\n\tgl_FragColor *= opacity;\n}",
      },
      f,
      "<default>"
    );
    this.Ga.push(r);
    r = this.ug(
      {
        src: "uniform mediump sampler2D samplerFront;\nvarying lowp float opacity;\nvoid main(void) {\n\tgl_FragColor = texture2D(samplerFront, gl_PointCoord);\n\tgl_FragColor *= opacity;\n}",
      },
      "attribute vec4 aPos;\nvarying float opacity;\nuniform mat4 matP;\nuniform mat4 matMV;\nvoid main(void) {\n\tgl_Position = matP * matMV * vec4(aPos.x, aPos.y, 0.0, 1.0);\n\tgl_PointSize = aPos.z;\n\topacity = aPos.w;\n}",
      "<point>"
    );
    this.Ga.push(r);
    r = this.ug(
      {
        src: "varying mediump vec2 vTex;\nuniform lowp sampler2D samplerFront;\nvoid main(void) {\n\tif (texture2D(samplerFront, vTex).a < 1.0)\n\t\tdiscard;\n}",
      },
      f,
      "<earlyz>"
    );
    this.Ga.push(r);
    r = this.ug(
      {
        src: "uniform lowp vec4 colorFill;\nvoid main(void) {\n\tgl_FragColor = colorFill;\n}",
      },
      f,
      "<fill>"
    );
    this.Ga.push(r);
    for (var b in ac)
      ac.hasOwnProperty(b) && this.Ga.push(this.ug(ac[b], f, b));
    a.activeTexture(a.TEXTURE0);
    a.bindTexture(a.TEXTURE_2D, null);
    this.Sd = [];
    this.md = 0;
    this.ub = !1;
    this.Em = this.$g = -1;
    this.vf = null;
    this.jk = a.createFramebuffer();
    this.ei = this.fj = null;
    this.tm = !1;
    this.ma && (this.ei = a.createRenderbuffer());
    this.xe = Wb([0, 0, 0]);
    this.Xn = a.getParameter(a.ALIASED_POINT_SIZE_RANGE)[1];
    2048 < this.Xn && (this.Xn = 2048);
    this.Mb(0);
  };
  m.prototype.Vl = function (a) {
    q(this.Pn, a) || (Yb(a, this.Pn), this.D.uniformMatrix4fv(this.Mi, !1, a));
  };
  d.prototype.ug = function (a, f, r) {
    var e = this.D,
      b = e.createShader(e.FRAGMENT_SHADER);
    e.shaderSource(b, a.src);
    e.compileShader(b);
    if (!e.getShaderParameter(b, e.COMPILE_STATUS))
      throw (
        ((a = e.getShaderInfoLog(b)),
        e.deleteShader(b),
        Error("error compiling fragment shader: " + a))
      );
    var p = e.createShader(e.VERTEX_SHADER);
    e.shaderSource(p, f);
    e.compileShader(p);
    if (!e.getShaderParameter(p, e.COMPILE_STATUS))
      throw (
        ((a = e.getShaderInfoLog(p)),
        e.deleteShader(b),
        e.deleteShader(p),
        Error("error compiling vertex shader: " + a))
      );
    f = e.createProgram();
    e.attachShader(f, b);
    e.attachShader(f, p);
    e.linkProgram(f);
    if (!e.getProgramParameter(f, e.LINK_STATUS))
      throw (
        ((a = e.getProgramInfoLog(f)),
        e.deleteShader(b),
        e.deleteShader(p),
        e.deleteProgram(f),
        Error("error linking shader program: " + a))
      );
    e.useProgram(f);
    e.deleteShader(b);
    e.deleteShader(p);
    b = new m(e, f, r);
    b.hk = a.hk || 0;
    b.ik = a.ik || 0;
    b.Dm = !!a.Dm;
    b.Pc = !!a.Pc;
    b.qm = !!a.qm;
    b.Q = a.Q || [];
    a = 0;
    for (p = b.Q.length; a < p; a++)
      (b.Q[a][1] = e.getUniformLocation(f, b.Q[a][0])),
        b.Qk.push(0),
        e.uniform1f(b.Q[a][1], 0);
    return b;
  };
  d.prototype.tk = function (a) {
    var f, r;
    f = 0;
    for (r = this.Ga.length; f < r; f++) if (this.Ga[f].name === a) return f;
    return -1;
  };
  d.prototype.uo = function (a, f, r) {
    var e = this.oc,
      b = this.Qi,
      p = [0, 0, 0, 0, 0, 0, 0, 0];
    p[0] = e[0] * a + e[4] * f + e[12];
    p[1] = e[1] * a + e[5] * f + e[13];
    p[2] = e[2] * a + e[6] * f + e[14];
    p[3] = e[3] * a + e[7] * f + e[15];
    p[4] = b[0] * p[0] + b[4] * p[1] + b[8] * p[2] + b[12] * p[3];
    p[5] = b[1] * p[0] + b[5] * p[1] + b[9] * p[2] + b[13] * p[3];
    p[6] = b[2] * p[0] + b[6] * p[1] + b[10] * p[2] + b[14] * p[3];
    p[7] = -p[2];
    0 !== p[7] &&
      ((p[7] = 1 / p[7]),
      (p[4] *= p[7]),
      (p[5] *= p[7]),
      (p[6] *= p[7]),
      (r[0] = (0.5 * p[4] + 0.5) * this.width),
      (r[1] = (0.5 * p[5] + 0.5) * this.height));
  };
  d.prototype.gf = function (a, f, r) {
    if (this.width !== a || this.height !== f || r) {
      this.Zd();
      r = this.D;
      this.width = a;
      this.height = f;
      r.viewport(0, 0, a, f);
      $b(this.Rj, this.In, this.dp, this.oc);
      if (this.ma) {
        var e = -a / 2;
        a = a / 2;
        var b = f / 2;
        f = -f / 2;
        var p = this.Hj,
          l = this.$l,
          c = this.Qi;
        c || (c = Xb());
        var d = a - e,
          n = f - b,
          g = l - p;
        c[0] = 2 / d;
        c[1] = 0;
        c[2] = 0;
        c[3] = 0;
        c[4] = 0;
        c[5] = 2 / n;
        c[6] = 0;
        c[7] = 0;
        c[8] = 0;
        c[9] = 0;
        c[10] = -2 / g;
        c[11] = 0;
        c[12] = -(e + a) / d;
        c[13] = -(f + b) / n;
        c[14] = -(l + p) / g;
        c[15] = 1;
        this.Lh[0] = 1;
        this.Lh[1] = 1;
      } else
        (f = a / f),
          (e = this.Hj),
          (a = this.$l),
          (c = this.Qi),
          (l = e * Math.tan((45 * Math.PI) / 360)),
          (f *= l),
          (b = -f),
          (p = -l),
          c || (c = Xb()),
          (d = f - b),
          (n = l - p),
          (g = a - e),
          (c[0] = (2 * e) / d),
          (c[1] = 0),
          (c[2] = 0),
          (c[3] = 0),
          (c[4] = 0),
          (c[5] = (2 * e) / n),
          (c[6] = 0),
          (c[7] = 0),
          (c[8] = (f + b) / d),
          (c[9] = (l + p) / n),
          (c[10] = -(a + e) / g),
          (c[11] = -1),
          (c[12] = 0),
          (c[13] = 0),
          (c[14] = -(a * e * 2) / g),
          (c[15] = 0),
          (e = [0, 0]),
          (a = [0, 0]),
          this.uo(0, 0, e),
          this.uo(1, 1, a),
          (this.Lh[0] = 1 / (a[0] - e[0])),
          (this.Lh[1] = -1 / (a[1] - e[1]));
      e = 0;
      for (a = this.Ga.length; e < a; e++)
        (b = this.Ga[e]),
          (b.Me = !1),
          b.Gn && (r.useProgram(b.Ch), r.uniformMatrix4fv(b.Gn, !1, this.Qi));
      r.useProgram(this.Ga[this.$g].Ch);
      r.bindTexture(r.TEXTURE_2D, null);
      r.activeTexture(r.TEXTURE1);
      r.bindTexture(r.TEXTURE_2D, null);
      r.activeTexture(r.TEXTURE0);
      this.Te = this.je = null;
      this.ei &&
        (r.bindFramebuffer(r.FRAMEBUFFER, this.jk),
        r.bindRenderbuffer(r.RENDERBUFFER, this.ei),
        r.renderbufferStorage(
          r.RENDERBUFFER,
          r.DEPTH_COMPONENT16,
          this.width,
          this.height
        ),
        this.tm ||
          (r.framebufferRenderbuffer(
            r.FRAMEBUFFER,
            r.DEPTH_ATTACHMENT,
            r.RENDERBUFFER,
            this.ei
          ),
          (this.tm = !0)),
        r.bindRenderbuffer(r.RENDERBUFFER, null),
        r.bindFramebuffer(r.FRAMEBUFFER, null),
        (this.fj = null));
    }
  };
  d.prototype.wc = function () {
    $b(this.Rj, this.In, this.dp, this.oc);
    Zb(this.oc, this.Lh);
  };
  d.prototype.translate = function (a, f) {
    if (0 !== a || 0 !== f) {
      this.xe[0] = a;
      this.xe[1] = f;
      this.xe[2] = 0;
      var c = this.oc,
        e = this.xe,
        b = e[0],
        p = e[1],
        e = e[2];
      c[12] = c[0] * b + c[4] * p + c[8] * e + c[12];
      c[13] = c[1] * b + c[5] * p + c[9] * e + c[13];
      c[14] = c[2] * b + c[6] * p + c[10] * e + c[14];
      c[15] = c[3] * b + c[7] * p + c[11] * e + c[15];
    }
  };
  d.prototype.scale = function (a, f) {
    if (1 !== a || 1 !== f)
      (this.xe[0] = a),
        (this.xe[1] = f),
        (this.xe[2] = 1),
        Zb(this.oc, this.xe);
  };
  d.prototype.hj = function (a) {
    if (0 !== a) {
      var f = this.oc,
        c,
        e = Math.sin(a);
      a = Math.cos(a);
      var b = f[0],
        p = f[1],
        l = f[2],
        d = f[3],
        n = f[4],
        m = f[5],
        g = f[6],
        k = f[7];
      c
        ? f !== c &&
          ((c[8] = f[8]),
          (c[9] = f[9]),
          (c[10] = f[10]),
          (c[11] = f[11]),
          (c[12] = f[12]),
          (c[13] = f[13]),
          (c[14] = f[14]),
          (c[15] = f[15]))
        : (c = f);
      c[0] = b * a + n * e;
      c[1] = p * a + m * e;
      c[2] = l * a + g * e;
      c[3] = d * a + k * e;
      c[4] = b * -e + n * a;
      c[5] = p * -e + m * a;
      c[6] = l * -e + g * a;
      c[7] = d * -e + k * a;
    }
  };
  d.prototype.pc = function () {
    if (!q(this.Rk, this.oc)) {
      var a = this.Yb();
      a.type = 5;
      a.P ? Yb(this.oc, a.P) : (a.P = Xb(this.oc));
      Yb(this.oc, this.Rk);
      this.ub = !1;
    }
  };
  d.prototype.kj = function (a) {
    this.ma &&
      (32760 < a && (a = 32760),
      (this.ak = this.Rj[2] - this.Hj - a * this.Rt));
  };
  h.prototype.Nq = function () {
    var a = this.D,
      f = this.A;
    0 !== this.Bb
      ? (a.depthMask(!0),
        a.colorMask(!1, !1, !1, !1),
        a.disable(a.BLEND),
        a.bindFramebuffer(a.FRAMEBUFFER, f.jk),
        a.framebufferTexture2D(
          a.FRAMEBUFFER,
          a.COLOR_ATTACHMENT0,
          a.TEXTURE_2D,
          null,
          0
        ),
        a.clear(a.DEPTH_BUFFER_BIT),
        a.bindFramebuffer(a.FRAMEBUFFER, null),
        (f.vi = !0))
      : (a.depthMask(!1),
        a.colorMask(!0, !0, !0, !0),
        a.enable(a.BLEND),
        (f.vi = !1));
  };
  h.prototype.Rq = function () {
    this.D.bindTexture(this.D.TEXTURE_2D, this.zc);
  };
  h.prototype.Sq = function () {
    var a = this.D;
    a.activeTexture(a.TEXTURE1);
    a.bindTexture(a.TEXTURE_2D, this.zc);
    a.activeTexture(a.TEXTURE0);
  };
  h.prototype.Oq = function () {
    var a = this.mo,
      f = this.A;
    f.ci = a;
    f = f.vf;
    f.Pf && f.Pi !== a && ((f.Pi = a), this.D.uniform1f(f.Pf, a));
  };
  h.prototype.Iq = function () {
    this.D.drawElements(
      this.D.TRIANGLES,
      this.cd,
      this.D.UNSIGNED_SHORT,
      this.Bb
    );
  };
  h.prototype.Kq = function () {
    this.D.blendFunc(this.Bb, this.cd);
  };
  h.prototype.Tq = function () {
    var a,
      f,
      c,
      e = this.A.Ga,
      b = this.A.Em;
    a = 0;
    for (f = e.length; a < f; a++)
      (c = e[a]), a === b && c.Mi ? (c.Vl(this.P), (c.Me = !0)) : (c.Me = !1);
    Yb(this.P, this.A.$j);
  };
  h.prototype.Jq = function () {
    var a = this.D,
      f = this.A;
    this.zc
      ? (f.Te === this.zc &&
          (a.activeTexture(a.TEXTURE1),
          a.bindTexture(a.TEXTURE_2D, null),
          (f.Te = null),
          a.activeTexture(a.TEXTURE0)),
        a.bindFramebuffer(a.FRAMEBUFFER, f.jk),
        f.vi ||
          a.framebufferTexture2D(
            a.FRAMEBUFFER,
            a.COLOR_ATTACHMENT0,
            a.TEXTURE_2D,
            this.zc,
            0
          ))
      : (f.ma ||
          a.framebufferTexture2D(
            a.FRAMEBUFFER,
            a.COLOR_ATTACHMENT0,
            a.TEXTURE_2D,
            null,
            0
          ),
        a.bindFramebuffer(a.FRAMEBUFFER, null));
  };
  h.prototype.Fq = function () {
    var a = this.D,
      f = this.Bb;
    0 === f
      ? (a.clearColor(this.P[0], this.P[1], this.P[2], this.P[3]),
        a.clear(a.COLOR_BUFFER_BIT))
      : 1 === f
      ? (a.enable(a.SCISSOR_TEST),
        a.scissor(this.P[0], this.P[1], this.P[2], this.P[3]),
        a.clearColor(0, 0, 0, 0),
        a.clear(a.COLOR_BUFFER_BIT),
        a.disable(a.SCISSOR_TEST))
      : a.clear(a.DEPTH_BUFFER_BIT);
  };
  h.prototype.Mq = function () {
    var a = this.D;
    0 !== this.Bb ? a.enable(a.DEPTH_TEST) : a.disable(a.DEPTH_TEST);
  };
  h.prototype.Hq = function () {
    var a = this.D,
      f = this.A;
    f.ma && a.disable(a.DEPTH_TEST);
    var c = f.Ga[1];
    a.useProgram(c.Ch);
    !c.Me && c.Mi && (c.Vl(f.$j), (c.Me = !0));
    a.enableVertexAttribArray(c.tc);
    a.bindBuffer(a.ARRAY_BUFFER, f.jl);
    a.vertexAttribPointer(c.tc, 4, a.FLOAT, !1, 0, 0);
    a.drawArrays(a.POINTS, this.Bb / 4, this.cd);
    c = f.vf;
    a.useProgram(c.Ch);
    0 <= c.tc &&
      (a.enableVertexAttribArray(c.tc),
      a.bindBuffer(a.ARRAY_BUFFER, f.Kh[f.od]),
      a.vertexAttribPointer(c.tc, f.ma ? 3 : 2, a.FLOAT, !1, 0, 0));
    0 <= c.ne &&
      (a.enableVertexAttribArray(c.ne),
      a.bindBuffer(a.ARRAY_BUFFER, f.Gh[f.od]),
      a.vertexAttribPointer(c.ne, 2, a.FLOAT, !1, 0, 0));
    f.ma && a.enable(a.DEPTH_TEST);
  };
  h.prototype.Pq = function () {
    var a = this.D,
      f = this.A,
      c = f.Ga[this.Bb];
    f.Em = this.Bb;
    f.vf = c;
    a.useProgram(c.Ch);
    !c.Me && c.Mi && (c.Vl(f.$j), (c.Me = !0));
    c.Pf && c.Pi !== f.ci && ((c.Pi = f.ci), a.uniform1f(c.Pf, f.ci));
    0 <= c.tc &&
      (a.enableVertexAttribArray(c.tc),
      a.bindBuffer(a.ARRAY_BUFFER, f.Kh[f.od]),
      a.vertexAttribPointer(c.tc, f.ma ? 3 : 2, a.FLOAT, !1, 0, 0));
    0 <= c.ne &&
      (a.enableVertexAttribArray(c.ne),
      a.bindBuffer(a.ARRAY_BUFFER, f.Gh[f.od]),
      a.vertexAttribPointer(c.ne, 2, a.FLOAT, !1, 0, 0));
  };
  h.prototype.Lq = function () {
    var a = this.P;
    this.D.uniform4f(this.A.vf.Yk, a[0], a[1], a[2], a[3]);
  };
  h.prototype.Qq = function () {
    var a,
      f,
      c = this.A.vf,
      e = this.D;
    a = this.P;
    c.gh &&
      this.A.Te !== this.zc &&
      (e.activeTexture(e.TEXTURE1),
      e.bindTexture(e.TEXTURE_2D, this.zc),
      (this.A.Te = this.zc),
      e.activeTexture(e.TEXTURE0));
    var b = a[0];
    c.$k && b !== c.Rn && ((c.Rn = b), e.uniform1f(c.$k, b));
    b = a[1];
    c.Zk && b !== c.Qn && ((c.Qn = b), e.uniform1f(c.Zk, b));
    b = a[2];
    f = a[3];
    !c.Xe ||
      (b === c.Ln && f === c.Mn) ||
      ((c.Ln = b), (c.Mn = f), e.uniform2f(c.Xe, b, f));
    b = a[4];
    f = a[5];
    !c.We ||
      (b === c.Jn && f === c.Kn) ||
      ((c.Jn = b), (c.Kn = f), e.uniform2f(c.We, b, f));
    b = a[6];
    c.fh && b !== c.On && ((c.On = b), e.uniform1f(c.fh, b));
    b = a[7];
    c.eh && b !== c.Nn && ((c.Nn = b), e.uniform1f(c.eh, b));
    b = a[8];
    f = a[9];
    !c.jh ||
      (b === c.Vn && f === c.Wn) ||
      ((c.Vn = b), (c.Wn = f), e.uniform2f(c.jh, b, f));
    b = a[10];
    f = a[11];
    !c.hh ||
      (b === c.Sn && f === c.Tn) ||
      ((c.Sn = b), (c.Tn = f), e.uniform2f(c.hh, b, f));
    b = a[12];
    c.ih && b !== c.Un && ((c.Un = b), e.uniform1f(c.ih, b));
    if (c.Q.length)
      for (a = 0, f = c.Q.length; a < f; a++)
        (b = this.Lo[a]),
          b !== c.Qk[a] && ((c.Qk[a] = b), e.uniform1f(c.Q[a][1], b));
  };
  d.prototype.Yb = function () {
    this.md === this.Sd.length && this.Sd.push(new h(0, this));
    return this.Sd[this.md++];
  };
  d.prototype.Zd = function () {
    if (0 !== this.md && !this.D.isContextLost()) {
      var a = this.D;
      0 < this.kl &&
        (a.bindBuffer(a.ARRAY_BUFFER, this.jl),
        a.bufferSubData(a.ARRAY_BUFFER, 0, this.ro.subarray(0, this.kl)),
        f &&
          0 <= f.tc &&
          "<point>" === f.name &&
          a.vertexAttribPointer(f.tc, 4, a.FLOAT, !1, 0, 0));
      if (0 < this.Kd) {
        var f = this.vf;
        a.bindBuffer(a.ARRAY_BUFFER, this.Kh[this.od]);
        a.bufferSubData(a.ARRAY_BUFFER, 0, this.Fj.subarray(0, this.Kd));
        f &&
          0 <= f.tc &&
          "<point>" !== f.name &&
          a.vertexAttribPointer(f.tc, this.ma ? 3 : 2, a.FLOAT, !1, 0, 0);
        a.bindBuffer(a.ARRAY_BUFFER, this.Gh[this.od]);
        a.bufferSubData(a.ARRAY_BUFFER, 0, this.tj.subarray(0, this.bg));
        f &&
          0 <= f.ne &&
          "<point>" !== f.name &&
          a.vertexAttribPointer(f.ne, 2, a.FLOAT, !1, 0, 0);
      }
      for (var c, a = 0, f = this.md; a < f; a++)
        switch (((c = this.Sd[a]), c.type)) {
          case 1:
            c.Iq();
            break;
          case 2:
            c.Rq();
            break;
          case 3:
            c.Oq();
            break;
          case 4:
            c.Kq();
            break;
          case 5:
            c.Tq();
            break;
          case 6:
            c.Jq();
            break;
          case 7:
            c.Fq();
            break;
          case 8:
            c.Hq();
            break;
          case 9:
            c.Pq();
            break;
          case 10:
            c.Qq();
            break;
          case 11:
            c.Sq();
            break;
          case 12:
            c.Lq();
            break;
          case 13:
            c.Mq();
            break;
          case 14:
            c.Nq();
        }
      this.kl = this.bg = this.Kd = this.md = 0;
      this.vi = this.ub = !1;
      this.od++;
      4 <= this.od && (this.od = 0);
    }
  };
  d.prototype.ff = function (a) {
    if (a !== this.xn && !this.Pg) {
      var f = this.Yb();
      f.type = 3;
      this.xn = f.mo = a;
      this.ub = !1;
    }
  };
  d.prototype.$b = function (a) {
    if (a !== this.je) {
      var f = this.Yb();
      f.type = 2;
      this.je = f.zc = a;
      this.ub = !1;
    }
  };
  d.prototype.ue = function (a, f) {
    if ((a !== this.zn || f !== this.vn) && !this.Pg) {
      var c = this.Yb();
      c.type = 4;
      c.Bb = a;
      c.cd = f;
      this.zn = a;
      this.vn = f;
      this.ub = !1;
    }
  };
  d.prototype.Do = function () {
    this.ue(this.D.ONE, this.D.ONE_MINUS_SRC_ALPHA);
  };
  d.prototype.wh = function (a, f, c, e, b, p, l, d) {
    15992 <= this.Kd && this.Zd();
    var n = this.Kd,
      m = this.bg,
      g = this.Fj,
      k = this.tj,
      u = this.ak;
    if (this.ub) this.Sd[this.md - 1].cd += 6;
    else {
      var h = this.Yb();
      h.type = 1;
      h.Bb = this.ma ? n : (n / 2) * 3;
      h.cd = 6;
      this.ub = !0;
    }
    this.ma
      ? ((g[n++] = a),
        (g[n++] = f),
        (g[n++] = u),
        (g[n++] = c),
        (g[n++] = e),
        (g[n++] = u),
        (g[n++] = b),
        (g[n++] = p),
        (g[n++] = u),
        (g[n++] = l),
        (g[n++] = d),
        (g[n++] = u))
      : ((g[n++] = a),
        (g[n++] = f),
        (g[n++] = c),
        (g[n++] = e),
        (g[n++] = b),
        (g[n++] = p),
        (g[n++] = l),
        (g[n++] = d));
    k[m++] = 0;
    k[m++] = 0;
    k[m++] = 1;
    k[m++] = 0;
    k[m++] = 1;
    k[m++] = 1;
    k[m++] = 0;
    k[m++] = 1;
    this.Kd = n;
    this.bg = m;
  };
  d.prototype.fd = function (a, f, c, e, b, p, l, d, n) {
    15992 <= this.Kd && this.Zd();
    var m = this.Kd,
      g = this.bg,
      k = this.Fj,
      u = this.tj,
      h = this.ak;
    if (this.ub) this.Sd[this.md - 1].cd += 6;
    else {
      var q = this.Yb();
      q.type = 1;
      q.Bb = this.ma ? m : (m / 2) * 3;
      q.cd = 6;
      this.ub = !0;
    }
    var q = n.left,
      t = n.top,
      F = n.right;
    n = n.bottom;
    this.ma
      ? ((k[m++] = a),
        (k[m++] = f),
        (k[m++] = h),
        (k[m++] = c),
        (k[m++] = e),
        (k[m++] = h),
        (k[m++] = b),
        (k[m++] = p),
        (k[m++] = h),
        (k[m++] = l),
        (k[m++] = d),
        (k[m++] = h))
      : ((k[m++] = a),
        (k[m++] = f),
        (k[m++] = c),
        (k[m++] = e),
        (k[m++] = b),
        (k[m++] = p),
        (k[m++] = l),
        (k[m++] = d));
    u[g++] = q;
    u[g++] = t;
    u[g++] = F;
    u[g++] = t;
    u[g++] = F;
    u[g++] = n;
    u[g++] = q;
    u[g++] = n;
    this.Kd = m;
    this.bg = g;
  };
  d.prototype.Mb = function (a) {
    if (this.$g !== a) {
      if (!this.Ga[a]) {
        if (0 === this.$g) return;
        a = 0;
      }
      var c = this.Yb();
      c.type = 9;
      this.$g = c.Bb = a;
      this.ub = !1;
    }
  };
  d.prototype.sh = function (a) {
    a = this.Ga[a];
    return !(!a.Xe && !a.We);
  };
  d.prototype.ol = function (a) {
    a = this.Ga[a];
    return !!(a.Xe || a.We || a.Dm);
  };
  d.prototype.nl = function (a) {
    return this.Ga[a].Pc;
  };
  d.prototype.jt = function (a) {
    a = this.Ga[a];
    return 0 !== a.hk || 0 !== a.ik;
  };
  d.prototype.as = function (a) {
    return this.Ga[a].hk;
  };
  d.prototype.bs = function (a) {
    return this.Ga[a].ik;
  };
  d.prototype.cs = function (a, c) {
    return this.Ga[a].Q[c][2];
  };
  d.prototype.$i = function (a) {
    return this.Ga[a].qm;
  };
  d.prototype.Yf = function (a, c, d, e, b, p, l, n, m, h, g, k, u, q, t) {
    var T = this.Ga[this.$g],
      F,
      C;
    if (T.zs || t.length) {
      F = this.Yb();
      F.type = 10;
      F.P ? Yb(this.oc, F.P) : (F.P = Xb());
      C = F.P;
      C[0] = c;
      C[1] = d;
      C[2] = e;
      C[3] = b;
      C[4] = p;
      C[5] = l;
      C[6] = n;
      C[7] = m;
      C[8] = h;
      C[9] = g;
      C[10] = k;
      C[11] = u;
      C[12] = q;
      T.gh ? (F.zc = a) : (F.zc = null);
      if (t.length)
        for (d = F.Lo, d.length = t.length, a = 0, c = t.length; a < c; a++)
          d[a] = t[a];
      this.ub = !1;
    }
  };
  d.prototype.clear = function (a, c, d, e) {
    var b = this.Yb();
    b.type = 7;
    b.Bb = 0;
    b.P || (b.P = Xb());
    b.P[0] = a;
    b.P[1] = c;
    b.P[2] = d;
    b.P[3] = e;
    this.ub = !1;
  };
  d.prototype.clearRect = function (a, c, d, e) {
    if (!(0 > d || 0 > e)) {
      var b = this.Yb();
      b.type = 7;
      b.Bb = 1;
      b.P || (b.P = Xb());
      b.P[0] = a;
      b.P[1] = c;
      b.P[2] = d;
      b.P[3] = e;
      this.ub = !1;
    }
  };
  d.prototype.Go = function (a) {
    if (this.ma && ((a = !!a), this.Pg !== a)) {
      var c = this.Yb();
      c.type = 14;
      c.Bb = a ? 1 : 0;
      this.ub = !1;
      this.Pg = a;
      this.fj = null;
      this.Pg ? this.Mb(2) : this.Mb(0);
    }
  };
  d.prototype.Fo = function (a) {
    if (this.ma) {
      var c = this.Yb();
      c.type = 13;
      c.Bb = a ? 1 : 0;
      this.ub = !1;
    }
  };
  d.prototype.Om = function () {
    Yb(this.Rk, n);
    this.wc();
    this.pc();
    var a = this.width / 2,
      c = this.height / 2;
    this.wh(-a, c, a, c, a, -c, -a, -c);
    Yb(n, this.oc);
    this.pc();
  };
  d.prototype.Eo = function (a, c, d) {
    this.Mb(3);
    var e = this.Yb();
    e.type = 12;
    e.P || (e.P = Xb());
    e.P[0] = a;
    e.P[1] = c;
    e.P[2] = d;
    e.P[3] = 1;
    this.ub = !1;
  };
  d.prototype.Bt = function () {
    this.Mb(0);
  };
  d.prototype.nt = function () {
    this.Mb(2);
  };
  d.prototype.it = function () {
    this.Zd();
    this.D.flush();
  };
  var t = [],
    c = {};
  d.prototype.wq = function () {
    V(t);
    c = {};
  };
  d.prototype.Ki = function (a, f, d, e) {
    f = !!f;
    d = !!d;
    var b = a.src + "," + f + "," + d + (f ? ",undefined" : ""),
      p = null;
    if ("undefined" !== typeof a.src && c.hasOwnProperty(b))
      return (p = c[b]), p.Vh++, p;
    this.Zd();
    var l = this.D,
      n = ia(a.width) && ia(a.height),
      p = l.createTexture();
    l.bindTexture(l.TEXTURE_2D, p);
    l.pixelStorei(l.UNPACK_PREMULTIPLY_ALPHA_WEBGL, !0);
    var m = l.RGBA,
      h = l.RGBA,
      g = l.UNSIGNED_BYTE;
    if (e && !this.fe)
      switch (e) {
        case 1:
          h = m = l.RGB;
          break;
        case 2:
          g = l.UNSIGNED_SHORT_4_4_4_4;
          break;
        case 3:
          g = l.UNSIGNED_SHORT_5_5_5_1;
          break;
        case 4:
          (h = m = l.RGB), (g = l.UNSIGNED_SHORT_5_6_5);
      }
    if (1 === this.version && !n && f) {
      e = document.createElement("canvas");
      e.width = ja(a.width);
      e.height = ja(a.height);
      var k = e.getContext("2d");
      "undefined" !== typeof k.imageSmoothingEnabled
        ? (k.imageSmoothingEnabled = d)
        : ((k.webkitImageSmoothingEnabled = d),
          (k.mozImageSmoothingEnabled = d),
          (k.msImageSmoothingEnabled = d));
      k.drawImage(a, 0, 0, a.width, a.height, 0, 0, e.width, e.height);
      l.texImage2D(l.TEXTURE_2D, 0, m, h, g, e);
    } else l.texImage2D(l.TEXTURE_2D, 0, m, h, g, a);
    f
      ? (l.texParameteri(l.TEXTURE_2D, l.TEXTURE_WRAP_S, l.REPEAT),
        l.texParameteri(l.TEXTURE_2D, l.TEXTURE_WRAP_T, l.REPEAT))
      : (l.texParameteri(l.TEXTURE_2D, l.TEXTURE_WRAP_S, l.CLAMP_TO_EDGE),
        l.texParameteri(l.TEXTURE_2D, l.TEXTURE_WRAP_T, l.CLAMP_TO_EDGE));
    d
      ? (l.texParameteri(l.TEXTURE_2D, l.TEXTURE_MAG_FILTER, l.LINEAR),
        (n || 2 <= this.version) && this.Hm
          ? (l.texParameteri(
              l.TEXTURE_2D,
              l.TEXTURE_MIN_FILTER,
              l.LINEAR_MIPMAP_LINEAR
            ),
            l.generateMipmap(l.TEXTURE_2D))
          : l.texParameteri(l.TEXTURE_2D, l.TEXTURE_MIN_FILTER, l.LINEAR))
      : (l.texParameteri(l.TEXTURE_2D, l.TEXTURE_MAG_FILTER, l.NEAREST),
        l.texParameteri(l.TEXTURE_2D, l.TEXTURE_MIN_FILTER, l.NEAREST));
    l.bindTexture(l.TEXTURE_2D, null);
    this.je = null;
    p.Ce = a.width;
    p.Be = a.height;
    p.Vh = 1;
    p.xm = b;
    t.push(p);
    return (c[b] = p);
  };
  d.prototype.hc = function (a, c, d, e) {
    this.Zd();
    var b = this.D;
    this.fe && (e = !1);
    var p = b.createTexture();
    b.bindTexture(b.TEXTURE_2D, p);
    b.texImage2D(
      b.TEXTURE_2D,
      0,
      b.RGBA,
      a,
      c,
      0,
      b.RGBA,
      e ? b.UNSIGNED_SHORT_4_4_4_4 : b.UNSIGNED_BYTE,
      null
    );
    b.texParameteri(b.TEXTURE_2D, b.TEXTURE_WRAP_S, b.CLAMP_TO_EDGE);
    b.texParameteri(b.TEXTURE_2D, b.TEXTURE_WRAP_T, b.CLAMP_TO_EDGE);
    b.texParameteri(
      b.TEXTURE_2D,
      b.TEXTURE_MAG_FILTER,
      d ? b.LINEAR : b.NEAREST
    );
    b.texParameteri(
      b.TEXTURE_2D,
      b.TEXTURE_MIN_FILTER,
      d ? b.LINEAR : b.NEAREST
    );
    b.bindTexture(b.TEXTURE_2D, null);
    this.je = null;
    p.Ce = a;
    p.Be = c;
    t.push(p);
    return p;
  };
  d.prototype.Pt = function (a, c, d) {
    this.Zd();
    var e = this.D;
    this.fe && (d = !1);
    e.bindTexture(e.TEXTURE_2D, c);
    e.pixelStorei(e.UNPACK_PREMULTIPLY_ALPHA_WEBGL, !0);
    try {
      e.texImage2D(
        e.TEXTURE_2D,
        0,
        e.RGBA,
        e.RGBA,
        d ? e.UNSIGNED_SHORT_4_4_4_4 : e.UNSIGNED_BYTE,
        a
      );
    } catch (b) {
      console &&
        console.error &&
        console.error("Error updating WebGL texture: ", b);
    }
    e.bindTexture(e.TEXTURE_2D, null);
    this.je = null;
  };
  d.prototype.deleteTexture = function (a) {
    a &&
      ("undefined" !== typeof a.Vh && 1 < a.Vh
        ? a.Vh--
        : (this.Zd(),
          a === this.je &&
            (this.D.bindTexture(this.D.TEXTURE_2D, null), (this.je = null)),
          a === this.Te &&
            (this.D.activeTexture(this.D.TEXTURE1),
            this.D.bindTexture(this.D.TEXTURE_2D, null),
            this.D.activeTexture(this.D.TEXTURE0),
            (this.Te = null)),
          Ba(t, a),
          "undefined" !== typeof a.xm && delete c[a.xm],
          this.D.deleteTexture(a)));
  };
  d.prototype.xc = function (a) {
    if (a !== this.fj) {
      var c = this.Yb();
      c.type = 6;
      this.fj = c.zc = a;
      this.ub = !1;
    }
  };
  db = d;
})();
(function () {
  function d(a) {
    if (a && (a.getContext || a.dc) && !a.c2runtime) {
      a.c2runtime = this;
      var e = this;
      this.ee =
        (this.Fk =
          /crosswalk/i.test(navigator.userAgent) ||
          /xwalk/i.test(navigator.userAgent) ||
          !(
            "undefined" === typeof window.c2isCrosswalk || !window.c2isCrosswalk
          )) ||
        ("undefined" !== typeof window.device &&
          ("undefined" !== typeof window.device.cordova ||
            "undefined" !== typeof window.device.phonegap)) ||
        ("undefined" !== typeof window.c2iscordova && window.c2iscordova);
      this.Qe = !!a.dc;
      this.Hs = "undefined" !== typeof window.AppMobi || this.Qe;
      this.Jc = !!window.c2cocoonjs;
      this.Kc = !!window.c2ejecta;
      this.Jc &&
        (CocoonJS.App.onSuspended.addEventListener(function () {
          e.setSuspended(!0);
        }),
        CocoonJS.App.onActivated.addEventListener(function () {
          e.setSuspended(!1);
        }));
      this.Kc &&
        (document.addEventListener("pagehide", function () {
          e.setSuspended(!0);
        }),
        document.addEventListener("pageshow", function () {
          e.setSuspended(!1);
        }),
        document.addEventListener("resize", function () {
          e.setSize(window.innerWidth, window.innerHeight);
        }));
      this.Ha = this.Qe || this.Jc || this.Kc;
      this.wi = /edge\//i.test(navigator.userAgent);
      this.fe =
        (/msie/i.test(navigator.userAgent) ||
          /trident/i.test(navigator.userAgent) ||
          /iemobile/i.test(navigator.userAgent)) &&
        !this.wi;
      this.mn = /tizen/i.test(navigator.userAgent);
      this.Ek =
        /android/i.test(navigator.userAgent) &&
        !this.mn &&
        !this.fe &&
        !this.wi;
      this.sn =
        (/iphone/i.test(navigator.userAgent) ||
          /ipod/i.test(navigator.userAgent)) &&
        !this.fe &&
        !this.wi;
      this.Os = /ipad/i.test(navigator.userAgent);
      this.Ok = this.sn || this.Os || this.Kc;
      this.Js =
        (/chrome/i.test(navigator.userAgent) ||
          /chromium/i.test(navigator.userAgent)) &&
        !this.fe &&
        !this.wi;
      this.Gs = /amazonwebappplatform/i.test(navigator.userAgent);
      this.Ks = /firefox/i.test(navigator.userAgent);
      this.Ik =
        "undefined" !== typeof window.c2nodewebkit ||
        "undefined" !== typeof window.c2nwjs ||
        /nodewebkit/i.test(navigator.userAgent) ||
        /nwjs/i.test(navigator.userAgent);
      this.qn = !(
        "undefined" === typeof window.c2isWindows8 || !window.c2isWindows8
      );
      this.Ns = !(
        "undefined" === typeof window.c2isWindows8Capable ||
        !window.c2isWindows8Capable
      );
      this.Sg = !(
        "undefined" === typeof window.c2isWindowsPhone8 ||
        !window.c2isWindowsPhone8
      );
      this.Mk = !(
        "undefined" === typeof window.c2isWindowsPhone81 ||
        !window.c2isWindowsPhone81
      );
      this.pn = !!window.cr_windows10;
      this.Ms = this.qn || this.Ns || this.Mk || this.pn;
      this.Is = !(
        "undefined" === typeof window.c2isBlackberry10 ||
        !window.c2isBlackberry10
      );
      this.kn =
        this.Ek && !this.Js && !this.Fk && !this.Ks && !this.Gs && !this.Ha;
      this.devicePixelRatio = 1;
      this.Re =
        this.ee ||
        this.Fk ||
        this.Hs ||
        this.Jc ||
        this.Ek ||
        this.Ok ||
        this.Sg ||
        this.Mk ||
        this.Is ||
        this.mn ||
        this.Kc;
      this.Re ||
        (this.Re =
          /(blackberry|bb10|playbook|palm|symbian|nokia|windows\s+ce|phone|mobile|tablet|kindle|silk)/i.test(
            navigator.userAgent
          ));
      this.nn = !!(this.Ok && this.ee && window.webkit);
      "undefined" === typeof cr_is_preview ||
        this.Ik ||
        ("?nw" !== window.location.search &&
          !/nodewebkit/i.test(navigator.userAgent) &&
          !/nwjs/i.test(navigator.userAgent)) ||
        (this.Ik = !0);
      this.canvas = a;
      this.Yh = document.getElementById("c2canvasdiv");
      this.A = this.D = null;
      this.uk = "(unavailable)";
      this.ma = !1;
      this.He = 0;
      this.ya = null;
      this.ki = !1;
      this.jo = this.ko = 0;
      this.canvas.oncontextmenu = function (a) {
        a.preventDefault && a.preventDefault();
        return !1;
      };
      this.canvas.onselectstart = function (a) {
        a.preventDefault && a.preventDefault();
        return !1;
      };
      this.canvas.ontouchstart = function (a) {
        a.preventDefault && a.preventDefault();
        return !1;
      };
      this.Qe && (window.c2runtime = this);
      this.Ik &&
        ((window.ondragover = function (a) {
          a.preventDefault();
          return !1;
        }),
        (window.ondrop = function (a) {
          a.preventDefault();
          return !1;
        }),
        window.nwgui &&
          window.nwgui.App.clearCache &&
          window.nwgui.App.clearCache());
      this.kn &&
        "undefined" !== typeof jQuery &&
        jQuery("canvas").parents("*").css("overflow", "visible");
      this.width = a.width;
      this.height = a.height;
      this.J = this.width;
      this.I = this.height;
      this.ai = this.width;
      this.wg = this.height;
      this.Nf = window.innerWidth;
      this.Mf = window.innerHeight;
      this.ca = !0;
      this.Rg = !1;
      Date.now ||
        (Date.now = function () {
          return +new Date();
        });
      this.plugins = [];
      this.types = {};
      this.C = [];
      this.Da = [];
      this.Tk = {};
      this.Mc = [];
      this.gk = {};
      this.rd = [];
      this.ig = [];
      this.Aj = [];
      this.jq = [];
      this.kq = [];
      this.tn = this.Ro = null;
      this.Ee = {};
      this.Hk = this.be = !1;
      this.Lc = 0;
      this.Gk = this.Kk = !1;
      this.Gc = [];
      this.Qg = !1;
      this.Ii = this.Al = "";
      this.Sa = null;
      this.vd = "";
      this.Fh = this.Oo = !1;
      this.ji = [];
      this.Ge = this.xf = 0;
      this.ao = 30;
      this.Zj = this.kh = 0;
      this.eg = 1;
      this.Vb = new Ta();
      this.Gj = new Ta();
      this.Vi = this.oi = this.Le = this.kd = this.Ve = this.lk = this.Bi = 0;
      this.rg = null;
      this.fi = [];
      this.fk = [];
      this.gi = -1;
      this.al = [[]];
      this.Pl = this.Ni = 0;
      this.bj(null);
      this.Bg = this.eo = this.nh = 0;
      this.rj = [];
      this.Ml = this.rl = -1;
      this.Ug = !0;
      this.Li = 0;
      this.xi = !1;
      this.Et = 0;
      this.pg = null;
      this.Ff = !1;
      this.ho = new ca();
      this.cl = new ca();
      this.io = new ca();
      this.ef = [];
      this.Tc = new Va([]);
      this.Il = new Va([]);
      this.tf = [];
      this.si = {};
      this.Vd = {};
      this.Pd = {};
      this.hg = {};
      this.um = {};
      this.Fn =
        this.Hi =
        this.La =
        this.Qa =
        this.En =
        this.Gi =
        this.ja =
          null;
      this.fg = this.Nk = !1;
      this.mk = [null, null];
      this.Cf = 0;
      this.yd = {};
      this.Eh = this.le = null;
      this.Po = "";
      this.Ui = [];
      this.mt();
    }
  }
  function m(a, e) {
    return 128 >= e ? a[3] : 256 >= e ? a[2] : 512 >= e ? a[1] : a[0];
  }
  function q() {
    try {
      return !!window.indexedDB;
    } catch (a) {
      return !1;
    }
  }
  function h(a) {
    a.target.result.createObjectStore("saves", { keyPath: "slot" });
  }
  function n(a, e, g, b) {
    try {
      var c = indexedDB.open("_C2SaveStates");
      c.onupgradeneeded = h;
      c.onerror = b;
      c.onsuccess = function (c) {
        c = c.target.result;
        c.onerror = b;
        c
          .transaction(["saves"], "readwrite")
          .objectStore("saves")
          .put({ slot: a, data: e }).onsuccess = g;
      };
    } catch (l) {
      b(l);
    }
  }
  function t(a, e, g) {
    try {
      var b = indexedDB.open("_C2SaveStates");
      b.onupgradeneeded = h;
      b.onerror = g;
      b.onsuccess = function (b) {
        b = b.target.result;
        b.onerror = g;
        var c = b.transaction(["saves"]).objectStore("saves").get(a);
        c.onsuccess = function () {
          c.result ? e(c.result.data) : e(null);
        };
      };
    } catch (c) {
      g(c);
    }
  }
  function c() {
    w("Reloading for continuous preview");
    window.c2cocoonjs
      ? CocoonJS.App.reload()
      : -1 < window.location.search.indexOf("continuous")
      ? window.location.reload(!0)
      : (window.location = window.location + "?continuous");
  }
  function a(a) {
    var e,
      g = {};
    for (e in a)
      !a.hasOwnProperty(e) ||
        a[e] instanceof ca ||
        (a[e] && "undefined" !== typeof a[e].Vu) ||
        ("spriteCreatedDestroyCallback" !== e && (g[e] = a[e]));
    return g;
  }
  var f =
    window.requestAnimationFrame ||
    window.mozRequestAnimationFrame ||
    window.webkitRequestAnimationFrame ||
    window.msRequestAnimationFrame ||
    window.oRequestAnimationFrame;
  d.prototype.mt = function () {
    var a = this;
    if (this.nn)
      this.Mr(
        function (e) {
          a.dh(JSON.parse(e));
        },
        function () {
          alert("Error fetching data.js");
        }
      );
    else {
      var e;
      this.Sg
        ? (e = new ActiveXObject("Microsoft.XMLHTTP"))
        : (e = new XMLHttpRequest());
      var g = "data.js";
      if (this.qn || this.Sg || this.Mk || this.pn) g = "data.json";
      e.open("GET", g, !0);
      var b = !1;
      if (!this.Ha && "response" in e && "responseType" in e)
        try {
          (e.responseType = "json"), (b = "json" === e.responseType);
        } catch (c) {
          b = !1;
        }
      if (!b && "responseType" in e)
        try {
          e.responseType = "text";
        } catch (l) {}
      if ("overrideMimeType" in e)
        try {
          e.overrideMimeType("application/json; charset=utf-8");
        } catch (f) {}
      this.Sg
        ? (e.onreadystatechange = function () {
            4 === e.readyState && a.dh(JSON.parse(e.responseText));
          })
        : ((e.onload = function () {
            if (b) a.dh(e.response);
            else if (a.Kc) {
              var g = e.responseText,
                g = g.substr(g.indexOf("{"));
              a.dh(JSON.parse(g));
            } else a.dh(JSON.parse(e.responseText));
          }),
          (e.onerror = function (a) {
            ea("Error requesting " + g + ":");
            ea(a);
          }));
      e.send();
    }
  };
  d.prototype.Cs = function () {
    var a = this,
      e,
      g,
      b,
      c,
      l,
      f,
      k,
      p,
      d;
    this.Se = (!this.Ha || this.Kc || this.ee) && this.Nt && !this.kn;
    0 === this.Sb && this.Ok && (this.Se = !1);
    this.devicePixelRatio = this.Se
      ? window.devicePixelRatio ||
        window.webkitDevicePixelRatio ||
        window.mozDevicePixelRatio ||
        window.msDevicePixelRatio ||
        1
      : 1;
    "object" === typeof window.StatusBar && window.StatusBar.hide();
    this.lb();
    0 < this.Sb && this.setSize(window.innerWidth, window.innerHeight, !0);
    this.canvas.addEventListener(
      "webglcontextlost",
      function (e) {
        e.preventDefault();
        a.Ws();
        w("[Construct 2] WebGL context lost");
        window.cr_setSuspended(!0);
      },
      !1
    );
    this.canvas.addEventListener(
      "webglcontextrestored",
      function () {
        a.A.cn();
        a.A.gf(a.A.width, a.A.height, !0);
        a.Qa = null;
        a.La = null;
        a.mk[0] = null;
        a.mk[1] = null;
        a.Xs();
        a.ca = !0;
        w("[Construct 2] WebGL context restored");
        window.cr_setSuspended(!1);
      },
      !1
    );
    try {
      this.br &&
        (this.Jc || this.Kc || !this.Ha) &&
        ((e = {
          alpha: !0,
          depth: !1,
          antialias: !1,
          powerPreference: "high-performance",
          failIfMajorPerformanceCaveat: !0,
        }),
        this.Ek || (this.D = this.canvas.getContext("webgl2", e)),
        this.D ||
          (this.D =
            this.canvas.getContext("webgl", e) ||
            this.canvas.getContext("experimental-webgl", e)));
    } catch (n) {}
    if (this.D) {
      if ((e = this.D.getExtension("WEBGL_debug_renderer_info")))
        this.uk =
          this.D.getParameter(e.UNMASKED_RENDERER_WEBGL) +
          " [" +
          this.D.getParameter(e.UNMASKED_VENDOR_WEBGL) +
          "]";
      this.ma && (this.uk += " [front-to-back enabled]");
      this.Ha ||
        ((this.ib = document.createElement("canvas")),
        jQuery(this.ib).appendTo(this.canvas.parentNode),
        (this.ib.oncontextmenu = function () {
          return !1;
        }),
        (this.ib.onselectstart = function () {
          return !1;
        }),
        (this.ib.width = Math.round(this.ai * this.devicePixelRatio)),
        (this.ib.height = Math.round(this.wg * this.devicePixelRatio)),
        jQuery(this.ib).css({ width: this.ai + "px", height: this.wg + "px" }),
        this.so(),
        (this.il = this.ib.getContext("2d")));
      this.A = new db(this.D, this.Re, this.ma);
      this.A.gf(this.canvas.width, this.canvas.height);
      this.A.Hm = 0 !== this.Uq;
      this.ya = null;
      e = 0;
      for (g = this.C.length; e < g; e++)
        for (l = this.C[e], b = 0, c = l.N.length; b < c; b++)
          (k = l.N[b]),
            (k.Na = this.A.tk(k.id)),
            (k.Pc = this.A.nl(k.Na)),
            (this.fg = this.fg || this.A.sh(k.Na));
      e = 0;
      for (g = this.Mc.length; e < g; e++) {
        p = this.Mc[e];
        b = 0;
        for (c = p.N.length; b < c; b++)
          (k = p.N[b]), (k.Na = this.A.tk(k.id)), (k.Pc = this.A.nl(k.Na));
        p.Vc();
        b = 0;
        for (c = p.O.length; b < c; b++) {
          d = p.O[b];
          l = 0;
          for (f = d.N.length; l < f; l++)
            (k = d.N[l]),
              (k.Na = this.A.tk(k.id)),
              (k.Pc = this.A.nl(k.Na)),
              (this.fg = this.fg || this.A.sh(k.Na));
          d.Vc();
        }
      }
    } else {
      if (0 < this.Sb && this.Qe) {
        this.canvas = null;
        document.oncontextmenu = function () {
          return !1;
        };
        document.onselectstart = function () {
          return !1;
        };
        this.ya = AppMobi.canvas.getContext("2d");
        try {
          (this.ya.samplingMode = this.Aa ? "smooth" : "sharp"),
            (this.ya.globalScale = 1),
            (this.ya.HTML5CompatibilityMode = !0),
            (this.ya.imageSmoothingEnabled = this.Aa);
        } catch (m) {}
        0 !== this.width &&
          0 !== this.height &&
          ((this.ya.width = this.width), (this.ya.height = this.height));
      }
      this.ya ||
        (this.Jc
          ? ((e = { antialias: !!this.Aa, alpha: !0 }),
            (this.ya = this.canvas.getContext("2d", e)))
          : ((e = { alpha: !0 }), (this.ya = this.canvas.getContext("2d", e))),
        this.jj(this.ya, this.Aa));
      this.il = this.ib = null;
    }
    this.Xo = function (e) {
      a.Ob(!1, e);
    };
    window == window.top ||
      this.Ha ||
      this.Ms ||
      this.Sg ||
      (document.addEventListener(
        "mousedown",
        function () {
          window.focus();
        },
        !0
      ),
      document.addEventListener(
        "touchstart",
        function () {
          window.focus();
        },
        !0
      ));
    "undefined" !== typeof cr_is_preview &&
      (this.Jc &&
        console.log("[Construct 2] In preview-over-wifi via CocoonJS mode"),
      -1 < window.location.search.indexOf("continuous") &&
        (w("Reloading for continuous preview"),
        (this.Ii = "__c2_continuouspreview"),
        (this.Fh = !0)),
      this.bt &&
        !this.Re &&
        (jQuery(window).focus(function () {
          a.setSuspended(!1);
        }),
        jQuery(window).blur(function () {
          var e = window.parent;
          (e && e.document.hasFocus()) || a.setSuspended(!0);
        })));
    window.addEventListener("blur", function () {
      a.cf();
    });
    this.Ha ||
      ((e = function (a) {
        if (
          cb(a) &&
          document.activeElement &&
          document.activeElement !== document.getElementsByTagName("body")[0] &&
          document.activeElement.blur
        )
          try {
            document.activeElement.blur();
          } catch (e) {}
      }),
      "undefined" !== typeof PointerEvent
        ? document.addEventListener("pointerdown", e)
        : window.navigator.msPointerEnabled
        ? document.addEventListener("MSPointerDown", e)
        : document.addEventListener("touchstart", e),
      document.addEventListener("mousedown", e));
    0 === this.Sb &&
      this.Se &&
      1 < this.devicePixelRatio &&
      this.setSize(this.Ua, this.Ta, !0);
    this.bp();
    this.ws();
    this.go();
    this.M = {};
  };
  d.prototype.setSize = function (a, e, g) {
    var b = 0,
      c = 0,
      l = 0,
      f = 0,
      f = 0;
    if (this.Nf !== a || this.Mf !== e || g) {
      this.Nf = a;
      this.Mf = e;
      var k = this.Sb;
      if (
        (l =
          (document.mozFullScreen ||
            document.webkitIsFullScreen ||
            !!document.msFullscreenElement ||
            document.fullScreen ||
            this.xi) &&
          !this.ee) ||
        0 !== this.Sb ||
        g
      )
        l && (k = this.Cf),
          (g = this.devicePixelRatio),
          4 <= k
            ? (5 === k && 1 !== g && ((a += 1), (e += 1)),
              (l = this.Ua / this.Ta),
              a / e > l
                ? ((l *= e),
                  5 === k
                    ? ((f = (l * g) / this.Ua),
                      1 < f
                        ? (f = Math.floor(f))
                        : 1 > f && (f = 1 / Math.ceil(1 / f)),
                      (l = (this.Ua * f) / g),
                      (f = (this.Ta * f) / g),
                      (b = (a - l) / 2),
                      (c = (e - f) / 2),
                      (a = l),
                      (e = f))
                    : ((b = (a - l) / 2), (a = l)))
                : ((f = a / l),
                  5 === k
                    ? ((f = (f * g) / this.Ta),
                      1 < f
                        ? (f = Math.floor(f))
                        : 1 > f && (f = 1 / Math.ceil(1 / f)),
                      (l = (this.Ua * f) / g),
                      (f = (this.Ta * f) / g),
                      (b = (a - l) / 2),
                      (c = (e - f) / 2),
                      (a = l))
                    : (c = (e - f) / 2),
                  (e = f)))
            : l &&
              0 === k &&
              ((b = Math.floor((a - this.Ua) / 2)),
              (c = Math.floor((e - this.Ta) / 2)),
              (a = this.Ua),
              (e = this.Ta)),
          2 > k && (this.og = g),
          (this.ai = Math.round(a)),
          (this.wg = Math.round(e)),
          (this.width = Math.round(a * g)),
          (this.height = Math.round(e * g)),
          (this.ca = !0),
          this.np
            ? ((this.J = this.width), (this.I = this.height), (this.jc = !0))
            : (this.width < this.Ua && this.height < this.Ta) || 1 === k
            ? ((this.J = this.width), (this.I = this.height), (this.jc = !0))
            : ((this.J = this.Ua),
              (this.I = this.Ta),
              (this.jc = !1),
              2 === k
                ? ((l = this.Ua / this.Ta),
                  (k = this.Nf / this.Mf),
                  k < l
                    ? (this.J = this.I * k)
                    : k > l && (this.I = this.J / k))
                : 3 === k &&
                  ((l = this.Ua / this.Ta),
                  (k = this.Nf / this.Mf),
                  k > l
                    ? (this.J = this.I * k)
                    : k < l && (this.I = this.J / k))),
          this.Yh &&
            !this.Ha &&
            (jQuery(this.Yh).css({
              width: Math.round(a) + "px",
              height: Math.round(e) + "px",
              "margin-left": Math.floor(b) + "px",
              "margin-top": Math.floor(c) + "px",
            }),
            "undefined" !== typeof cr_is_preview &&
              jQuery("#borderwrap").css({
                width: Math.round(a) + "px",
                height: Math.round(e) + "px",
              })),
          this.canvas &&
            ((this.canvas.width = Math.round(a * g)),
            (this.canvas.height = Math.round(e * g)),
            this.Kc
              ? ((this.canvas.style.left = Math.floor(b) + "px"),
                (this.canvas.style.top = Math.floor(c) + "px"),
                (this.canvas.style.width = Math.round(a) + "px"),
                (this.canvas.style.height = Math.round(e) + "px"))
              : this.Se &&
                !this.Ha &&
                ((this.canvas.style.width = Math.round(a) + "px"),
                (this.canvas.style.height = Math.round(e) + "px"))),
          this.ib &&
            ((this.ib.width = Math.round(a * g)),
            (this.ib.height = Math.round(e * g)),
            (this.ib.style.width = this.ai + "px"),
            (this.ib.style.height = this.wg + "px")),
          this.A && this.A.gf(Math.round(a * g), Math.round(e * g)),
          this.Qe &&
            this.ya &&
            ((this.ya.width = Math.round(a)), (this.ya.height = Math.round(e))),
          this.ya && this.jj(this.ya, this.Aa),
          this.bp(),
          this.sn && !this.ee && window.scrollTo(0, 0);
    }
  };
  d.prototype.bp = function () {
    if (this.nq && 0 !== this.hl) {
      var a = "portrait";
      2 === this.hl && (a = "landscape");
      try {
        screen.orientation && screen.orientation.lock
          ? screen.orientation.lock(a).catch(function () {})
          : screen.lockOrientation
          ? screen.lockOrientation(a)
          : screen.webkitLockOrientation
          ? screen.webkitLockOrientation(a)
          : screen.mozLockOrientation
          ? screen.mozLockOrientation(a)
          : screen.msLockOrientation && screen.msLockOrientation(a);
      } catch (e) {
        console &&
          console.warn &&
          console.warn("Failed to lock orientation: ", e);
      }
    }
  };
  d.prototype.Ws = function () {
    this.A.wq();
    this.Nk = !0;
    var a, e, g;
    a = 0;
    for (e = this.C.length; a < e; a++) (g = this.C[a]), g.Wi && g.Wi();
  };
  d.prototype.Xs = function () {
    this.Nk = !1;
    var a, e, g;
    a = 0;
    for (e = this.C.length; a < e; a++) (g = this.C[a]), g.gl && g.gl();
  };
  d.prototype.so = function () {
    if (!this.Ha) {
      var a =
        (document.mozFullScreen ||
          document.webkitIsFullScreen ||
          document.fullScreen ||
          document.msFullscreenElement ||
          this.xi) &&
        !this.ee
          ? jQuery(this.canvas).offset()
          : jQuery(this.canvas).position();
      a.position = "absolute";
      jQuery(this.ib).css(a);
    }
  };
  var r =
    window.cancelAnimationFrame ||
    window.mozCancelAnimationFrame ||
    window.webkitCancelAnimationFrame ||
    window.msCancelAnimationFrame ||
    window.oCancelAnimationFrame;
  d.prototype.setSuspended = function (a) {
    var e;
    if (a && !this.Rg)
      for (
        w("[Construct 2] Suspending"),
          this.Rg = !0,
          -1 !== this.rl && r && r(this.rl),
          -1 !== this.Ml && clearTimeout(this.Ml),
          a = 0,
          e = this.rj.length;
        a < e;
        a++
      )
        this.rj[a](!0);
    else if (!a && this.Rg) {
      w("[Construct 2] Resuming");
      this.Rg = !1;
      this.Bi = Pa();
      this.Ve = Pa();
      a = this.kh = this.oi = 0;
      for (e = this.rj.length; a < e; a++) this.rj[a](!1);
      this.Ob(!1);
    }
  };
  d.prototype.Nd = function (a) {
    return this.Ui[a];
  };
  d.prototype.dh = function (a) {
    (a && a.project) || ea("Project model unavailable");
    a = a.project;
    this.name = a[0];
    this.Nm = a[1];
    this.Sb = a[12];
    this.Ua = a[10];
    this.Ta = a[11];
    this.no = this.Ua / 2;
    this.oo = this.Ta / 2;
    this.Ha &&
      !this.Kc &&
      (4 <= a[12] || 0 === a[12]) &&
      (w(
        "[Construct 2] Letterbox scale fullscreen modes are not supported on this platform - falling back to 'Scale outer'"
      ),
      (this.Sb = 3));
    this.Xl = a[18];
    this.me = a[19];
    if (0 === this.me) {
      var e = new Image();
      e.crossOrigin = "anonymous";
      this.Ho(e, "loading-logo.png");
      this.le = { Oi: e };
    } else if (4 === this.me) {
      e = new Image();
      e.src =
        "data:image/png;base64,";
      var g = new Image();
      g.src =
        "data:image/png;base64,";
      var b = new Image();
      b.src =
        "data:image/png;base64,";
      var c = new Image();
      c.src =
        "data:image/png;base64,";
      var l = new Image();
      l.src =
        "data:image/png;base64,";
      var f = new Image();
      f.src =
        "data:image/png;base64,";
      var k = new Image();
      k.src =
        "data:image/png;base64,";
      var p = new Image();
      p.src =
        "data:image/png;base64,";
      var d = new Image();
      d.src =
        "data:image/png;base64,";
      var n = new Image();
      n.src =
        "data:image/png;base64,";
      var m = new Image();
      m.src =
        "data:image/png;base64,";
      var u = new Image();
      u.src =
        "data:image/png;base64,";
      this.le = { Oi: [e, g, b, c], et: [l, f, k, p], Qt: [d, n, m, u] };
    }
    this.nh = a[21];
    this.Ui = bc();
    this.jd = new Y(this);
    e = 0;
    for (g = a[2].length; e < g; e++)
      (k = a[2][e]),
        (b = this.Nd(k[0])),
        eb(k, b.prototype),
        (p = new b(this)),
        (p.pj = k[1]),
        (p.ge = k[2]),
        (p.$u = k[5]),
        (p.bo = k[9]),
        p.U && p.U(),
        this.plugins.push(p);
    this.Ui = bc();
    e = 0;
    for (g = a[3].length; e < g; e++) {
      k = a[3][e];
      l = this.Nd(k[1]);
      p = null;
      b = 0;
      for (c = this.plugins.length; b < c; b++)
        if (this.plugins[b] instanceof l) {
          p = this.plugins[b];
          break;
        }
      d = new p.ta(p);
      d.name = k[0];
      d.K = k[2];
      d.Ck = k[3].slice(0);
      d.Ot = k[3].length;
      d.oq = k[4];
      d.Pr = k[5];
      d.V = k[11];
      d.K
        ? ((d.Ze = []), (d.sd = this.Bg++), (d.za = null))
        : ((d.Ze = null), (d.sd = -1), (d.za = []));
      d.hi = null;
      d.Af = null;
      d.Im = null;
      d.Jb = !1;
      d.Qb = null;
      k[6]
        ? ((d.Jl = k[6][0]), (d.Kl = k[6][1]), (d.Ll = k[6][2]))
        : ((d.Jl = null), (d.Kl = 0), (d.Ll = 0));
      k[7] ? (d.Pb = k[7]) : (d.Pb = null);
      d.index = e;
      d.j = [];
      d.di = [];
      d.Gd = [new fb(d)];
      d.$c = 0;
      d.sc = null;
      d.Bq = 0;
      d.ag = !0;
      d.Bj = gb;
      d.Um = hb;
      d.$r = ib;
      d.ba = jb;
      d.vh = kb;
      d.aj = lb;
      d.df = mb;
      d.pi = nb;
      d.ok = ob;
      d.qk = pb;
      d.Zc = qb;
      d.rk = rb;
      d.Zh = new Wa(this.Ua, this.Ta);
      d.Ph = !0;
      d.Qh = !1;
      d.M = {};
      d.toString = sb;
      d.Da = [];
      b = 0;
      for (c = k[8].length; b < c; b++) {
        n = k[8][b];
        m = this.Nd(n[1]);
        u = null;
        l = 0;
        for (f = this.Da.length; l < f; l++)
          if (this.Da[l] instanceof m) {
            u = this.Da[l];
            break;
          }
        u ||
          ((u = new m(this)),
          (u.Ti = []),
          (u.Si = new ca()),
          u.U && u.U(),
          this.Da.push(u),
          cc && u instanceof cc && (this.Ro = u),
          dc && u instanceof dc && (this.tn = u));
        -1 === u.Ti.indexOf(d) && u.Ti.push(d);
        l = new u.ta(u, d);
        l.name = n[0];
        l.V = n[2];
        l.U();
        d.Da.push(l);
      }
      d.global = k[9];
      d.Jk = k[10];
      d.N = [];
      b = 0;
      for (c = k[12].length; b < c; b++)
        d.N.push({
          id: k[12][b][0],
          name: k[12][b][1],
          Na: -1,
          Pc: !1,
          mb: !0,
          index: b,
        });
      d.kv = k[13];
      (this.Xl && !d.K && !d.Jk && p.ge) || d.U();
      d.name && (this.types[d.name] = d);
      this.C.push(d);
      p.pj &&
        ((b = new p.fa(d)),
        (b.uid = this.nh++),
        (b.wo = this.eo++),
        (b.Ne = 0),
        (b.Ig = tb),
        (b.toString = ub),
        (b.G = k[14]),
        b.U(),
        d.j.push(b),
        (this.yd[b.uid.toString()] = b));
    }
    e = 0;
    for (g = a[4].length; e < g; e++)
      for (l = a[4][e], f = this.C[l[0]], b = 1, c = l.length; b < c; b++)
        (k = this.C[l[b]]), k.za.push(f), f.Ze.push(k);
    e = 0;
    for (g = a[28].length; e < g; e++) {
      l = a[28][e];
      f = [];
      b = 0;
      for (c = l.length; b < c; b++) f.push(this.C[l[b]]);
      b = 0;
      for (c = f.length; b < c; b++) (f[b].Jb = !0), (f[b].Qb = f);
    }
    if (0 < this.Bg)
      for (e = 0, g = this.C.length; e < g; e++)
        if (((k = this.C[e]), !k.K && k.za.length)) {
          k.hi = Array(this.Bg);
          k.Af = Array(this.Bg);
          k.Im = Array(this.Bg);
          d = [];
          b = u = m = n = 0;
          for (c = k.za.length; b < c; b++)
            for (
              p = k.za[b],
                k.hi[p.sd] = n,
                n += p.Ot,
                k.Af[p.sd] = m,
                m += p.oq,
                k.Im[p.sd] = u,
                u += p.Pr,
                l = 0,
                f = p.N.length;
              l < f;
              l++
            )
              d.push(va({}, p.N[l]));
          k.N = d.concat(k.N);
          b = 0;
          for (c = k.N.length; b < c; b++) k.N[b].index = b;
        }
    e = 0;
    for (g = a[5].length; e < g; e++)
      (k = a[5][e]),
        (b = new vb(this, k)),
        (this.Tk[b.name] = b),
        this.Mc.push(b);
    e = 0;
    for (g = a[6].length; e < g; e++)
      (k = a[6][e]),
        (b = new wb(this, k)),
        (this.gk[b.name] = b),
        this.rd.push(b);
    e = 0;
    for (g = this.rd.length; e < g; e++) this.rd[e].Ia();
    e = 0;
    for (g = this.rd.length; e < g; e++) this.rd[e].Ul();
    e = 0;
    for (g = this.Aj.length; e < g; e++) this.Aj[e].Ia();
    V(this.Aj);
    this.lq = a[7];
    this.Oc = a[9];
    this.og = 1;
    this.br = a[13];
    this.Aa = a[14];
    this.zm = a[15];
    this.Nt = a[17];
    this.hl = a[20];
    this.nq = 0 < this.hl;
    this.bt = a[22];
    this.jc = this.np = a[23];
    this.Uq = a[24];
    this.gt = a[25];
    this.ma = a[27] && !this.fe;
    this.qj = Date.now();
    V(this.Ui);
    this.Cs();
  };
  var e = !1,
    b = 0,
    p = [];
  d.prototype.lt = function (a, e) {
    function g() {
      b--;
      c.Yn();
    }
    var c = this;
    a.addEventListener("load", g);
    a.addEventListener("error", g);
    p.push([a, e]);
    this.Yn();
  };
  d.prototype.Yn = function () {
    for (var a; p.length && 100 > b; )
      b++, (a = p.shift()), this.Ho(a[0], a[1]);
  };
  d.prototype.mp = function (a, g) {
    a.cocoonLazyLoad = !0;
    a.onerror = function (g) {
      e = a.wm = !0;
      console &&
        console.error &&
        console.error("Error loading image '" + a.src + "': ", g);
    };
    this.Kc
      ? (a.src = g)
      : a.src ||
        ("undefined" !== typeof XAPKReader
          ? XAPKReader.get(
              g,
              function (e) {
                a.src = e;
              },
              function (b) {
                e = a.wm = !0;
                console &&
                  console.error &&
                  console.error(
                    "Error extracting image '" + g + "' from expansion file: ",
                    b
                  );
              }
            )
          : ((a.crossOrigin = "anonymous"), this.lt(a, g)));
    this.ig.push(a);
  };
  d.prototype.Or = function (a) {
    var e, g;
    e = 0;
    for (g = this.ig.length; e < g; e++)
      if (this.ig[e].xq === a) return this.ig[e];
    return null;
  };
  var l = 0,
    x = !1;
  d.prototype.ws = function () {
    this.pg && (l = this.pg.dv(this.lq));
  };
  d.prototype.rm = function () {
    var a = l,
      e = 0,
      g = 0,
      b = !0,
      c,
      f,
      g = 0;
    for (c = this.ig.length; g < c; g++) {
      f = this.ig[g];
      var k = f.Bm;
      if (!k || 0 >= k) k = 5e4;
      a += k;
      f.src && (f.complete || f.loaded) && !f.wm ? (e += k) : (b = !1);
    }
    b &&
      this.gt &&
      this.pg &&
      (x || (this.pg.gv(), (x = !0)),
      (g = this.pg.Yu()),
      (e += g),
      g < l && (b = !1));
    this.Qc = 0 == a ? 1 : e / a;
    return b;
  };
  var A = !1;
  d.prototype.go = function () {
    if (this.ya || this.A) {
      var a = this.ya || this.il;
      this.ib && this.so();
      var g = window.innerWidth,
        b = window.innerHeight;
      (this.Nf === g && this.Mf === b) || this.setSize(g, b);
      this.Qc = 0;
      this.Dn = -1;
      var c = this;
      if (this.rm() && (4 !== this.me || A)) this.xs();
      else {
        b = Date.now() - this.qj;
        if (a) {
          var l = this.width,
            k = this.height,
            g = this.devicePixelRatio;
          if (3 > this.me && (this.Jc || (500 <= b && this.Dn != this.Qc))) {
            a.clearRect(0, 0, l, k);
            var b = l / 2,
              k = k / 2,
              l = 0 === this.me && this.le.Oi.complete,
              d = 40 * g,
              p = 0,
              n = 80 * g,
              u;
            if (l) {
              var m = this.le.Oi,
                n = m.width * g;
              u = m.height * g;
              d = n / 2;
              p = u / 2;
              a.drawImage(m, O(b - d), O(k - p), n, u);
            }
            1 >= this.me
              ? ((b = O(b - d) + 0.5),
                (k = O(k + (p + (l ? 12 * g : 0))) + 0.5),
                (a.fillStyle = e ? "red" : "DodgerBlue"),
                a.fillRect(b, k, Math.floor(n * this.Qc), 6 * g),
                (a.strokeStyle = "black"),
                a.strokeRect(b, k, n, 6 * g),
                (a.strokeStyle = "white"),
                a.strokeRect(b - 1 * g, k - 1 * g, n + 2 * g, 8 * g))
              : 2 === this.me &&
                ((a.font = this.Kc ? "12pt ArialMT" : "12pt Arial"),
                (a.fillStyle = e ? "#f00" : "#999"),
                (a.jv = "middle"),
                (g = Math.round(100 * this.Qc) + "%"),
                (l = a.measureText ? a.measureText(g) : null),
                a.fillText(g, b - (l ? l.width : 0) / 2, k));
            this.Dn = this.Qc;
          } else if (4 === this.me) {
            this.Zq(a);
            f
              ? f(function () {
                  c.go();
                })
              : setTimeout(function () {
                  c.go();
                }, 16);
            return;
          }
        }
        setTimeout(
          function () {
            c.go();
          },
          this.Jc ? 10 : 100
        );
      }
    }
  };
  var z = -1,
    g = "undefined" === typeof cr_is_preview ? 200 : 0,
    k = !0,
    u = !1,
    P = 0,
    E = 0,
    T = "undefined" === typeof cr_is_preview ? 3e3 : 0,
    F = null,
    C = null,
    I = 0;
  d.prototype.Zq = function (a) {
    if (!A) {
      for (
        var b = Math.ceil(this.width),
          c = Math.ceil(this.height),
          l = this.le.Oi,
          f = this.le.et,
          d = this.le.Qt,
          p = 0;
        4 > p;
        ++p
      )
        if (!l[p].complete || !f[p].complete || !d[p].complete) return;
      0 === I && (z = Date.now());
      var p = Date.now(),
        n = !1,
        h = a,
        r,
        x;
      k || u
        ? (a.clearRect(0, 0, b, c),
          (F && F.width === b && F.height === c) ||
            ((F = document.createElement("canvas")),
            (F.width = b),
            (F.height = c),
            (C = F.getContext("2d"))),
          (h = C),
          (n = !0),
          k && 1 === I && (z = Date.now()))
        : (a.globalAlpha = 1);
      h.fillStyle = "#333333";
      h.fillRect(0, 0, b, c);
      256 < this.wg
        ? ((r = Ca(0.22 * c, 105, 0.6 * b)),
          (x = 0.25 * r),
          h.drawImage(m(f, r), 0.5 * b - r / 2, 0.2 * c - x / 2, r, x),
          (x = r = Math.min(0.395 * c, 0.95 * b)),
          h.drawImage(m(l, r), 0.5 * b - r / 2, 0.485 * c - x / 2, r, x),
          (r = Ca(0.22 * c, 105, 0.6 * b)),
          (x = 0.25 * r),
          h.drawImage(m(d, r), 0.5 * b - r / 2, 0.868 * c - x / 2, r, x),
          (h.fillStyle = "#3C3C3C"),
          (r = b),
          (x = Math.max(0.005 * c, 2)),
          h.fillRect(0, 0.8 * c - x / 2, r, x),
          (h.fillStyle = e ? "red" : "#E0FF65"),
          (r = b * this.Qc),
          h.fillRect(0.5 * b - r / 2, 0.8 * c - x / 2, r, x))
        : ((x = r = 0.55 * c),
          h.drawImage(m(l, r), 0.5 * b - r / 2, 0.45 * c - x / 2, r, x),
          (h.fillStyle = "#3C3C3C"),
          (r = b),
          (x = Math.max(0.005 * c, 2)),
          h.fillRect(0, 0.85 * c - x / 2, r, x),
          (h.fillStyle = e ? "red" : "#E0FF65"),
          (r = b * this.Qc),
          h.fillRect(0.5 * b - r / 2, 0.85 * c - x / 2, r, x));
      n &&
        (k
          ? (a.globalAlpha = 0 === I ? 0 : Math.min((p - z) / 300, 1))
          : u && (a.globalAlpha = Math.max(1 - (p - E) / 300, 0)),
        a.drawImage(F, 0, 0, b, c));
      k && 300 <= p - z && 2 <= I && ((k = !1), (P = p));
      !k && p - P >= T && !u && 1 <= this.Qc && ((u = !0), (E = p));
      if (
        (u && p - E >= 300 + g) ||
        ("undefined" !== typeof cr_is_preview &&
          1 <= this.Qc &&
          500 > Date.now() - z)
      )
        (A = !0), (u = k = !1), (this.le = C = F = null);
      ++I;
    }
  };
  d.prototype.xs = function () {
    this.ib &&
      (this.canvas.parentNode.removeChild(this.ib), (this.ib = this.il = null));
    this.qj = Date.now();
    this.Ve = Pa();
    var a, e, g;
    if (this.Xl)
      for (a = 0, e = this.C.length; a < e; a++)
        (g = this.C[a]), g.K || g.Jk || !g.Ba.ge || g.U();
    else this.Ug = !1;
    a = 0;
    for (e = this.Mc.length; a < e; a++) this.Mc[a].yq();
    2 <= this.Sb &&
      ((a = this.Ua / this.Ta),
      (e = this.width / this.height),
      (this.og =
        (2 !== this.Sb && e > a) || (2 === this.Sb && e < a)
          ? this.height / this.Ta
          : this.width / this.Ua));
    this.Nm ? this.Tk[this.Nm].Hl() : this.Mc[0].Hl();
    this.Xl ||
      ((this.Li = 1),
      this.trigger(Y.prototype.q.jm, null),
      window.C2_RegisterSW && window.C2_RegisterSW());
    navigator.splashscreen &&
      navigator.splashscreen.hide &&
      navigator.splashscreen.hide();
    a = 0;
    for (e = this.C.length; a < e; a++) (g = this.C[a]), g.Vs && g.Vs();
    document.hidden ||
    document.webkitHidden ||
    document.mozHidden ||
    document.msHidden
      ? window.cr_setSuspended(!0)
      : this.Ob(!1);
    this.Qe && AppMobi.webview.execute("onGameReady();");
  };
  d.prototype.Ob = function (a, e, g) {
    if (this.ja) {
      var b = Pa();
      if (g || !this.Rg || a) {
        a ||
          (f
            ? (this.rl = f(this.Xo))
            : (this.Ml = setTimeout(this.Xo, this.Re ? 1 : 16)));
        e = e || b;
        var c = this.Sb;
        ((g =
          (document.mozFullScreen ||
            document.webkitIsFullScreen ||
            document.fullScreen ||
            !!document.msFullscreenElement) &&
          !this.ee) ||
          this.xi) &&
          0 < this.Cf &&
          (c = this.Cf);
        if (0 < c) {
          var c = window.innerWidth,
            l = window.innerHeight;
          (this.Nf === c && this.Mf === l) || this.setSize(c, l);
        }
        this.Ha ||
          (g
            ? this.ki || (this.ki = !0)
            : this.ki
            ? ((this.ki = !1),
              0 === this.Sb &&
                this.setSize(
                  Math.round(this.ko / this.devicePixelRatio),
                  Math.round(this.jo / this.devicePixelRatio),
                  !0
                ))
            : ((this.ko = this.width), (this.jo = this.height)));
        this.Ug &&
          ((g = this.rm()),
          (this.Li = this.Qc),
          g &&
            ((this.Ug = !1),
            (this.Qc = 1),
            this.trigger(Y.prototype.q.jm, null),
            window.C2_RegisterSW && window.C2_RegisterSW()));
        this.Rs(e);
        (!this.ca && !this.Jc) ||
          this.Nk ||
          this.Fh ||
          a ||
          ((this.ca = !1),
          this.A ? this.Eb() : this.Hc(),
          this.Eh &&
            (this.canvas &&
              this.canvas.toDataURL &&
              ((this.Po = this.canvas.toDataURL(this.Eh[0], this.Eh[1])),
              window.cr_onSnapshot && window.cr_onSnapshot(this.Po),
              this.trigger(Y.prototype.q.Gp, null)),
            (this.Eh = null)));
        this.Zu || (this.kd++, this.Le++, this.oi++);
        this.kh += Pa() - b;
      }
    }
  };
  d.prototype.Rs = function (a) {
    var e, g, b, c, l, k, f, d;
    1e3 <= a - this.Ve &&
      ((this.Ve += 1e3),
      1e3 <= a - this.Ve && (this.Ve = a),
      (this.lk = this.oi),
      (this.oi = 0),
      (this.Zj = this.kh),
      (this.kh = 0));
    e = 0;
    0 !== this.Bi &&
      ((e = a - this.Bi),
      0 > e && (e = 0),
      (this.Ge = e /= 1e3),
      0.5 < this.Ge
        ? (this.Ge = 0)
        : this.Ge > 1 / this.ao && (this.Ge = 1 / this.ao));
    this.Bi = a;
    this.xf = this.Ge * this.eg;
    this.Vb.add(this.xf);
    this.Gj.add(e);
    a =
      (document.mozFullScreen ||
        document.webkitIsFullScreen ||
        document.fullScreen ||
        !!document.msFullscreenElement ||
        this.xi) &&
      !this.ee;
    2 <= this.Sb || (a && 0 < this.Cf)
      ? ((e = this.Ua / this.Ta),
        (g = this.width / this.height),
        (b = this.Sb),
        a && 0 < this.Cf && (b = this.Cf),
        (this.og =
          (2 !== b && g > e) || (2 === b && g < e)
            ? this.height / this.Ta
            : this.width / this.Ua),
        this.ja && (this.ja.Bl(this.ja.scrollX), this.ja.Cl(this.ja.scrollY)))
      : (this.og = this.Se ? this.devicePixelRatio : 1);
    this.lb();
    this.Lc++;
    this.jd.qt();
    this.Lc--;
    this.lb();
    this.Lc++;
    g = this.ho.Jd();
    a = 0;
    for (e = g.length; a < e; a++) g[a].av();
    a = 0;
    for (e = this.C.length; a < e; a++)
      if (((k = this.C[a]), !k.K && (k.Da.length || k.za.length)))
        for (g = 0, b = k.j.length; g < b; g++)
          for (f = k.j[g], c = 0, l = f.L.length; c < l; c++) f.L[c].Ob();
    a = 0;
    for (e = this.C.length; a < e; a++)
      if (((k = this.C[a]), !k.K && (k.Da.length || k.za.length)))
        for (g = 0, b = k.j.length; g < b; g++)
          for (f = k.j[g], c = 0, l = f.L.length; c < l; c++)
            (d = f.L[c]), d.to && d.to();
    g = this.cl.Jd();
    a = 0;
    for (e = g.length; a < e; a++) g[a].Ob();
    this.Lc--;
    this.ys();
    for (a = 0; this.rg && 10 > a++; ) this.Gm(this.rg);
    a = 0;
    for (e = this.rd.length; a < e; a++) this.rd[a].wk = !1;
    this.ja.Je && this.ja.Je.Ma();
    V(this.ef);
    this.Lc++;
    a = 0;
    for (e = this.C.length; a < e; a++)
      if (((k = this.C[a]), !k.K && (k.Da.length || k.za.length)))
        for (g = 0, b = k.j.length; g < b; g++)
          for (f = k.j[g], c = 0, l = f.L.length; c < l; c++)
            (d = f.L[c]), d.wj && d.wj();
    g = this.io.Jd();
    a = 0;
    for (e = g.length; a < e; a++) g[a].wj();
    this.Lc--;
  };
  d.prototype.cf = function () {
    var a, e, g, b, c, l, k, f, d;
    a = 0;
    for (e = this.C.length; a < e; a++)
      if (((k = this.C[a]), !k.K))
        for (g = 0, b = k.j.length; g < b; g++)
          if (((f = k.j[g]), f.cf && f.cf(), f.L))
            for (c = 0, l = f.L.length; c < l; c++)
              (d = f.L[c]), d.cf && d.cf();
  };
  d.prototype.Gm = function (a) {
    var e = this.ja;
    this.ja.Ft();
    var g, b, c;
    if (this.A)
      for (g = 0, b = this.C.length; g < b; g++)
        (c = this.C[g]),
          c.K ||
            !c.Tl ||
            (c.global && 0 !== c.j.length) ||
            -1 !== a.Ng.indexOf(c) ||
            c.Tl();
    e == a && V(this.jd.ec);
    V(this.ef);
    this.Ao(!0);
    a.Hl();
    this.Ao(!1);
    this.ca = !0;
    this.lb();
  };
  d.prototype.Ao = function (a) {
    var e, g, b, c, l, k, f, d, p;
    e = 0;
    for (g = this.Da.length; e < g; e++)
      (b = this.Da[e]), a ? b.oh && b.oh() : b.ph && b.ph();
    e = 0;
    for (g = this.C.length; e < g; e++)
      if (((b = this.C[e]), b.global || b.Ba.pj))
        for (c = 0, l = b.j.length; c < l; c++)
          if (((k = b.j[c]), a ? k.oh && k.oh() : k.ph && k.ph(), k.L))
            for (f = 0, d = k.L.length; f < d; f++)
              (p = k.L[f]), a ? p.oh && p.oh() : p.ph && p.ph();
  };
  d.prototype.xj = function (a) {
    this.cl.add(a);
  };
  d.prototype.Dg = function (a) {
    return a && -1 !== a.mh ? this.Ge * a.mh : this.xf;
  };
  d.prototype.Hc = function () {
    this.ja.Hc(this.ya);
    this.Qe && this.ya.present();
  };
  d.prototype.Eb = function () {
    this.ma && ((this.He = 1), this.ja.Fe(this.A));
    this.ja.Eb(this.A);
    this.A.it();
  };
  d.prototype.Kj = function (a) {
    a && this.fi.push(a);
  };
  d.prototype.zo = function (a) {
    Ba(this.fi, a);
  };
  d.prototype.Fg = function (a) {
    a = a.toString();
    return this.yd.hasOwnProperty(a) ? this.yd[a] : null;
  };
  var M = [];
  d.prototype.pf = function (a) {
    var e, g;
    e = a.type.name;
    var b = null;
    if (this.Ee.hasOwnProperty(e)) {
      if (((b = this.Ee[e]), b.contains(a))) return;
    } else (b = M.length ? M.pop() : new ca()), (this.Ee[e] = b);
    b.add(a);
    this.be = !0;
    if (a.Jb)
      for (e = 0, g = a.siblings.length; e < g; e++) this.pf(a.siblings[e]);
    this.Hk && b.gg.push(a);
    this.Gk ||
      (this.Lc++,
      this.trigger(Object.getPrototypeOf(a.type.Ba).q.Ip, a),
      this.Lc--);
  };
  d.prototype.lb = function () {
    if (this.be) {
      var a, e, g, b, c, l;
      this.Hk = !0;
      g = 0;
      for (c = this.Gc.length; g < c; ++g)
        for (
          a = this.Gc[g], e = a.type, e.j.push(a), b = 0, l = e.za.length;
          b < l;
          ++b
        )
          e.za[b].j.push(a), (e.za[b].ag = !0);
      V(this.Gc);
      this.Cp();
      Oa(this.Ee);
      this.be = this.Hk = !1;
    }
  };
  d.prototype.Cp = function () {
    for (var a in this.Ee) this.Ee.hasOwnProperty(a) && this.sp(this.Ee[a]);
  };
  d.prototype.sp = function (a) {
    var e = a.Jd(),
      g = e[0].type,
      b,
      c,
      l,
      k,
      f,
      d;
    Qa(g.j, a);
    g.ag = !0;
    0 === g.j.length && (g.Qh = !1);
    b = 0;
    for (c = g.za.length; b < c; ++b) (d = g.za[b]), Qa(d.j, a), (d.ag = !0);
    b = 0;
    for (c = this.jd.ec.length; b < c; ++b)
      if (
        ((f = this.jd.ec[b]),
        f.cc.hasOwnProperty(g.index) && Qa(f.cc[g.index].Pe, a),
        !g.K)
      )
        for (l = 0, k = g.za.length; l < k; ++l)
          (d = g.za[l]),
            f.cc.hasOwnProperty(d.index) && Qa(f.cc[d.index].Pe, a);
    if ((f = e[0].n)) {
      if (f.qc)
        for (l = f.j, b = 0, c = l.length; b < c; ++b)
          (k = l[b]),
            a.contains(k) &&
              (k.qa(), f.jb.update(k, k.Lb, null), k.Lb.set(0, 0, -1, -1));
      Qa(f.j, a);
      f.zh(0);
    }
    for (b = 0; b < e.length; ++b) this.rp(e[b], g);
    a.clear();
    M.push(a);
    this.ca = !0;
  };
  d.prototype.rp = function (a, e) {
    var g, b, c;
    g = 0;
    for (b = this.fi.length; g < b; ++g) this.fi[g](a);
    a.Wd && e.Zh.update(a, a.Wd, null);
    (g = a.n) && g.Wf(a, !0);
    if (a.L)
      for (g = 0, b = a.L.length; g < b; ++g)
        (c = a.L[g]), c.oe && c.oe(), c.behavior.Si.remove(a);
    this.ho.remove(a);
    this.cl.remove(a);
    this.io.remove(a);
    a.oe && a.oe();
    this.yd.hasOwnProperty(a.uid.toString()) &&
      delete this.yd[a.uid.toString()];
    this.Vi--;
    100 > e.di.length && e.di.push(a);
  };
  d.prototype.Cm = function (a, e, g, b) {
    if (a.K) {
      var c = O(Math.random() * a.Ze.length);
      return this.Cm(a.Ze[c], e, g, b);
    }
    return a.sc ? this.Xd(a.sc, e, !1, g, b, !1) : null;
  };
  var ba = [];
  d.prototype.Xd = function (a, e, g, b, c, l) {
    var k, f, d, p;
    if (!a) return null;
    var n = this.C[a[1]],
      u = n.Ba.ge;
    if ((this.Ug && u && !n.Jk) || (u && !this.A && 11 === a[0][11]))
      return null;
    var m = e;
    u || (e = null);
    var h;
    n.di.length
      ? ((h = n.di.pop()), (h.Zb = !0), n.Ba.fa.call(h, n))
      : ((h = new n.Ba.fa(n)), (h.Zb = !1));
    !g || l || this.yd.hasOwnProperty(a[2].toString())
      ? (h.uid = this.nh++)
      : (h.uid = a[2]);
    this.yd[h.uid.toString()] = h;
    h.wo = this.eo++;
    h.Ne = n.j.length;
    k = 0;
    for (f = this.Gc.length; k < f; ++k) this.Gc[k].type === n && h.Ne++;
    h.Ig = tb;
    h.toString = ub;
    d = a[3];
    if (h.Zb) Oa(h.M);
    else {
      h.M = {};
      if ("undefined" !== typeof cr_is_preview)
        for (h.hn = [], h.hn.length = d.length, k = 0, f = d.length; k < f; k++)
          h.hn[k] = d[k][1];
      h.Pa = [];
      h.Pa.length = d.length;
    }
    k = 0;
    for (f = d.length; k < f; k++) h.Pa[k] = d[k][0];
    if (u) {
      var r = a[0];
      h.x = fa(b) ? r[0] : b;
      h.y = fa(c) ? r[1] : c;
      h.z = r[2];
      h.width = r[3];
      h.height = r[4];
      h.depth = r[5];
      h.k = r[6];
      h.opacity = r[7];
      h.Hb = r[8];
      h.Ib = r[9];
      h.nb = r[10];
      k = r[11];
      !this.A && n.N.length && (h.nb = k);
      h.sg = Ya(h.nb);
      this.D && $a(h, h.nb, this.D);
      if (h.Zb) {
        k = 0;
        for (f = r[12].length; k < f; k++)
          for (d = 0, p = r[12][k].length; d < p; d++) h.Ea[k][d] = r[12][k][d];
        h.xa.set(0, 0, 0, 0);
        h.Wd.set(0, 0, -1, -1);
        h.Lb.set(0, 0, -1, -1);
        h.zb.Ah(h.xa);
        V(h.Pj);
      } else {
        h.Ea = r[12].slice(0);
        k = 0;
        for (f = h.Ea.length; k < f; k++) h.Ea[k] = r[12][k].slice(0);
        h.X = [];
        h.Od = [];
        h.Od.length = n.N.length;
        h.xa = new U(0, 0, 0, 0);
        h.Wd = new U(0, 0, -1, -1);
        h.Lb = new U(0, 0, -1, -1);
        h.zb = new sa();
        h.Pj = [];
        h.B = xb;
        h.Uu = yb;
        h.Rb = zb;
        h.qa = Ab;
        h.Mt = Bb;
        h.jp = Cb;
        h.bd = Db;
      }
      h.dg = !1;
      h.Jt = 0;
      h.It = 0;
      h.Ht = null;
      14 === r.length &&
        ((h.dg = !0), (h.Jt = r[13][0]), (h.It = r[13][1]), (h.Ht = r[13][2]));
      k = 0;
      for (f = n.N.length; k < f; k++) h.Od[k] = !0;
      h.Ed = !0;
      h.Vc = Eb;
      h.Vc();
      h.kp = !!h.X.length;
      h.Oj = !0;
      h.Sj = !0;
      n.Ph = !0;
      h.visible = !0;
      h.mh = -1;
      h.n = e;
      h.Wc = e.j.length;
      h.He = 0;
      "undefined" === typeof h.Z && (h.Z = null);
      this.ca = h.nd = !0;
    }
    var x;
    V(ba);
    k = 0;
    for (f = n.za.length; k < f; k++) ba.push.apply(ba, n.za[k].Da);
    ba.push.apply(ba, n.Da);
    if (h.Zb)
      for (k = 0, f = ba.length; k < f; k++) {
        var q = ba[k];
        x = h.L[k];
        x.Zb = !0;
        q.behavior.fa.call(x, q, h);
        r = a[4][k];
        d = 0;
        for (p = r.length; d < p; d++) x.G[d] = r[d];
        x.U();
        q.behavior.Si.add(h);
      }
    else
      for (h.L = [], k = 0, f = ba.length; k < f; k++)
        (q = ba[k]),
          (x = new q.behavior.fa(q, h)),
          (x.Zb = !1),
          (x.G = a[4][k].slice(0)),
          x.U(),
          h.L.push(x),
          q.behavior.Si.add(h);
    r = a[5];
    if (h.Zb) for (k = 0, f = r.length; k < f; k++) h.G[k] = r[k];
    else h.G = r.slice(0);
    this.Gc.push(h);
    this.be = !0;
    e && (e.ng(h, !0), 1 !== e.zd || 1 !== e.Ad) && (n.Qh = !0);
    this.Vi++;
    if (n.Jb) {
      if (((h.Jb = !0), h.Zb ? V(h.siblings) : (h.siblings = []), !g && !l)) {
        k = 0;
        for (f = n.Qb.length; k < f; k++)
          if (n.Qb[k] !== n) {
            if (!n.Qb[k].sc) return null;
            h.siblings.push(
              this.Xd(n.Qb[k].sc, m, !1, u ? h.x : b, u ? h.y : c, !0)
            );
          }
        k = 0;
        for (f = h.siblings.length; k < f; k++)
          for (h.siblings[k].siblings.push(h), d = 0; d < f; d++)
            k !== d && h.siblings[k].siblings.push(h.siblings[d]);
      }
    } else (h.Jb = !1), (h.siblings = null);
    h.U();
    k = 0;
    for (f = h.L.length; k < f; k++) h.L[k].dt && h.L[k].dt();
    return h;
  };
  d.prototype.Xr = function (a) {
    var e, g;
    e = 0;
    for (g = this.ja.O.length; e < g; e++) {
      var b = this.ja.O[e];
      if (bb(b.name, a)) return b;
    }
    return null;
  };
  d.prototype.Yr = function (a) {
    a = O(a);
    0 > a && (a = 0);
    a >= this.ja.O.length && (a = this.ja.O.length - 1);
    return this.ja.O[a];
  };
  d.prototype.Xj = function (a) {
    var e, g;
    e = 0;
    for (g = a.length; e < g; e++) a[e].ba().na = !0;
  };
  d.prototype.vh = function (a) {
    var e, g;
    e = 0;
    for (g = a.length; e < g; e++) a[e].vh();
  };
  d.prototype.aj = function (a) {
    var e, g;
    e = 0;
    for (g = a.length; e < g; e++) a[e].aj();
  };
  d.prototype.df = function (a) {
    var e, g;
    e = 0;
    for (g = a.length; e < g; e++) a[e].df();
  };
  d.prototype.fp = function (a) {
    if (a.Ph) {
      var e,
        g,
        b = a.j;
      e = 0;
      for (g = b.length; e < g; ++e) b[e].jp();
      b = this.Gc;
      e = 0;
      for (g = b.length; e < g; ++e) b[e].type === a && b[e].jp();
      a.Ph = !1;
    }
  };
  d.prototype.Qm = function (a, e, g, b) {
    var c,
      k,
      l = a ? 1 !== a.zd || 1 !== a.Ad : !1;
    if (e.K)
      for (a = 0, c = e.Ze.length; a < c; ++a)
        (k = e.Ze[a]), l || k.Qh ? za(b, k.j) : (this.fp(k), k.Zh.ej(g, b));
    else l || e.Qh ? za(b, e.j) : (this.fp(e), e.Zh.ej(g, b));
  };
  d.prototype.Xm = function (a, e, g, b) {
    var c, k;
    c = 0;
    for (k = e.length; c < k; ++c) this.Qm(a, e[c], g, b);
  };
  d.prototype.ds = function (a, e, g) {
    var b = this.Ro;
    b && this.Xm(a, b.Ti, e, g);
  };
  d.prototype.Wr = function (a, e, g) {
    var b = this.tn;
    b && this.Xm(a, b.Ti, e, g);
  };
  d.prototype.Nb = function (a, e) {
    if (!(a && e && a !== e && a.nd && e.nd)) return !1;
    a.qa();
    e.qa();
    var g = a.n,
      b = e.n,
      c,
      k,
      l,
      f,
      d,
      p,
      h,
      n;
    if (
      g === b ||
      (g.zd === b.zd &&
        b.Ad === b.Ad &&
        g.scale === b.scale &&
        g.k === b.k &&
        g.Mh === b.Mh)
    ) {
      if (!a.xa.Ds(e.xa) || !a.zb.jn(e.zb) || (a.dg && e.dg)) return !1;
      if (a.dg) return this.Uo(a, e);
      if (e.dg) return this.Uo(e, a);
      h = a.Z && !a.Z.Gf();
      c = e.Z && !e.Z.Gf();
      if (!h && !c) return !0;
      h
        ? (a.Z.uf(a.width, a.height, a.k), (h = a.Z))
        : (this.Tc.Zf(a.zb, a.x, a.y, a.width, a.height), (h = this.Tc));
      c
        ? (e.Z.uf(e.width, e.height, e.k), (n = e.Z))
        : (this.Tc.Zf(e.zb, e.x, e.y, e.width, e.height), (n = this.Tc));
      return h.Og(n, e.x - a.x, e.y - a.y);
    }
    h = a.Z && !a.Z.Gf();
    c = e.Z && !e.Z.Gf();
    h
      ? (a.Z.uf(a.width, a.height, a.k), this.Tc.Jo(a.Z))
      : this.Tc.Zf(a.zb, a.x, a.y, a.width, a.height);
    h = this.Tc;
    c
      ? (e.Z.uf(e.width, e.height, e.k), this.Il.Jo(e.Z))
      : this.Il.Zf(e.zb, e.x, e.y, e.width, e.height);
    n = this.Il;
    c = 0;
    for (k = h.Rc; c < k; c++)
      (l = 2 * c),
        (f = l + 1),
        (d = h.Wa[l]),
        (p = h.Wa[f]),
        (h.Wa[l] = g.ia(d + a.x, p + a.y, !0)),
        (h.Wa[f] = g.ia(d + a.x, p + a.y, !1));
    h.qa();
    c = 0;
    for (k = n.Rc; c < k; c++)
      (l = 2 * c),
        (f = l + 1),
        (d = n.Wa[l]),
        (p = n.Wa[f]),
        (n.Wa[l] = b.ia(d + e.x, p + e.y, !0)),
        (n.Wa[f] = b.ia(d + e.x, p + e.y, !1));
    n.qa();
    return h.Og(n, 0, 0);
  };
  var R = new sa();
  new U(0, 0, 0, 0);
  var K = [];
  d.prototype.Uo = function (a, e) {
    var g,
      b,
      c,
      k,
      l = e.xa,
      f = a.x,
      d = a.y;
    a.Wu(l, K);
    var p = e.Z && !e.Z.Gf();
    g = 0;
    for (b = K.length; g < b; ++g)
      if (
        ((c = K[g]),
        (k = c.bv),
        l.Es(k, f, d) && (R.Ah(k), R.offset(f, d), R.jn(e.zb)))
      )
        if (p)
          if ((e.Z.uf(e.width, e.height, e.k), c.ll)) {
            if (c.ll.Og(e.Z, e.x - (f + k.left), e.y - (d + k.top)))
              return V(K), !0;
          } else {
            if (
              (this.Tc.Zf(R, 0, 0, k.right - k.left, k.bottom - k.top),
              this.Tc.Og(e.Z, e.x, e.y))
            )
              return V(K), !0;
          }
        else if (c.ll) {
          if (
            (this.Tc.Zf(e.zb, 0, 0, e.width, e.height),
            c.ll.Og(this.Tc, -(f + k.left), -(d + k.top)))
          )
            return V(K), !0;
        } else return V(K), !0;
    V(K);
    return !1;
  };
  d.prototype.Ql = function (a, e) {
    if (!e) return !1;
    var g, b, c, k, l;
    g = 0;
    for (b = a.Da.length; g < b; g++)
      if (a.Da[g].behavior instanceof e) return !0;
    if (!a.K)
      for (g = 0, b = a.za.length; g < b; g++)
        for (l = a.za[g], c = 0, k = l.Da.length; c < k; c++)
          if (l.Da[c].behavior instanceof e) return !0;
    return !1;
  };
  d.prototype.Rl = function (a) {
    return this.Ql(a, Ub.ru);
  };
  d.prototype.Sl = function (a) {
    return this.Ql(a, Ub.su);
  };
  var G = [];
  d.prototype.cb = function (a) {
    var e, g, b;
    a.qa();
    this.ds(a.n, a.xa, G);
    e = 0;
    for (g = G.length; e < g; ++e)
      if (((b = G[e]), b.M.solidEnabled && this.Nb(a, b))) return V(G), b;
    V(G);
    return null;
  };
  var N = [];
  d.prototype.we = function (a, e) {
    var g = null;
    e && ((g = N), V(g));
    a.qa();
    this.Wr(a.n, a.xa, G);
    var b, c, k;
    b = 0;
    for (c = G.length; b < c; ++b)
      if (((k = G[b]), k.M.jumpthruEnabled && this.Nb(a, k)))
        if (e) g.push(k);
        else return V(G), k;
    V(G);
    return g;
  };
  d.prototype.vc = function (a, e, g, b, c, k) {
    b = b || 50;
    var l = a.x,
      f = a.y,
      d,
      p = null,
      h = null;
    for (d = 0; d < b; d++)
      if (
        ((a.x = l + e * d),
        (a.y = f + g * d),
        a.B(),
        !this.Nb(a, p) &&
          ((p = this.cb(a)) && (h = p),
          !p &&
            (c &&
              (k ? (p = this.Nb(a, k) ? k : null) : (p = this.we(a)),
              p && (h = p)),
            !p)))
      )
        return h && this.cj(a, e, g, h), !0;
    a.x = l;
    a.y = f;
    a.B();
    return !1;
  };
  d.prototype.pl = function (a, e, g, b) {
    b = b || 50;
    var c = a.x,
      k = a.y,
      l = null,
      f = null,
      d,
      p,
      h;
    for (d = 0; d < b; ++d)
      for (p = 0; 2 > p; ++p)
        if (
          ((h = 2 * p - 1),
          (a.x = c + e * d * h),
          (a.y = k + g * d * h),
          a.B(),
          !this.Nb(a, l))
        )
          if ((l = this.cb(a))) f = l;
          else return f && this.cj(a, e * h, g * h, f), !0;
    a.x = c;
    a.y = k;
    a.B();
    return !1;
  };
  d.prototype.cj = function (a, e, g, b) {
    var c = 2,
      k,
      l = !1;
    k = !1;
    for (var f = a.x, d = a.y; 16 >= c; )
      (k = 1 / c),
        (c *= 2),
        (a.x += e * k * (l ? 1 : -1)),
        (a.y += g * k * (l ? 1 : -1)),
        a.B(),
        this.Nb(a, b) ? (k = l = !0) : ((k = l = !1), (f = a.x), (d = a.y));
    k && ((a.x = f), (a.y = d), a.B());
  };
  d.prototype.xo = function (a, e) {
    var g = fa(e) ? 100 : e,
      b = 0,
      c = a.x,
      k = a.y,
      l = 0,
      f = 0,
      d = 0,
      p = this.cb(a);
    if (!p) return !0;
    for (; b <= g; ) {
      switch (l) {
        case 0:
          f = 0;
          d = -1;
          b++;
          break;
        case 1:
          f = 1;
          d = -1;
          break;
        case 2:
          f = 1;
          d = 0;
          break;
        case 3:
          d = f = 1;
          break;
        case 4:
          f = 0;
          d = 1;
          break;
        case 5:
          f = -1;
          d = 1;
          break;
        case 6:
          f = -1;
          d = 0;
          break;
        case 7:
          d = f = -1;
      }
      l = (l + 1) % 8;
      a.x = O(c + f * b);
      a.y = O(k + d * b);
      a.B();
      if (!this.Nb(a, p) && ((p = this.cb(a)), !p)) return !0;
    }
    a.x = c;
    a.y = k;
    a.B();
    return !1;
  };
  d.prototype.Dd = function (a, e) {
    a.nd && e.nd && this.ef.push([a, e]);
  };
  d.prototype.iq = function (a, e, g) {
    var b, c, k;
    b = 0;
    for (c = this.ef.length; b < c; ++b) {
      k = this.ef[b];
      if (k[0] === a) k = k[1];
      else if (k[1] === a) k = k[0];
      else continue;
      if (e.K) {
        if (-1 === e.Ze.indexOf(e)) continue;
      } else if (k.type !== e) continue;
      -1 === g.indexOf(k) && g.push(k);
    }
  };
  d.prototype.uq = function (a, e) {
    var g, b, c;
    g = 0;
    for (b = this.ef.length; g < b; g++)
      if (
        ((c = this.ef[g]),
        (c[0] === a && c[1] === e) || (c[0] === e && c[1] === a))
      )
        return !0;
    return !1;
  };
  d.prototype.sq = function (a, e, g) {
    var b = a.x,
      c = a.y,
      k = la(10, La(e, g, b, c)),
      l = Ha(e, g, b, c),
      f = this.cb(a);
    if (!f) return X(l + pa);
    var d = f,
      p,
      h,
      n,
      u,
      m = W(5);
    for (p = 1; 36 > p; p++)
      if (
        ((h = l - p * m),
        (a.x = e + Math.cos(h) * k),
        (a.y = g + Math.sin(h) * k),
        a.B(),
        !this.Nb(a, d) && ((d = this.cb(a)), !d))
      ) {
        n = h;
        break;
      }
    36 === p && (n = X(l + pa));
    d = f;
    for (p = 1; 36 > p; p++)
      if (
        ((h = l + p * m),
        (a.x = e + Math.cos(h) * k),
        (a.y = g + Math.sin(h) * k),
        a.B(),
        !this.Nb(a, d) && ((d = this.cb(a)), !d))
      ) {
        u = h;
        break;
      }
    36 === p && (u = X(l + pa));
    a.x = b;
    a.y = c;
    a.B();
    if (u === n) return u;
    a = Ia(u, n) / 2;
    a = Ka(u, n) ? X(n + a + pa) : X(u + a);
    n = Math.cos(l);
    l = Math.sin(l);
    u = Math.cos(a);
    a = Math.sin(a);
    e = n * u + l * a;
    return Ha(0, 0, n - 2 * e * u, l - 2 * e * a);
  };
  var na = -1;
  d.prototype.trigger = function (a, e, g) {
    if (!this.ja) return !1;
    var b = this.ja.Je;
    if (!b) return !1;
    var c = !1,
      k,
      l,
      f;
    na++;
    var d = b.ck;
    l = 0;
    for (f = d.length; l < f; ++l) (k = this.Zo(a, e, d[l], g)), (c = c || k);
    k = this.Zo(a, e, b, g);
    na--;
    return c || k;
  };
  d.prototype.Zo = function (a, e, g, b) {
    var c = !1,
      k,
      l,
      f,
      d;
    if (e)
      for (
        f = this.Ol(a, e, e.type.name, g, b),
          c = c || f,
          d = e.type.za,
          k = 0,
          l = d.length;
        k < l;
        ++k
      )
        (f = this.Ol(a, e, d[k].name, g, b)), (c = c || f);
    else (f = this.Ol(a, e, "system", g, b)), (c = c || f);
    return c;
  };
  d.prototype.Ol = function (a, e, g, b, c) {
    var k,
      l = !1,
      f = !1,
      f = "undefined" !== typeof c,
      d = (f ? b.Km : b.$o)[g];
    if (!d) return l;
    var p = null;
    b = 0;
    for (k = d.length; b < k; ++b)
      if (d[b].method == a) {
        p = d[b].Ag;
        break;
      }
    if (!p) return l;
    var h;
    f ? (h = p[c]) : (h = p);
    if (!h) return null;
    b = 0;
    for (k = h.length; b < k; b++)
      (a = h[b][0]), (c = h[b][1]), (f = this.Kr(e, g, a, c)), (l = l || f);
    return l;
  };
  d.prototype.Kr = function (a, e, g, b) {
    var c,
      k,
      l = !1;
    this.Pl++;
    var f = this.tb().ic;
    f && this.vh(f.ve);
    var d = 1 < this.Pl;
    this.vh(g.ve);
    d && this.kt();
    var p = this.bj(g);
    p.ic = g;
    a &&
      ((c = this.types[e].ba()),
      (c.na = !1),
      V(c.j),
      (c.j[0] = a),
      this.types[e].Zc());
    a = !0;
    if (g.parent) {
      e = p.To;
      for (c = g.parent; c; ) e.push(c), (c = c.parent);
      e.reverse();
      c = 0;
      for (k = e.length; c < k; c++)
        if (!e[c].ut()) {
          a = !1;
          break;
        }
    }
    a && (this.Le++, g.Nc ? g.tt(b) : g.Ma(), (l = l || p.Ue));
    this.Zi();
    d && this.ct();
    this.df(g.ve);
    f && this.df(f.ve);
    this.be && 0 === this.Lc && 0 === na && !this.Kk && this.lb();
    this.Pl--;
    return l;
  };
  d.prototype.Rm = function () {
    var a = this.tb();
    return a.ic.$a[a.Za];
  };
  d.prototype.Sr = function () {
    var a = this.tb();
    return a.ic.rc[a.fc];
  };
  d.prototype.kt = function () {
    this.Ni++;
    this.Ni >= this.al.length && this.al.push([]);
  };
  d.prototype.ct = function () {
    this.Ni--;
  };
  d.prototype.Sm = function () {
    return this.al[this.Ni];
  };
  d.prototype.bj = function (a) {
    this.gi++;
    this.gi >= this.fk.length && this.fk.push(new Fb());
    var e = this.tb();
    e.reset(a);
    return e;
  };
  d.prototype.Zi = function () {
    this.gi--;
  };
  d.prototype.tb = function () {
    return this.fk[this.gi];
  };
  d.prototype.Tm = function (a, e) {
    for (var g, b, c, k, l, f; e; ) {
      g = 0;
      for (b = e.yc.length; g < b; g++)
        if (((f = e.yc[g]), f instanceof Gb && bb(a, f.name))) return f;
      e = e.parent;
    }
    g = 0;
    for (b = this.rd.length; g < b; g++)
      for (l = this.rd[g], c = 0, k = l.$d.length; c < k; c++)
        if (((f = l.$d[c]), f instanceof Gb && bb(a, f.name))) return f;
    return null;
  };
  d.prototype.Vm = function (a) {
    var e, g;
    e = 0;
    for (g = this.Mc.length; e < g; e++)
      if (this.Mc[e].V === a) return this.Mc[e];
    return null;
  };
  d.prototype.ri = function (a) {
    var e, g;
    e = 0;
    for (g = this.C.length; e < g; e++) if (this.C[e].V === a) return this.C[e];
    return null;
  };
  d.prototype.Tr = function (a) {
    var e, g;
    e = 0;
    for (g = this.tf.length; e < g; e++)
      if (this.tf[e].V === a) return this.tf[e];
    return null;
  };
  d.prototype.Cq = function (a, e) {
    this.Eh = [a, e];
    this.ca = !0;
  };
  d.prototype.ys = function () {
    var a = this,
      e = this.Al,
      g = this.vd,
      b = this.Ii,
      k = !1;
    this.Oo && ((k = !0), (e = "__c2_continuouspreview"), (this.Oo = !1));
    if (e.length) {
      this.lb();
      g = this.zt();
      if (q() && !this.Jc)
        n(
          e,
          g,
          function () {
            w("Saved state to IndexedDB storage (" + g.length + " bytes)");
            a.vd = g;
            a.trigger(Y.prototype.q.Jj, null);
            g = a.vd = "";
            k && c();
          },
          function (b) {
            try {
              localStorage.setItem("__c2save_" + e, g),
                w("Saved state to WebStorage (" + g.length + " bytes)"),
                (a.vd = g),
                a.trigger(Y.prototype.q.Jj, null),
                (g = a.vd = ""),
                k && c();
            } catch (l) {
              w("Failed to save game state: " + b + "; " + l),
                a.trigger(Y.prototype.q.km, null);
            }
          }
        );
      else
        try {
          localStorage.setItem("__c2save_" + e, g),
            w("Saved state to WebStorage (" + g.length + " bytes)"),
            (a.vd = g),
            this.trigger(Y.prototype.q.Jj, null),
            (g = a.vd = ""),
            k && c();
        } catch (l) {
          w("Error saving to WebStorage: " + l),
            a.trigger(Y.prototype.q.km, null);
        }
      this.Ii = this.Al = "";
      this.Sa = null;
    }
    if (b.length) {
      if (q() && !this.Jc)
        t(
          b,
          function (e) {
            e
              ? ((a.Sa = e),
                w(
                  "Loaded state from IndexedDB storage (" +
                    a.Sa.length +
                    " bytes)"
                ))
              : ((a.Sa = localStorage.getItem("__c2save_" + b) || ""),
                w("Loaded state from WebStorage (" + a.Sa.length + " bytes)"));
            a.Fh = !1;
            a.Sa || ((a.Sa = null), a.trigger(Y.prototype.q.Oh, null));
          },
          function () {
            a.Sa = localStorage.getItem("__c2save_" + b) || "";
            w("Loaded state from WebStorage (" + a.Sa.length + " bytes)");
            a.Fh = !1;
            a.Sa || ((a.Sa = null), a.trigger(Y.prototype.q.Oh, null));
          }
        );
      else {
        try {
          (this.Sa = localStorage.getItem("__c2save_" + b) || ""),
            w("Loaded state from WebStorage (" + this.Sa.length + " bytes)");
        } catch (f) {
          this.Sa = null;
        }
        this.Fh = !1;
        a.Sa || ((a.Sa = null), a.trigger(Y.prototype.q.Oh, null));
      }
      this.Al = this.Ii = "";
    }
    null !== this.Sa &&
      (this.lb(),
      this.Qs(this.Sa)
        ? ((this.vd = this.Sa),
          this.trigger(Y.prototype.q.Op, null),
          (this.vd = ""))
        : a.trigger(Y.prototype.q.Oh, null),
      (this.Sa = null));
  };
  d.prototype.zt = function () {
    var e,
      g,
      b,
      c,
      k,
      l,
      f,
      d = {
        c2save: !0,
        version: 1,
        rt: {
          time: this.Vb.wa,
          walltime: this.Gj.wa,
          timescale: this.eg,
          tickcount: this.kd,
          execcount: this.Le,
          next_uid: this.nh,
          running_layout: this.ja.V,
          start_time_offset: Date.now() - this.qj,
        },
        types: {},
        layouts: {},
        events: { groups: {}, cnds: {}, acts: {}, vars: {} },
      };
    e = 0;
    for (g = this.C.length; e < g; e++)
      if (((k = this.C[e]), !k.K && !this.Rl(k))) {
        l = { instances: [] };
        Na(k.M) && (l.ex = a(k.M));
        b = 0;
        for (c = k.j.length; b < c; b++) l.instances.push(this.zl(k.j[b]));
        d.types[k.V.toString()] = l;
      }
    e = 0;
    for (g = this.Mc.length; e < g; e++)
      (b = this.Mc[e]), (d.layouts[b.V.toString()] = b.kb());
    c = d.events.groups;
    e = 0;
    for (g = this.tf.length; e < g; e++)
      (b = this.tf[e]), (c[b.V.toString()] = this.si[b.Kg].Jg);
    g = d.events.cnds;
    for (f in this.Vd)
      this.Vd.hasOwnProperty(f) &&
        ((e = this.Vd[f]), Na(e.M) && (g[f] = { ex: a(e.M) }));
    g = d.events.acts;
    for (f in this.Pd)
      this.Pd.hasOwnProperty(f) &&
        ((e = this.Pd[f]), Na(e.M) && (g[f] = { ex: a(e.M) }));
    g = d.events.vars;
    for (f in this.hg)
      this.hg.hasOwnProperty(f) &&
        ((e = this.hg[f]), e.yi || (e.parent && !e.Tg) || (g[f] = e.data));
    d.system = this.jd.kb();
    return JSON.stringify(d);
  };
  d.prototype.yo = function () {
    var a, e, g, b, c, k;
    this.yd = {};
    a = 0;
    for (e = this.C.length; a < e; a++)
      if (((g = this.C[a]), !g.K))
        for (b = 0, c = g.j.length; b < c; b++)
          (k = g.j[b]), (this.yd[k.uid.toString()] = k);
  };
  d.prototype.Qs = function (a) {
    var e;
    try {
      e = JSON.parse(a);
    } catch (g) {
      return !1;
    }
    if (!e.c2save || 1 < e.version) return !1;
    this.Qg = !0;
    a = e.rt;
    this.Vb.reset();
    this.Vb.wa = a.time;
    this.Gj.reset();
    this.Gj.wa = a.walltime || 0;
    this.eg = a.timescale;
    this.kd = a.tickcount;
    this.Le = a.execcount;
    this.qj = Date.now() - a.start_time_offset;
    var b = a.running_layout;
    if (b !== this.ja.V)
      if ((b = this.Vm(b))) this.Gm(b);
      else return;
    var c, k, l, f, d, p, h;
    p = e.types;
    for (k in p)
      if (
        p.hasOwnProperty(k) &&
        (f = this.ri(parseInt(k, 10))) &&
        !f.K &&
        !this.Rl(f)
      ) {
        p[k].ex ? (f.M = p[k].ex) : Oa(f.M);
        d = f.j;
        l = p[k].instances;
        b = 0;
        for (c = ma(d.length, l.length); b < c; b++) this.Ji(d[b], l[b]);
        b = l.length;
        for (c = d.length; b < c; b++) this.pf(d[b]);
        b = d.length;
        for (c = l.length; b < c; b++) {
          d = null;
          if (f.Ba.ge && ((d = this.ja.qi(l[b].w.l)), !d)) continue;
          d = this.Xd(f.sc, d, !1, 0, 0, !0);
          this.Ji(d, l[b]);
        }
        f.ag = !0;
      }
    this.lb();
    this.yo();
    c = e.layouts;
    for (k in c)
      c.hasOwnProperty(k) && (b = this.Vm(parseInt(k, 10))) && b.Ab(c[k]);
    c = e.events.groups;
    for (k in c)
      c.hasOwnProperty(k) &&
        (b = this.Tr(parseInt(k, 10))) &&
        this.si[b.Kg] &&
        this.si[b.Kg].At(c[k]);
    b = e.events.cnds;
    for (k in this.Vd)
      this.Vd.hasOwnProperty(k) &&
        (b.hasOwnProperty(k) ? (this.Vd[k].M = b[k].ex) : (this.Vd[k].M = {}));
    b = e.events.acts;
    for (k in this.Pd)
      this.Pd.hasOwnProperty(k) &&
        (b.hasOwnProperty(k) ? (this.Pd[k].M = b[k].ex) : (this.Pd[k].M = {}));
    b = e.events.vars;
    for (k in b)
      b.hasOwnProperty(k) &&
        this.hg.hasOwnProperty(k) &&
        (this.hg[k].data = b[k]);
    this.nh = a.next_uid;
    this.Qg = !1;
    b = 0;
    for (c = this.ji.length; b < c; ++b)
      (d = this.ji[b]), this.trigger(Object.getPrototypeOf(d.type.Ba).q.Nh, d);
    V(this.ji);
    this.jd.Ab(e.system);
    b = 0;
    for (c = this.C.length; b < c; b++)
      if (((f = this.C[b]), !f.K && !this.Rl(f)))
        for (e = 0, k = f.j.length; e < k; e++) {
          d = f.j[e];
          if (f.Jb)
            for (p = d.Ig(), V(d.siblings), a = 0, l = f.Qb.length; a < l; a++)
              (h = f.Qb[a]), f !== h && d.siblings.push(h.j[p]);
          d.Xc && d.Xc();
          if (d.L)
            for (a = 0, l = d.L.length; a < l; a++)
              (p = d.L[a]), p.Xc && p.Xc();
        }
    return (this.ca = !0);
  };
  d.prototype.zl = function (e, g) {
    var b, c, k, f, l;
    f = e.type;
    k = f.Ba;
    var d = {};
    g ? (d.c2 = !0) : (d.uid = e.uid);
    Na(e.M) && (d.ex = a(e.M));
    if (e.Pa && e.Pa.length)
      for (d.ivs = {}, b = 0, c = e.Pa.length; b < c; b++)
        d.ivs[e.type.Ck[b].toString()] = e.Pa[b];
    if (k.ge) {
      k = { x: e.x, y: e.y, w: e.width, h: e.height, l: e.n.V, zi: e.bd() };
      0 !== e.k && (k.a = e.k);
      1 !== e.opacity && (k.o = e.opacity);
      0.5 !== e.Hb && (k.hX = e.Hb);
      0.5 !== e.Ib && (k.hY = e.Ib);
      0 !== e.nb && (k.bm = e.nb);
      e.visible || (k.v = e.visible);
      e.nd || (k.ce = e.nd);
      -1 !== e.mh && (k.mts = e.mh);
      if (f.N.length)
        for (k.fx = [], b = 0, c = f.N.length; b < c; b++)
          (l = f.N[b]),
            k.fx.push({
              name: l.name,
              active: e.Od[l.index],
              params: e.Ea[l.index],
            });
      d.w = k;
    }
    if (e.L && e.L.length)
      for (d.behs = {}, b = 0, c = e.L.length; b < c; b++)
        (f = e.L[b]), f.kb && (d.behs[f.type.V.toString()] = f.kb());
    e.kb && (d.data = e.kb());
    return d;
  };
  d.prototype.Vr = function (a, e) {
    var g, b;
    g = 0;
    for (b = a.Ck.length; g < b; g++) if (a.Ck[g] === e) return g;
    return -1;
  };
  d.prototype.Rr = function (a, e) {
    var g, b;
    g = 0;
    for (b = a.L.length; g < b; g++) if (a.L[g].type.V === e) return g;
    return -1;
  };
  d.prototype.Ji = function (a, e, g) {
    var b, c, k, f, l;
    l = a.type;
    var d = l.Ba;
    if (g) {
      if (!e.c2) return;
    } else a.uid = e.uid;
    e.ex ? (a.M = e.ex) : Oa(a.M);
    if ((c = e.ivs))
      for (b in c)
        c.hasOwnProperty(b) &&
          ((k = this.Vr(l, parseInt(b, 10))),
          0 > k ||
            k >= a.Pa.length ||
            ((f = c[b]), null === f && (f = NaN), (a.Pa[k] = f)));
    if (d.ge) {
      k = e.w;
      a.n.V !== k.l &&
        ((c = a.n),
        (a.n = this.ja.qi(k.l)),
        a.n
          ? (c.Wf(a, !0), a.n.ng(a, !0), a.B(), a.n.zh(0))
          : ((a.n = c), g || this.pf(a)));
      a.x = k.x;
      a.y = k.y;
      a.width = k.w;
      a.height = k.h;
      a.Wc = k.zi;
      a.k = k.hasOwnProperty("a") ? k.a : 0;
      a.opacity = k.hasOwnProperty("o") ? k.o : 1;
      a.Hb = k.hasOwnProperty("hX") ? k.hX : 0.5;
      a.Ib = k.hasOwnProperty("hY") ? k.hY : 0.5;
      a.visible = k.hasOwnProperty("v") ? k.v : !0;
      a.nd = k.hasOwnProperty("ce") ? k.ce : !0;
      a.mh = k.hasOwnProperty("mts") ? k.mts : -1;
      a.nb = k.hasOwnProperty("bm") ? k.bm : 0;
      a.sg = Ya(a.nb);
      this.D && $a(a, a.nb, this.D);
      a.B();
      if (k.hasOwnProperty("fx"))
        for (g = 0, c = k.fx.length; g < c; g++)
          (f = l.qk(k.fx[g].name)),
            0 > f || ((a.Od[f] = k.fx[g].active), (a.Ea[f] = k.fx[g].params));
      a.Vc();
    }
    if ((l = e.behs))
      for (b in l)
        l.hasOwnProperty(b) &&
          ((g = this.Rr(a, parseInt(b, 10))), 0 > g || a.L[g].Ab(l[b]));
    e.data && a.Ab(e.data);
  };
  d.prototype.Lm = function (a, e, g) {
    window.resolveLocalFileSystemURL(
      cordova.file.applicationDirectory + "www/" + a,
      function (a) {
        a.file(e, g);
      },
      g
    );
  };
  d.prototype.Mr = function (a, e) {
    this.Lm(
      "data.js",
      function (g) {
        var b = new FileReader();
        b.onload = function (e) {
          a(e.target.result);
        };
        b.onerror = e;
        b.readAsText(g);
      },
      e
    );
  };
  var ha = [],
    J = 0;
  d.prototype.bl = function () {
    if (ha.length && !(8 <= J)) {
      J++;
      var a = ha.shift();
      this.Gq(a.filename, a.Gt, a.er);
    }
  };
  d.prototype.Lr = function (a, e, g) {
    var b = this;
    ha.push({
      filename: a,
      Gt: function (a) {
        J--;
        b.bl();
        e(a);
      },
      er: function (a) {
        J--;
        b.bl();
        g(a);
      },
    });
    this.bl();
  };
  d.prototype.Gq = function (a, e, g) {
    this.Lm(
      a,
      function (a) {
        var g = new FileReader();
        g.onload = function (a) {
          e(a.target.result);
        };
        g.readAsArrayBuffer(a);
      },
      g
    );
  };
  d.prototype.Nr = function (a, e, g) {
    var b = "",
      k = a.toLowerCase(),
      c = k.substr(k.length - 4),
      k = k.substr(k.length - 5);
    ".mp4" === c
      ? (b = "video/mp4")
      : ".webm" === k
      ? (b = "video/webm")
      : ".m4a" === c
      ? (b = "audio/mp4")
      : ".mp3" === c && (b = "audio/mpeg");
    this.Lr(
      a,
      function (a) {
        a = URL.createObjectURL(new Blob([a], { type: b }));
        e(a);
      },
      g
    );
  };
  d.prototype.Fs = function (a) {
    return (
      /^(?:[a-z]+:)?\/\//.test(a) ||
      "data:" === a.substr(0, 5) ||
      "blob:" === a.substr(0, 5)
    );
  };
  d.prototype.Ho = function (a, e) {
    this.nn && !this.Fs(e)
      ? this.Nr(
          e,
          function (e) {
            a.src = e;
          },
          function (a) {
            alert("Failed to load image: " + a);
          }
        )
      : (a.src = e);
  };
  d.prototype.jj = function (a, e) {
    "undefined" !== typeof a.imageSmoothingEnabled
      ? (a.imageSmoothingEnabled = e)
      : ((a.webkitImageSmoothingEnabled = e),
        (a.mozImageSmoothingEnabled = e),
        (a.msImageSmoothingEnabled = e));
  };
  Hb = function (a) {
    return new d(document.getElementById(a));
  };
  Ib = function (a, e) {
    return new d({ dc: !0, width: a, height: e });
  };
  window.cr_createRuntime = Hb;
  window.cr_createDCRuntime = Ib;
  window.createCocoonJSRuntime = function () {
    window.c2cocoonjs = !0;
    var a =
      document.createElement("screencanvas") ||
      document.createElement("canvas");
    a.Nj = !0;
    document.body.appendChild(a);
    a = new d(a);
    window.c2runtime = a;
    window.addEventListener("orientationchange", function () {
      window.c2runtime.setSize(window.innerWidth, window.innerHeight);
    });
    window.c2runtime.setSize(window.innerWidth, window.innerHeight);
    return a;
  };
  window.createEjectaRuntime = function () {
    var a = new d(document.getElementById("canvas"));
    window.c2runtime = a;
    window.c2runtime.setSize(window.innerWidth, window.innerHeight);
    return a;
  };
})();
window.cr_getC2Runtime = function () {
  var d = document.getElementById("c2canvas");
  return d ? d.c2runtime : window.c2runtime ? window.c2runtime : null;
};
window.cr_getSnapshot = function (d, m) {
  var q = window.cr_getC2Runtime();
  q && q.Cq(d, m);
};
window.cr_sizeCanvas = function (d, m) {
  if (0 !== d && 0 !== m) {
    var q = window.cr_getC2Runtime();
    q && q.setSize(d, m);
  }
};
window.cr_setSuspended = function (d) {
  var m = window.cr_getC2Runtime();
  m && m.setSuspended(d);
};
(function () {
  function d(a, b) {
    this.b = a;
    this.Je = null;
    this.scrollX = this.b.Ua / 2;
    this.scrollY = this.b.Ta / 2;
    this.scale = 1;
    this.k = 0;
    this.Bf = !0;
    this.name = b[0];
    this.Zs = b[1];
    this.Ys = b[2];
    this.width = b[1];
    this.height = b[2];
    this.cp = b[3];
    this.Mo = b[4];
    this.V = b[5];
    var c = b[6],
      f,
      d;
    this.O = [];
    this.Ng = [];
    f = 0;
    for (d = c.length; f < d; f++) {
      var h = new Jb(this, c[f]);
      h.fo = f;
      this.O.push(h);
    }
    c = b[7];
    this.de = [];
    f = 0;
    for (d = c.length; f < d; f++) {
      var h = c[f],
        n = this.b.C[h[1]];
      n.sc || (n.sc = h);
      this.de.push(h);
      -1 === this.Ng.indexOf(n) && this.Ng.push(n);
    }
    this.N = [];
    this.X = [];
    this.Ed = !0;
    this.Ea = [];
    f = 0;
    for (d = b[8].length; f < d; f++)
      this.N.push({
        id: b[8][f][0],
        name: b[8][f][1],
        Na: -1,
        Pc: !1,
        mb: !0,
        index: f,
      }),
        this.Ea.push(b[8][f][2].slice(0));
    this.Vc();
    this.qe = new U(0, 0, 1, 1);
    this.sl = new U(0, 0, 1, 1);
    this.pe = {};
  }
  function m(a, b) {
    return a.Wc - b.Wc;
  }
  function q(a, b) {
    this.Ra = a;
    this.b = a.b;
    this.j = [];
    this.scale = 1;
    this.k = 0;
    this.qd = !1;
    this.Hd = new U(0, 0, 0, 0);
    this.Yo = new sa();
    this.ka = this.ea = this.la = this.da = 0;
    this.of = !1;
    this.Md = -1;
    this.Yj = 0;
    this.name = b[0];
    this.index = b[1];
    this.V = b[2];
    this.visible = b[3];
    this.Ec = b[4];
    this.Uc = b[5];
    this.zd = b[6];
    this.Ad = b[7];
    this.opacity = b[8];
    this.ni = b[9];
    this.qc = b[10];
    this.Mh = b[11];
    this.nb = b[12];
    this.$q = b[13];
    this.sg = "source-over";
    this.sb = this.wb = 0;
    this.jb = null;
    this.wd = h();
    this.Sc = !0;
    this.Of = new U(0, 0, -1, -1);
    this.fb = new U(0, 0, -1, -1);
    this.qc && (this.jb = new Xa(this.b.Ua, this.b.Ta));
    this.gd = !1;
    var c = b[14],
      f,
      d;
    this.So = [];
    this.lc = [];
    this.vg = [];
    f = 0;
    for (d = c.length; f < d; f++) {
      var n = c[f],
        m = this.b.C[n[1]];
      m.sc || ((m.sc = n), (m.Bq = this.index));
      this.lc.push(n);
      -1 === this.Ra.Ng.indexOf(m) && this.Ra.Ng.push(m);
    }
    ya(this.So, this.lc);
    this.N = [];
    this.X = [];
    this.Ed = !0;
    this.Ea = [];
    f = 0;
    for (d = b[15].length; f < d; f++)
      this.N.push({
        id: b[15][f][0],
        name: b[15][f][1],
        Na: -1,
        Pc: !1,
        mb: !0,
        index: f,
      }),
        this.Ea.push(b[15][f][2].slice(0));
    this.Vc();
    this.qe = new U(0, 0, 1, 1);
    this.sl = new U(0, 0, 1, 1);
  }
  function h() {
    return a.length ? a.pop() : [];
  }
  function n(e) {
    V(e);
    a.push(e);
  }
  d.prototype.yt = function (a) {
    var b = a.type.V.toString();
    this.pe.hasOwnProperty(b) || (this.pe[b] = []);
    this.pe[b].push(this.b.zl(a));
  };
  d.prototype.Zm = function () {
    var a = this.O[0];
    return !a.Uc && 1 === a.opacity && !a.ni && a.visible;
  };
  d.prototype.Vc = function () {
    V(this.X);
    this.Ed = !0;
    var a, b, c;
    a = 0;
    for (b = this.N.length; a < b; a++)
      (c = this.N[a]), c.mb && (this.X.push(c), c.Pc || (this.Ed = !1));
  };
  d.prototype.pk = function (a) {
    var b, c, f;
    b = 0;
    for (c = this.N.length; b < c; b++)
      if (((f = this.N[b]), f.name === a)) return f;
    return null;
  };
  var t = [],
    c = !0;
  d.prototype.Hl = function () {
    this.Mo && ((this.Je = this.b.gk[this.Mo]), this.Je.Ul());
    this.b.ja = this;
    this.width = this.Zs;
    this.height = this.Ys;
    this.scrollX = this.b.Ua / 2;
    this.scrollY = this.b.Ta / 2;
    var a, b, f, l, d, h, n;
    a = 0;
    for (f = this.b.C.length; a < f; a++)
      if (((b = this.b.C[a]), !b.K))
        for (d = b.j, b = 0, l = d.length; b < l; b++)
          if (((h = d[b]), h.n)) {
            var g = h.n.fo;
            g >= this.O.length && (g = this.O.length - 1);
            h.n = this.O[g];
            -1 === h.n.j.indexOf(h) && h.n.j.push(h);
            h.n.of = !0;
          }
    if (!c) for (a = 0, f = this.O.length; a < f; ++a) this.O[a].j.sort(m);
    V(t);
    this.qq();
    a = 0;
    for (f = this.O.length; a < f; a++) (h = this.O[a]), h.zq(), h.Cj();
    d = !1;
    if (!this.Bf) {
      for (n in this.pe)
        if (
          this.pe.hasOwnProperty(n) &&
          (b = this.b.ri(parseInt(n, 10))) &&
          !b.K &&
          this.b.Sl(b)
        ) {
          l = this.pe[n];
          a = 0;
          for (f = l.length; a < f; a++) {
            h = null;
            if (b.Ba.ge && ((h = this.qi(l[a].w.l)), !h)) continue;
            h = this.b.Xd(b.sc, h, !1, 0, 0, !0);
            this.b.Ji(h, l[a]);
            d = !0;
            t.push(h);
          }
          V(l);
        }
      a = 0;
      for (f = this.O.length; a < f; a++)
        this.O[a].j.sort(m), (this.O[a].of = !0);
    }
    d && (this.b.lb(), this.b.yo());
    for (a = 0; a < t.length; a++)
      if (((h = t[a]), h.type.Jb))
        for (f = h.Ig(), b = 0, l = h.type.Qb.length; b < l; b++)
          (n = h.type.Qb[b]),
            h.type !== n &&
              (n.j.length > f
                ? h.siblings.push(n.j[f])
                : n.sc &&
                  ((d = this.b.Xd(n.sc, h.n, !0, h.x, h.y, !0)),
                  this.b.lb(),
                  n.Bj(),
                  h.siblings.push(d),
                  t.push(d)));
    a = 0;
    for (f = this.de.length; a < f; a++)
      (h = this.de[a]),
        (b = this.b.C[h[1]]),
        b.Jb || this.b.Xd(this.de[a], null, !0);
    this.b.rg = null;
    this.b.lb();
    if (this.b.ya && !this.b.Ha)
      for (a = 0, f = this.b.C.length; a < f; a++)
        (n = this.b.C[a]), !n.K && n.j.length && n.ml && n.ml(this.b.ya);
    if (this.b.Qg) ya(this.b.ji, t);
    else
      for (a = 0, f = t.length; a < f; a++)
        (h = t[a]), this.b.trigger(Object.getPrototypeOf(h.type.Ba).q.Nh, h);
    V(t);
    this.b.Qg || this.b.trigger(Y.prototype.q.im, null);
    this.Bf = !1;
  };
  d.prototype.yq = function () {
    var a, b, c, f, d;
    b = a = 0;
    for (c = this.de.length; a < c; a++)
      (f = this.de[a]),
        (d = this.b.C[f[1]]),
        d.global ? d.Jb || this.b.Xd(f, null, !0) : ((this.de[b] = f), b++);
    xa(this.de, b);
  };
  d.prototype.Ft = function () {
    this.b.Qg || this.b.trigger(Y.prototype.q.Np, null);
    this.b.Gk = !0;
    V(this.b.jd.ec);
    var a, b, f, l, d, h;
    if (!this.Bf)
      for (a = 0, b = this.O.length; a < b; a++)
        for (this.O[a].Wl(), d = this.O[a].j, f = 0, l = d.length; f < l; f++)
          (h = d[f]), h.type.global || (this.b.Sl(h.type) && this.yt(h));
    a = 0;
    for (b = this.O.length; a < b; a++) {
      d = this.O[a].j;
      f = 0;
      for (l = d.length; f < l; f++) (h = d[f]), h.type.global || this.b.pf(h);
      this.b.lb();
      V(d);
      this.O[a].of = !0;
    }
    a = 0;
    for (b = this.b.C.length; a < b; a++)
      if (((d = this.b.C[a]), !(d.global || d.Ba.ge || d.Ba.pj || d.K))) {
        f = 0;
        for (l = d.j.length; f < l; f++) this.b.pf(d.j[f]);
        this.b.lb();
      }
    c = !1;
    this.b.Gk = !1;
  };
  new U(0, 0, 0, 0);
  d.prototype.Hc = function (a) {
    var b,
      c = a,
      f = !1,
      d = !this.b.jc;
    d &&
      (this.b.Hi ||
        ((this.b.Hi = document.createElement("canvas")),
        (b = this.b.Hi),
        (b.width = this.b.J),
        (b.height = this.b.I),
        (this.b.Fn = b.getContext("2d")),
        (f = !0)),
      (b = this.b.Hi),
      (c = this.b.Fn),
      b.width !== this.b.J && ((b.width = this.b.J), (f = !0)),
      b.height !== this.b.I && ((b.height = this.b.I), (f = !0)),
      f && this.b.jj(c, this.b.Aa));
    c.globalAlpha = 1;
    c.globalCompositeOperation = "source-over";
    this.b.zm && !this.Zm() && c.clearRect(0, 0, this.b.J, this.b.I);
    var h,
      n,
      f = 0;
    for (h = this.O.length; f < h; f++)
      (n = this.O[f]),
        n.visible && 0 < n.opacity && 11 !== n.nb && (n.j.length || !n.Uc)
          ? n.Hc(c)
          : n.Cj();
    d && a.drawImage(b, 0, 0, this.b.width, this.b.height);
  };
  d.prototype.Fe = function (a) {
    a.Go(!0);
    this.b.La || (this.b.La = a.hc(this.b.J, this.b.I, this.b.Aa));
    if (this.b.La.Ce !== this.b.J || this.b.La.Be !== this.b.I)
      a.deleteTexture(this.b.La),
        (this.b.La = a.hc(this.b.J, this.b.I, this.b.Aa));
    a.xc(this.b.La);
    this.b.jc || a.gf(this.b.J, this.b.I);
    var b, c;
    for (b = this.O.length - 1; 0 <= b; --b)
      (c = this.O[b]),
        c.visible &&
        1 === c.opacity &&
        c.Ed &&
        0 === c.nb &&
        (c.j.length || !c.Uc)
          ? c.Fe(a)
          : c.Cj();
    a.Go(!1);
  };
  d.prototype.Eb = function (a) {
    var b = 0 < this.X.length || this.b.fg || !this.b.jc || this.b.ma;
    if (b) {
      this.b.La || (this.b.La = a.hc(this.b.J, this.b.I, this.b.Aa));
      if (this.b.La.Ce !== this.b.J || this.b.La.Be !== this.b.I)
        a.deleteTexture(this.b.La),
          (this.b.La = a.hc(this.b.J, this.b.I, this.b.Aa));
      a.xc(this.b.La);
      this.b.jc || a.gf(this.b.J, this.b.I);
    } else
      this.b.La && (a.xc(null), a.deleteTexture(this.b.La), (this.b.La = null));
    this.b.zm && !this.Zm() && a.clear(0, 0, 0, 0);
    var c, f, d;
    c = 0;
    for (f = this.O.length; c < f; c++)
      (d = this.O[c]),
        d.visible && 0 < d.opacity && (d.j.length || !d.Uc) ? d.Eb(a) : d.Cj();
    b &&
      (0 === this.X.length || (1 === this.X.length && this.b.jc)
        ? (1 === this.X.length
            ? ((b = this.X[0].index),
              a.Mb(this.X[0].Na),
              a.Yf(
                null,
                1 / this.b.J,
                1 / this.b.I,
                0,
                0,
                1,
                1,
                this.scale,
                this.k,
                0,
                0,
                this.b.J / 2,
                this.b.I / 2,
                this.b.Vb.wa,
                this.Ea[b]
              ),
              a.$i(this.X[0].Na) && (this.b.ca = !0))
            : a.Mb(0),
          this.b.jc || a.gf(this.b.width, this.b.height),
          a.xc(null),
          a.Fo(!1),
          a.ff(1),
          a.$b(this.b.La),
          a.Do(),
          a.wc(),
          a.pc(),
          (b = this.b.width / 2),
          (c = this.b.height / 2),
          a.wh(-b, c, b, c, b, -c, -b, -c),
          a.$b(null),
          a.Fo(!0))
        : this.tl(a, null, null, null));
  };
  d.prototype.Gg = function () {
    return 0 < this.X.length || this.b.fg || !this.b.jc || this.b.ma
      ? this.b.La
      : null;
  };
  d.prototype.Wm = function () {
    var a = this.O[0].Gb(),
      b,
      c,
      f;
    b = 1;
    for (c = this.O.length; b < c; b++)
      (f = this.O[b]), (0 !== f.zd || 0 !== f.Ad) && f.Gb() < a && (a = f.Gb());
    return a;
  };
  d.prototype.Bl = function (a) {
    if (!this.cp) {
      var b = ((1 / this.Wm()) * this.b.J) / 2;
      a > this.width - b && (a = this.width - b);
      a < b && (a = b);
    }
    this.scrollX !== a && ((this.scrollX = a), (this.b.ca = !0));
  };
  d.prototype.Cl = function (a) {
    if (!this.cp) {
      var b = ((1 / this.Wm()) * this.b.I) / 2;
      a > this.height - b && (a = this.height - b);
      a < b && (a = b);
    }
    this.scrollY !== a && ((this.scrollY = a), (this.b.ca = !0));
  };
  d.prototype.qq = function () {
    this.Bl(this.scrollX);
    this.Cl(this.scrollY);
  };
  d.prototype.tl = function (a, b, c, f) {
    var d = c ? c.X : b ? b.X : this.X,
      h = 1,
      n = 0,
      g = 0,
      k = 0,
      u = this.b.J,
      m = this.b.I;
    c
      ? ((h = c.n.Gb()),
        (n = c.n.Ub()),
        (g = c.n.da),
        (k = c.n.ea),
        (u = c.n.la),
        (m = c.n.ka))
      : b &&
        ((h = b.Gb()),
        (n = b.Ub()),
        (g = b.da),
        (k = b.ea),
        (u = b.la),
        (m = b.ka));
    var r = this.b.mk,
      q,
      t,
      C,
      I,
      M = 0,
      ba = 1,
      R,
      K,
      G = this.b.J,
      N = this.b.I,
      na = G / 2,
      ha = N / 2,
      J = b ? b.qe : this.qe,
      v = b ? b.sl : this.sl,
      y = 0,
      B = 0,
      H = 0,
      D = 0,
      S = G,
      ta = G,
      Q = N,
      ua = N,
      oa = (C = 0);
    I = c ? c.n.Ub() : 0;
    if (c) {
      q = 0;
      for (t = d.length; q < t; q++)
        (C += a.as(d[q].Na)), (oa += a.bs(d[q].Na));
      D = c.xa;
      y = b.ia(D.left, D.top, !0, !0);
      H = b.ia(D.left, D.top, !1, !0);
      S = b.ia(D.right, D.bottom, !0, !0);
      Q = b.ia(D.right, D.bottom, !1, !0);
      0 !== I &&
        ((q = b.ia(D.right, D.top, !0, !0)),
        (t = b.ia(D.right, D.top, !1, !0)),
        (B = b.ia(D.left, D.bottom, !0, !0)),
        (D = b.ia(D.left, D.bottom, !1, !0)),
        (I = Math.min(y, S, q, B)),
        (S = Math.max(y, S, q, B)),
        (y = I),
        (I = Math.min(H, Q, t, D)),
        (Q = Math.max(H, Q, t, D)),
        (H = I));
      y -= C;
      H -= oa;
      S += C;
      Q += oa;
      v.left = y / G;
      v.top = 1 - H / N;
      v.right = S / G;
      v.bottom = 1 - Q / N;
      B = y = O(y);
      D = H = O(H);
      ta = S = qa(S);
      ua = Q = qa(Q);
      B -= C;
      D -= oa;
      ta += C;
      ua += oa;
      0 > y && (y = 0);
      0 > H && (H = 0);
      S > G && (S = G);
      Q > N && (Q = N);
      0 > B && (B = 0);
      0 > D && (D = 0);
      ta > G && (ta = G);
      ua > N && (ua = N);
      J.left = y / G;
      J.top = 1 - H / N;
      J.right = S / G;
      J.bottom = 1 - Q / N;
    } else
      (J.left = v.left = 0),
        (J.top = v.top = 0),
        (J.right = v.right = 1),
        (J.bottom = v.bottom = 1);
    oa =
      (c &&
        (a.sh(d[0].Na) ||
          0 !== C ||
          0 !== oa ||
          1 !== c.opacity ||
          c.type.Ba.bo)) ||
      (b && !c && 1 !== b.opacity);
    a.Do();
    if (oa) {
      r[M] || (r[M] = a.hc(G, N, this.b.Aa));
      if (r[M].Ce !== G || r[M].Be !== N)
        a.deleteTexture(r[M]), (r[M] = a.hc(G, N, this.b.Aa));
      a.Mb(0);
      a.xc(r[M]);
      K = ua - D;
      a.clearRect(B, N - D - K, ta - B, K);
      c
        ? c.Eb(a)
        : (a.$b(this.b.Qa),
          a.ff(b.opacity),
          a.wc(),
          a.translate(-na, -ha),
          a.pc(),
          a.fd(y, Q, S, Q, S, H, y, H, J));
      v.left = v.top = 0;
      v.right = v.bottom = 1;
      c && ((I = J.top), (J.top = J.bottom), (J.bottom = I));
      M = 1;
      ba = 0;
    }
    a.ff(1);
    C = d.length - 1;
    var Za = a.ol(d[C].Na) || (!b && !c && !this.b.jc);
    q = I = 0;
    for (t = d.length; q < t; q++) {
      r[M] || (r[M] = a.hc(G, N, this.b.Aa));
      if (r[M].Ce !== G || r[M].Be !== N)
        a.deleteTexture(r[M]), (r[M] = a.hc(G, N, this.b.Aa));
      a.Mb(d[q].Na);
      I = d[q].index;
      a.$i(d[q].Na) && (this.b.ca = !0);
      0 != q || oa
        ? (a.Yf(
            f,
            1 / G,
            1 / N,
            v.left,
            v.top,
            v.right,
            v.bottom,
            h,
            n,
            g,
            k,
            (g + u) / 2,
            (k + m) / 2,
            this.b.Vb.wa,
            c ? c.Ea[I] : b ? b.Ea[I] : this.Ea[I]
          ),
          a.$b(null),
          q !== C || Za
            ? (a.xc(r[M]),
              (K = ua - D),
              (R = N - D - K),
              a.clearRect(B, R, ta - B, K))
            : (c ? a.ue(c.wb, c.sb) : b && a.ue(b.wb, b.sb), a.xc(f)),
          a.$b(r[ba]),
          a.wc(),
          a.translate(-na, -ha),
          a.pc(),
          a.fd(y, Q, S, Q, S, H, y, H, J),
          q !== C || Za || a.$b(null))
        : (a.xc(r[M]),
          (K = ua - D),
          (R = N - D - K),
          a.clearRect(B, R, ta - B, K),
          c
            ? (c.eb && c.eb.T
                ? ((R = c.eb.T), (ba = 1 / R.width), (R = 1 / R.height))
                : ((ba = 1 / c.width), (R = 1 / c.height)),
              a.Yf(
                f,
                ba,
                R,
                v.left,
                v.top,
                v.right,
                v.bottom,
                h,
                n,
                g,
                k,
                (g + u) / 2,
                (k + m) / 2,
                this.b.Vb.wa,
                c.Ea[I]
              ),
              c.Eb(a))
            : (a.Yf(
                f,
                1 / G,
                1 / N,
                0,
                0,
                1,
                1,
                h,
                n,
                g,
                k,
                (g + u) / 2,
                (k + m) / 2,
                this.b.Vb.wa,
                b ? b.Ea[I] : this.Ea[I]
              ),
              a.$b(b ? this.b.Qa : this.b.La),
              a.wc(),
              a.translate(-na, -ha),
              a.pc(),
              a.fd(y, Q, S, Q, S, H, y, H, J)),
          (v.left = v.top = 0),
          (v.right = v.bottom = 1),
          c && !Za && ((I = Q), (Q = H), (H = I)));
      M = 0 === M ? 1 : 0;
      ba = 0 === M ? 1 : 0;
    }
    Za &&
      (a.Mb(0),
      c
        ? a.ue(c.wb, c.sb)
        : b
        ? a.ue(b.wb, b.sb)
        : this.b.jc ||
          (a.gf(this.b.width, this.b.height),
          (na = this.b.width / 2),
          (ha = this.b.height / 2),
          (H = y = 0),
          (S = this.b.width),
          (Q = this.b.height)),
      a.xc(f),
      a.$b(r[ba]),
      a.wc(),
      a.translate(-na, -ha),
      a.pc(),
      c && 1 === d.length && !oa
        ? a.fd(y, H, S, H, S, Q, y, Q, J)
        : a.fd(y, Q, S, Q, S, H, y, H, J),
      a.$b(null));
  };
  d.prototype.qi = function (a) {
    var b, c;
    b = 0;
    for (c = this.O.length; b < c; b++) if (this.O[b].V === a) return this.O[b];
    return null;
  };
  d.prototype.kb = function () {
    var a,
      b,
      c,
      f = {
        sx: this.scrollX,
        sy: this.scrollY,
        s: this.scale,
        a: this.k,
        w: this.width,
        h: this.height,
        fv: this.Bf,
        persist: this.pe,
        fx: [],
        layers: {},
      };
    a = 0;
    for (b = this.N.length; a < b; a++)
      (c = this.N[a]),
        f.fx.push({ name: c.name, active: c.mb, params: this.Ea[c.index] });
    a = 0;
    for (b = this.O.length; a < b; a++)
      (c = this.O[a]), (f.layers[c.V.toString()] = c.kb());
    return f;
  };
  d.prototype.Ab = function (a) {
    var b, c, f, d;
    this.scrollX = a.sx;
    this.scrollY = a.sy;
    this.scale = a.s;
    this.k = a.a;
    this.width = a.w;
    this.height = a.h;
    this.pe = a.persist;
    "undefined" !== typeof a.fv && (this.Bf = a.fv);
    var h = a.fx;
    b = 0;
    for (c = h.length; b < c; b++)
      if ((f = this.pk(h[b].name)))
        (f.mb = h[b].active), (this.Ea[f.index] = h[b].params);
    this.Vc();
    b = a.layers;
    for (d in b)
      b.hasOwnProperty(d) && (a = this.qi(parseInt(d, 10))) && a.Ab(b[d]);
  };
  vb = d;
  q.prototype.Vc = function () {
    V(this.X);
    this.Ed = !0;
    var a, b, c;
    a = 0;
    for (b = this.N.length; a < b; a++)
      (c = this.N[a]), c.mb && (this.X.push(c), c.Pc || (this.Ed = !1));
  };
  q.prototype.pk = function (a) {
    var b, c, f;
    b = 0;
    for (c = this.N.length; b < c; b++)
      if (((f = this.N[b]), f.name === a)) return f;
    return null;
  };
  q.prototype.zq = function () {
    var a, b, c, f, d, h;
    b = a = 0;
    for (c = this.lc.length; a < c; a++) {
      f = this.lc[a];
      d = this.b.C[f[1]];
      h = this.b.Sl(d);
      d = !0;
      if (!h || this.Ra.Bf) {
        f = this.b.Xd(f, this, !0);
        if (!f) continue;
        t.push(f);
        f.type.global && ((d = !1), this.vg.push(f.uid));
      }
      d && ((this.lc[b] = this.lc[a]), b++);
    }
    this.lc.length = b;
    this.b.lb();
    !this.b.A && this.N.length && (this.nb = this.$q);
    this.sg = Ya(this.nb);
    this.b.D && $a(this, this.nb, this.b.D);
    this.Sc = !0;
  };
  q.prototype.Wf = function (a, b) {
    var c = Aa(this.j, a);
    0 > c ||
      (b &&
        this.qc &&
        a.Lb &&
        a.Lb.right >= a.Lb.left &&
        (a.qa(), this.jb.update(a, a.Lb, null), a.Lb.set(0, 0, -1, -1)),
      c === this.j.length - 1 ? this.j.pop() : (wa(this.j, c), this.zh(c)),
      (this.Sc = !0));
  };
  q.prototype.ng = function (a, b) {
    a.Wc = this.j.length;
    this.j.push(a);
    b && this.qc && a.Lb && a.B();
    this.Sc = !0;
  };
  q.prototype.ht = function (a) {
    this.j.unshift(a);
    this.zh(0);
  };
  q.prototype.Us = function (a, b, c) {
    var f = a.bd();
    b = b.bd();
    wa(this.j, f);
    f < b && b--;
    c && b++;
    b === this.j.length ? this.j.push(a) : this.j.splice(b, 0, a);
    this.zh(f < b ? f : b);
  };
  q.prototype.zh = function (a) {
    -1 === this.Md ? (this.Md = a) : a < this.Md && (this.Md = a);
    this.Sc = this.of = !0;
  };
  q.prototype.Wl = function () {
    if (this.of) {
      -1 === this.Md && (this.Md = 0);
      var a, b, c;
      if (this.qc)
        for (a = this.Md, b = this.j.length; a < b; ++a)
          (c = this.j[a]), (c.Wc = a), this.jb.Ss(c.Lb);
      else for (a = this.Md, b = this.j.length; a < b; ++a) this.j[a].Wc = a;
      this.of = !1;
      this.Md = -1;
    }
  };
  q.prototype.Gb = function (a) {
    return this.Zr() * (this.b.jc || a ? this.b.og : 1);
  };
  q.prototype.Zr = function () {
    return (this.scale * this.Ra.scale - 1) * this.Mh + 1;
  };
  q.prototype.Ub = function () {
    return this.qd ? 0 : X(this.Ra.k + this.k);
  };
  var a = [],
    f = [],
    r = [];
  q.prototype.sk = function () {
    this.Wl();
    this.jb.ej(this.da, this.ea, this.la, this.ka, r);
    if (!r.length) return h();
    if (1 === r.length) {
      var a = h();
      ya(a, r[0]);
      V(r);
      return a;
    }
    for (var b = !0; 1 < r.length; ) {
      for (
        var a = r,
          c = void 0,
          d = void 0,
          m = void 0,
          q = void 0,
          t = void 0,
          c = 0,
          d = a.length;
        c < d - 1;
        c += 2
      ) {
        var m = a[c],
          q = a[c + 1],
          t = h(),
          g = m,
          k = q,
          u = t,
          P = 0,
          E = 0,
          T = 0,
          F = g.length,
          C = k.length,
          I = void 0,
          M = void 0;
        for (u.length = F + C; P < F && E < C; ++T)
          (I = g[P]),
            (M = k[E]),
            I.Wc < M.Wc ? ((u[T] = I), ++P) : ((u[T] = M), ++E);
        for (; P < F; ++P, ++T) u[T] = g[P];
        for (; E < C; ++E, ++T) u[T] = k[E];
        b || (n(m), n(q));
        f.push(t);
      }
      1 === d % 2 &&
        (b ? ((m = h()), ya(m, a[d - 1]), f.push(m)) : f.push(a[d - 1]));
      ya(a, f);
      V(f);
      b = !1;
    }
    a = r[0];
    V(r);
    return a;
  };
  q.prototype.Hc = function (a) {
    this.gd = this.ni || 1 !== this.opacity || 0 !== this.nb;
    var b = this.b.canvas,
      c = a,
      f = !1;
    this.gd &&
      (this.b.Gi ||
        ((this.b.Gi = document.createElement("canvas")),
        (b = this.b.Gi),
        (b.width = this.b.J),
        (b.height = this.b.I),
        (this.b.En = b.getContext("2d")),
        (f = !0)),
      (b = this.b.Gi),
      (c = this.b.En),
      b.width !== this.b.J && ((b.width = this.b.J), (f = !0)),
      b.height !== this.b.I && ((b.height = this.b.I), (f = !0)),
      f && this.b.jj(c, this.b.Aa),
      this.Uc && c.clearRect(0, 0, this.b.J, this.b.I));
    c.globalAlpha = 1;
    c.globalCompositeOperation = "source-over";
    this.Uc ||
      ((c.fillStyle =
        "rgb(" + this.Ec[0] + "," + this.Ec[1] + "," + this.Ec[2] + ")"),
      c.fillRect(0, 0, this.b.J, this.b.I));
    c.save();
    this.qd = !0;
    var f = this.Ud(!0),
      d = this.Ud(!1);
    this.qd = !1;
    this.b.Oc && ((f = Math.round(f)), (d = Math.round(d)));
    this.gj(f, d, c);
    var h = this.Gb();
    c.scale(h, h);
    c.translate(-f, -d);
    this.qc
      ? ((this.fb.left = this.jb.Cb(this.da)),
        (this.fb.top = this.jb.Db(this.ea)),
        (this.fb.right = this.jb.Cb(this.la)),
        (this.fb.bottom = this.jb.Db(this.ka)),
        this.Sc || !this.fb.yg(this.Of)
          ? (n(this.wd), (f = this.sk()), (this.Sc = !1), this.Of.tg(this.fb))
          : (f = this.wd))
      : (f = this.j);
    for (var m, g = null, d = 0, h = f.length; d < h; ++d)
      (m = f[d]), m !== g && (this.Vq(m, c), (g = m));
    this.qc && (this.wd = f);
    c.restore();
    this.gd &&
      ((a.globalCompositeOperation = this.sg),
      (a.globalAlpha = this.opacity),
      a.drawImage(b, 0, 0));
  };
  q.prototype.Vq = function (a, b) {
    if (a.visible && 0 !== a.width && 0 !== a.height) {
      a.qa();
      var c = a.xa;
      c.right < this.da ||
        c.bottom < this.ea ||
        c.left > this.la ||
        c.top > this.ka ||
        ((b.globalCompositeOperation = a.sg), a.Hc(b));
    }
  };
  q.prototype.Cj = function () {
    this.qd = !0;
    var a = this.Ud(!0),
      b = this.Ud(!1);
    this.qd = !1;
    this.b.Oc && ((a = Math.round(a)), (b = Math.round(b)));
    this.gj(a, b, null);
  };
  q.prototype.gj = function (a, b, c) {
    var f = this.Gb();
    this.da = a;
    this.ea = b;
    this.la = a + (1 / f) * this.b.J;
    this.ka = b + (1 / f) * this.b.I;
    this.da > this.la && ((a = this.da), (this.da = this.la), (this.la = a));
    this.ea > this.ka && ((a = this.ea), (this.ea = this.ka), (this.ka = a));
    a = this.Ub();
    0 !== a &&
      (c &&
        (c.translate(this.b.J / 2, this.b.I / 2),
        c.rotate(-a),
        c.translate(this.b.J / -2, this.b.I / -2)),
      this.Hd.set(this.da, this.ea, this.la, this.ka),
      this.Hd.offset((this.da + this.la) / -2, (this.ea + this.ka) / -2),
      this.Yo.Ko(this.Hd, a),
      this.Yo.vm(this.Hd),
      this.Hd.offset((this.da + this.la) / 2, (this.ea + this.ka) / 2),
      (this.da = this.Hd.left),
      (this.ea = this.Hd.top),
      (this.la = this.Hd.right),
      (this.ka = this.Hd.bottom));
  };
  q.prototype.Fe = function (a) {
    if ((this.gd = this.ni)) {
      this.b.Qa || (this.b.Qa = a.hc(this.b.J, this.b.I, this.b.Aa));
      if (this.b.Qa.Ce !== this.b.J || this.b.Qa.Be !== this.b.I)
        a.deleteTexture(this.b.Qa),
          (this.b.Qa = a.hc(this.b.J, this.b.I, this.b.Aa));
      a.xc(this.b.Qa);
    }
    this.qd = !0;
    var b = this.Ud(!0),
      c = this.Ud(!1);
    this.qd = !1;
    this.b.Oc && ((b = Math.round(b)), (c = Math.round(c)));
    this.gj(b, c, null);
    b = this.Gb();
    a.wc();
    a.scale(b, b);
    a.hj(-this.Ub());
    a.translate((this.da + this.la) / -2, (this.ea + this.ka) / -2);
    a.pc();
    this.qc
      ? ((this.fb.left = this.jb.Cb(this.da)),
        (this.fb.top = this.jb.Db(this.ea)),
        (this.fb.right = this.jb.Cb(this.la)),
        (this.fb.bottom = this.jb.Db(this.ka)),
        this.Sc || !this.fb.yg(this.Of)
          ? (n(this.wd), (b = this.sk()), (this.Sc = !1), this.Of.tg(this.fb))
          : (b = this.wd))
      : (b = this.j);
    for (var f, d = null, c = b.length - 1; 0 <= c; --c)
      (f = b[c]), f !== d && (this.Xq(b[c], a), (d = f));
    this.qc && (this.wd = b);
    this.Uc ||
      ((this.Yj = this.b.He++), a.kj(this.Yj), a.Eo(1, 1, 1), a.Om(), a.nt());
  };
  q.prototype.Eb = function (a) {
    var b = 0,
      c = 0;
    if (
      (this.gd =
        this.ni || 1 !== this.opacity || 0 < this.X.length || 0 !== this.nb)
    ) {
      this.b.Qa || (this.b.Qa = a.hc(this.b.J, this.b.I, this.b.Aa));
      if (this.b.Qa.Ce !== this.b.J || this.b.Qa.Be !== this.b.I)
        a.deleteTexture(this.b.Qa),
          (this.b.Qa = a.hc(this.b.J, this.b.I, this.b.Aa));
      a.xc(this.b.Qa);
      this.Uc && a.clear(0, 0, 0, 0);
    }
    this.Uc ||
      (this.b.ma
        ? (a.kj(this.Yj),
          a.Eo(this.Ec[0] / 255, this.Ec[1] / 255, this.Ec[2] / 255),
          a.Om(),
          a.Bt())
        : a.clear(this.Ec[0] / 255, this.Ec[1] / 255, this.Ec[2] / 255, 1));
    this.qd = !0;
    var f = this.Ud(!0),
      b = this.Ud(!1);
    this.qd = !1;
    this.b.Oc && ((f = Math.round(f)), (b = Math.round(b)));
    this.gj(f, b, null);
    f = this.Gb();
    a.wc();
    a.scale(f, f);
    a.hj(-this.Ub());
    a.translate((this.da + this.la) / -2, (this.ea + this.ka) / -2);
    a.pc();
    this.qc
      ? ((this.fb.left = this.jb.Cb(this.da)),
        (this.fb.top = this.jb.Db(this.ea)),
        (this.fb.right = this.jb.Cb(this.la)),
        (this.fb.bottom = this.jb.Db(this.ka)),
        this.Sc || !this.fb.yg(this.Of)
          ? (n(this.wd), (b = this.sk()), (this.Sc = !1), this.Of.tg(this.fb))
          : (b = this.wd))
      : (b = this.j);
    var d,
      h,
      m = null,
      c = 0;
    for (d = b.length; c < d; ++c)
      (h = b[c]), h !== m && (this.Wq(b[c], a), (m = h));
    this.qc && (this.wd = b);
    this.gd &&
      ((b = this.X.length ? this.X[0].Na : 0),
      (c = this.X.length ? this.X[0].index : 0),
      0 === this.X.length ||
      (1 === this.X.length && !a.ol(b) && 1 === this.opacity)
        ? (1 === this.X.length
            ? (a.Mb(b),
              a.Yf(
                this.Ra.Gg(),
                1 / this.b.J,
                1 / this.b.I,
                0,
                0,
                1,
                1,
                f,
                this.Ub(),
                this.da,
                this.ea,
                (this.da + this.la) / 2,
                (this.ea + this.ka) / 2,
                this.b.Vb.wa,
                this.Ea[c]
              ),
              a.$i(b) && (this.b.ca = !0))
            : a.Mb(0),
          a.xc(this.Ra.Gg()),
          a.ff(this.opacity),
          a.$b(this.b.Qa),
          a.ue(this.wb, this.sb),
          a.wc(),
          a.pc(),
          (f = this.b.J / 2),
          (b = this.b.I / 2),
          a.wh(-f, b, f, b, f, -b, -f, -b),
          a.$b(null))
        : this.Ra.tl(a, this, null, this.Ra.Gg()));
  };
  q.prototype.Wq = function (a, b) {
    if (a.visible && 0 !== a.width && 0 !== a.height) {
      a.qa();
      var c = a.xa;
      c.right < this.da ||
        c.bottom < this.ea ||
        c.left > this.la ||
        c.top > this.ka ||
        (b.kj(a.He),
        a.kp ? this.Yq(a, b) : (b.Mb(0), b.ue(a.wb, a.sb), a.Eb(b)));
    }
  };
  q.prototype.Xq = function (a, b) {
    if (a.visible && 0 !== a.width && 0 !== a.height) {
      a.qa();
      var c = a.xa;
      c.right < this.da ||
        c.bottom < this.ea ||
        c.left > this.la ||
        c.top > this.ka ||
        ((a.He = this.b.He++),
        0 === a.nb && 1 === a.opacity && a.Ed && a.Fe && (b.kj(a.He), a.Fe(b)));
    }
  };
  q.prototype.Yq = function (a, b) {
    var c = a.X[0].Na,
      f = a.X[0].index,
      d = this.Gb();
    if (
      1 !== a.X.length ||
      b.ol(c) ||
      b.jt(c) ||
      ((a.k || a.n.Ub()) && b.sh(c)) ||
      1 !== a.opacity ||
      a.type.Ba.bo
    )
      this.Ra.tl(b, this, a, this.gd ? this.b.Qa : this.Ra.Gg()),
        b.wc(),
        b.scale(d, d),
        b.hj(-this.Ub()),
        b.translate((this.da + this.la) / -2, (this.ea + this.ka) / -2),
        b.pc();
    else {
      b.Mb(c);
      b.ue(a.wb, a.sb);
      b.$i(c) && (this.b.ca = !0);
      var h = 0,
        n = 0,
        g = 0,
        k = 0;
      b.sh(c) &&
        ((k = a.xa),
        (h = this.ia(k.left, k.top, !0, !0)),
        (n = this.ia(k.left, k.top, !1, !0)),
        (g = this.ia(k.right, k.bottom, !0, !0)),
        (k = this.ia(k.right, k.bottom, !1, !0)),
        (h = h / windowWidth),
        (n = 1 - n / windowHeight),
        (g = g / windowWidth),
        (k = 1 - k / windowHeight));
      var m;
      a.eb && a.eb.T
        ? ((m = a.eb.T), (c = 1 / m.width), (m = 1 / m.height))
        : ((c = 1 / a.width), (m = 1 / a.height));
      b.Yf(
        this.gd ? this.b.Qa : this.Ra.Gg(),
        c,
        m,
        h,
        n,
        g,
        k,
        d,
        this.Ub(),
        this.da,
        this.ea,
        (this.da + this.la) / 2,
        (this.ea + this.ka) / 2,
        this.b.Vb.wa,
        a.Ea[f]
      );
      a.Eb(b);
    }
  };
  q.prototype.Ud = function (a) {
    var b = 0,
      c = 0,
      f = this.b.devicePixelRatio;
    this.b.Se && ((b *= f), (c *= f));
    var f = this.b.no,
      d = this.b.oo,
      f = (this.Ra.scrollX - f) * this.zd + f,
      d = (this.Ra.scrollY - d) * this.Ad + d,
      h,
      n,
      g = 1 / this.Gb(!1);
    h = f - (this.b.J * g) / 2;
    n = d - (this.b.I * g) / 2;
    h += b * g;
    n += c * g;
    c = this.Ub();
    0 !== c &&
      ((h -= f),
      (n -= d),
      (b = Math.cos(c)),
      (c = Math.sin(c)),
      (g = h * b - n * c),
      (n = n * b + h * c),
      (h = g + f),
      (n += d));
    return a ? h : n;
  };
  q.prototype.ia = function (a, b, c, f) {
    var d = this.b.no,
      h = this.b.oo,
      n = (this.Ra.scrollX - d) * this.zd + d,
      g = (this.Ra.scrollY - h) * this.Ad + h,
      h = n,
      d = g,
      k = this.Ub();
    if (0 !== k) {
      a -= n;
      b -= g;
      var m = Math.cos(-k),
        k = Math.sin(-k),
        r = a * m - b * k;
      b = b * m + a * k;
      a = r + n;
      b += g;
    }
    n = 1 / this.Gb(!f);
    f
      ? ((h -= (this.b.J * n) / 2), (d -= (this.b.I * n) / 2))
      : ((h -= (this.b.width * n) / 2), (d -= (this.b.height * n) / 2));
    h = (a - h) / n;
    d = (b - d) / n;
    a = this.b.devicePixelRatio;
    this.b.Se && !f && ((h /= a), (d /= a));
    return c ? h : d;
  };
  q.prototype.kb = function () {
    var a,
      b,
      c,
      f = {
        s: this.scale,
        a: this.k,
        vl: this.da,
        vt: this.ea,
        vr: this.la,
        vb: this.ka,
        v: this.visible,
        bc: this.Ec,
        t: this.Uc,
        px: this.zd,
        py: this.Ad,
        o: this.opacity,
        zr: this.Mh,
        fx: [],
        cg: this.vg,
        instances: [],
      };
    a = 0;
    for (b = this.N.length; a < b; a++)
      (c = this.N[a]),
        f.fx.push({ name: c.name, active: c.mb, params: this.Ea[c.index] });
    return f;
  };
  q.prototype.Ab = function (a) {
    var b, c, f;
    this.scale = a.s;
    this.k = a.a;
    this.da = a.vl;
    this.ea = a.vt;
    this.la = a.vr;
    this.ka = a.vb;
    this.visible = a.v;
    this.Ec = a.bc;
    this.Uc = a.t;
    this.zd = a.px;
    this.Ad = a.py;
    this.opacity = a.o;
    this.Mh = a.zr;
    this.vg = a.cg || [];
    ya(this.lc, this.So);
    var d = new ca();
    b = 0;
    for (f = this.vg.length; b < f; ++b) d.add(this.vg[b]);
    c = b = 0;
    for (f = this.lc.length; b < f; ++b)
      d.contains(this.lc[b][2]) || ((this.lc[c] = this.lc[b]), ++c);
    xa(this.lc, c);
    c = a.fx;
    b = 0;
    for (f = c.length; b < f; b++)
      if ((a = this.pk(c[b].name)))
        (a.mb = c[b].active), (this.Ea[a.index] = c[b].params);
    this.Vc();
    this.j.sort(m);
    this.of = !0;
  };
  Jb = q;
})();
(function () {
  function d(a, b) {
    var c,
      e = a.length;
    switch (e) {
      case 0:
        return !0;
      case 1:
        return a[0] === b[0];
      case 2:
        return a[0] === b[0] && a[1] === b[1];
      default:
        for (c = 0; c < e; c++) if (a[c] !== b[c]) return !1;
        return !0;
    }
  }
  function m(a, b) {
    return a.index - b.index;
  }
  function q(a) {
    var b, c, e, f;
    2 === a.length
      ? a[0].index > a[1].index && ((b = a[0]), (a[0] = a[1]), (a[1] = b))
      : 2 < a.length && a.sort(m);
    a.length >= x.length && (x.length = a.length + 1);
    x[a.length] || (x[a.length] = []);
    f = x[a.length];
    b = 0;
    for (c = f.length; b < c; b++) if (((e = f[b]), d(a, e))) return e;
    f.push(a);
    return a;
  }
  function h(a, b) {
    this.b = a;
    this.$o = {};
    this.Km = {};
    this.wk = !1;
    this.bn = new ca();
    this.ck = [];
    this.Lj = [];
    this.name = b[0];
    var c = b[1];
    this.$d = [];
    var e, f;
    e = 0;
    for (f = c.length; e < f; e++) this.dn(c[e], null, this.$d);
  }
  function n(a) {
    this.type = a;
    this.j = [];
    this.Y = [];
    this.na = !0;
  }
  function t(a, b, c) {
    this.sheet = a;
    this.parent = b;
    this.b = a.b;
    this.Ca = [];
    this.ve = [];
    this.$m = this.yj = this.Nl = this.Ak = this.group = this.Qo = !1;
    this.$a = [];
    this.rc = [];
    this.yc = [];
    this.Kg = "";
    this.Jg = this.Ak = this.group = !1;
    this.$h = null;
    c[1] &&
      ((this.Kg = c[1][1].toLowerCase()),
      (this.group = !0),
      (this.Ak = !!c[1][0]),
      (this.$h = []),
      (this.Jg = this.Ak),
      this.b.tf.push(this),
      (this.b.si[this.Kg] = this));
    this.Nc = c[2];
    this.V = c[4];
    this.group || (this.b.um[this.V.toString()] = this);
    var e = c[5];
    a = 0;
    for (b = e.length; a < b; a++) {
      var f = new Kb(this, e[a]);
      f.index = a;
      this.$a.push(f);
      this.om(f.type);
    }
    e = c[6];
    a = 0;
    for (b = e.length; a < b; a++)
      (f = new Lb(this, e[a])), (f.index = a), this.rc.push(f);
    if (8 === c.length)
      for (c = c[7], a = 0, b = c.length; a < b; a++)
        this.sheet.dn(c[a], this, this.yc);
    this.Ai = !1;
    this.$a.length &&
      (this.Ai = null == this.$a[0].type && this.$a[0].gb == Y.prototype.q.up);
  }
  function c(a, b) {
    var c, e, f;
    if (a && (-1 === b.indexOf(a) && b.push(a), a.Jb))
      for (c = 0, e = a.Qb.length; c < e; c++)
        (f = a.Qb[c]), a !== f && -1 === b.indexOf(f) && b.push(f);
  }
  function a(a, b) {
    this.gc = a;
    this.sheet = a.sheet;
    this.b = a.b;
    this.Q = [];
    this.Fa = [];
    this.M = {};
    this.index = -1;
    this.lg = !1;
    this.gb = this.b.Nd(b[1]);
    this.trigger = 0 < b[3];
    this.Jm = 2 === b[3];
    this.Dk = b[5];
    this.Ps = b[6];
    this.V = b[7];
    this.b.Vd[this.V.toString()] = this;
    -1 === b[0]
      ? ((this.type = null),
        (this.Ma = this.yl),
        (this.Ae = null),
        (this.Fc = -1))
      : ((this.type = this.b.C[b[0]]),
        (this.Ma = this.Ps ? this.wt : this.xl),
        b[2]
          ? ((this.Ae = this.type.pi(b[2])), (this.Fc = this.type.ok(b[2])))
          : ((this.Ae = null), (this.Fc = -1)),
        this.gc.parent && this.gc.parent.lj());
    this.Jm && (this.Ma = this.xt);
    if (10 === b.length) {
      var c,
        e,
        f = b[9];
      c = 0;
      for (e = f.length; c < e; c++) {
        var d = new Mb(this, f[c]);
        this.Q.push(d);
      }
      this.Fa.length = f.length;
    }
  }
  function f(a, b) {
    this.gc = a;
    this.sheet = a.sheet;
    this.b = a.b;
    this.Q = [];
    this.Fa = [];
    this.M = {};
    this.index = -1;
    this.lg = !1;
    this.gb = this.b.Nd(b[1]);
    -1 === b[0]
      ? ((this.type = null),
        (this.Ma = this.yl),
        (this.Ae = null),
        (this.Fc = -1))
      : ((this.type = this.b.C[b[0]]),
        (this.Ma = this.xl),
        b[2]
          ? ((this.Ae = this.type.pi(b[2])), (this.Fc = this.type.ok(b[2])))
          : ((this.Ae = null), (this.Fc = -1)));
    this.V = b[3];
    this.b.Pd[this.V.toString()] = this;
    if (6 === b.length) {
      var c,
        e,
        f = b[5];
      c = 0;
      for (e = f.length; c < e; c++) {
        var d = new Mb(this, f[c]);
        this.Q.push(d);
      }
      this.Fa.length = f.length;
    }
  }
  function r() {
    z++;
    A.length === z && A.push(new Nb());
    return A[z];
  }
  function e(a, b) {
    this.uc = a;
    this.gc = a.gc;
    this.sheet = a.sheet;
    this.b = a.b;
    this.type = b[0];
    this.ad = null;
    this.Fd = 0;
    this.get = null;
    this.Am = 0;
    this.Ra = null;
    this.key = 0;
    this.object = null;
    this.index = 0;
    this.Ih = this.kf = this.Ih = this.kf = this.Mm = this.Ke = this.Jh = null;
    this.Ac = !1;
    var c, e, f;
    switch (b[0]) {
      case 0:
      case 7:
        this.ad = new Ob(this, b[1]);
        this.Fd = 0;
        this.get = this.ks;
        break;
      case 1:
        this.ad = new Ob(this, b[1]);
        this.Fd = 0;
        this.get = this.ls;
        break;
      case 5:
        this.ad = new Ob(this, b[1]);
        this.Fd = 0;
        this.get = this.rs;
        break;
      case 3:
      case 8:
        this.Am = b[1];
        this.get = this.gs;
        break;
      case 6:
        this.Ra = this.b.Tk[b[1]];
        this.get = this.ts;
        break;
      case 9:
        this.key = b[1];
        this.get = this.qs;
        break;
      case 4:
        this.object = this.b.C[b[1]];
        this.get = this.us;
        this.gc.om(this.object);
        this.uc instanceof Lb
          ? this.gc.lj()
          : this.gc.parent && this.gc.parent.lj();
        break;
      case 10:
        this.index = b[1];
        a.type && a.type.K
          ? ((this.get = this.ns), (this.Ac = !0))
          : (this.get = this.os);
        break;
      case 11:
        this.Jh = b[1];
        this.Ke = null;
        this.get = this.hs;
        break;
      case 2:
      case 12:
        this.Mm = b[1];
        this.get = this.es;
        break;
      case 13:
        for (
          this.get = this.vs, this.kf = [], this.Ih = [], c = 1, e = b.length;
          c < e;
          c++
        )
          (f = new Mb(this.uc, b[c])), this.kf.push(f), this.Ih.push(0);
    }
  }
  function b(a, b, c) {
    this.sheet = a;
    this.parent = b;
    this.b = a.b;
    this.Ca = [];
    this.name = c[1];
    this.Ej = c[2];
    this.zk = c[3];
    this.Tg = !!c[4];
    this.yi = !!c[5];
    this.V = c[6];
    this.b.hg[this.V.toString()] = this;
    this.data = this.zk;
    this.parent
      ? ((this.Ye = this.Tg || this.yi ? -1 : this.b.Et++),
        this.b.kq.push(this))
      : ((this.Ye = -1), this.b.jq.push(this));
  }
  function p(a, b, c) {
    this.sheet = a;
    this.parent = b;
    this.b = a.b;
    this.Ca = [];
    this.Mg = null;
    this.As = c[1];
    this.mb = !0;
  }
  function l() {
    this.To = [];
    this.reset(null);
  }
  var x = [];
  h.prototype.toString = function () {
    return this.name;
  };
  h.prototype.dn = function (a, b, c) {
    switch (a[0]) {
      case 0:
        a = new Pb(this, b, a);
        if (a.Nc)
          for (c.push(a), c = 0, b = a.$a.length; c < b; c++)
            a.$a[c].trigger && this.gn(a, c);
        else a.rn() ? this.gn(a, 0) : c.push(a);
        break;
      case 1:
        a = new Gb(this, b, a);
        c.push(a);
        break;
      case 2:
        (a = new Qb(this, b, a)), c.push(a);
    }
  };
  h.prototype.Ia = function () {
    var a, b;
    a = 0;
    for (b = this.$d.length; a < b; a++)
      this.$d[a].Ia(a < b - 1 && this.$d[a + 1].Ai);
  };
  h.prototype.Ul = function () {
    V(this.ck);
    V(this.Lj);
    this.nm(this);
    V(this.Lj);
  };
  h.prototype.nm = function (a) {
    var b,
      c,
      e,
      f,
      d = a.ck,
      l = a.Lj,
      h = this.bn.Jd();
    b = 0;
    for (c = h.length; b < c; ++b)
      (e = h[b]),
        (f = e.Mg),
        !e.mb ||
          a === f ||
          -1 < l.indexOf(f) ||
          (l.push(f), f.nm(a), d.push(f));
  };
  h.prototype.Ma = function (a) {
    this.b.cv || ((this.wk = !0), a || (this.b.Kk = !0));
    var b, c;
    b = 0;
    for (c = this.$d.length; b < c; b++) {
      var e = this.$d[b];
      e.Ma();
      this.b.Xj(e.Ca);
      this.b.be && this.b.lb();
    }
    a || (this.b.Kk = !1);
  };
  h.prototype.gn = function (a, b) {
    a.Nc || this.b.Aj.push(a);
    var c,
      e,
      f = a.$a[b],
      d;
    f.type ? (d = f.type.name) : (d = "system");
    var l = (c = f.Jm) ? this.Km : this.$o;
    l[d] || (l[d] = []);
    d = l[d];
    l = f.gb;
    if (c) {
      if (f.Q.length && ((f = f.Q[0]), 1 === f.type && 2 === f.ad.type)) {
        f = f.ad.value.toLowerCase();
        c = 0;
        for (e = d.length; c < e; c++)
          if (d[c].method == l) {
            c = d[c].Ag;
            c[f] ? c[f].push([a, b]) : (c[f] = [[a, b]]);
            return;
          }
        c = {};
        c[f] = [[a, b]];
        d.push({ method: l, Ag: c });
      }
    } else {
      c = 0;
      for (e = d.length; c < e; c++)
        if (d[c].method == l) {
          d[c].Ag.push([a, b]);
          return;
        }
      Z && l === Z.prototype.q.sf
        ? d.unshift({ method: l, Ag: [[a, b]] })
        : d.push({ method: l, Ag: [[a, b]] });
    }
  };
  wb = h;
  n.prototype.Ym = function () {
    return this.na ? this.type.j.length : this.j.length;
  };
  n.prototype.kc = function () {
    return this.na ? this.type.j : this.j;
  };
  n.prototype.qh = function (a) {
    a &&
      (a.b.tb().ic.Nc
        ? (this.na && (V(this.j), ya(this.Y, a.type.j), (this.na = !1)),
          (a = this.Y.indexOf(a)),
          -1 !== a && (this.j.push(this.Y[a]), this.Y.splice(a, 1)))
        : ((this.na = !1), V(this.j), (this.j[0] = a)));
  };
  fb = n;
  window._c2hh_ = "";
  t.prototype.Ia = function (a) {
    var b,
      c = this.parent;
    if (this.group)
      for (this.yj = !0; c; ) {
        if (!c.group) {
          this.yj = !1;
          break;
        }
        c = c.parent;
      }
    this.Nl =
      !this.rn() && (!this.parent || (this.parent.group && this.parent.yj));
    this.$m = !!a;
    this.ve = this.Ca.slice(0);
    for (c = this.parent; c; ) {
      a = 0;
      for (b = c.Ca.length; a < b; a++) this.hq(c.Ca[a]);
      c = c.parent;
    }
    this.Ca = q(this.Ca);
    this.ve = q(this.ve);
    a = 0;
    for (b = this.$a.length; a < b; a++) this.$a[a].Ia();
    a = 0;
    for (b = this.rc.length; a < b; a++) this.rc[a].Ia();
    a = 0;
    for (b = this.yc.length; a < b; a++)
      this.yc[a].Ia(a < b - 1 && this.yc[a + 1].Ai);
  };
  t.prototype.At = function (a) {
    if (this.Jg !== !!a) {
      this.Jg = !!a;
      var b;
      a = 0;
      for (b = this.$h.length; a < b; ++a) this.$h[a].ep();
      0 < b && this.b.ja.Je && this.b.ja.Je.Ul();
    }
  };
  t.prototype.om = function (a) {
    c(a, this.Ca);
  };
  t.prototype.hq = function (a) {
    c(a, this.ve);
  };
  t.prototype.lj = function () {
    this.Qo = !0;
    this.parent && this.parent.lj();
  };
  t.prototype.rn = function () {
    return this.$a.length ? this.$a[0].trigger : !1;
  };
  t.prototype.Ma = function () {
    var a,
      b,
      c = !1,
      e = this.b,
      f = this.b.tb();
    f.ic = this;
    var d = this.$a;
    this.Ai || (f.dk = !1);
    if (this.Nc) {
      0 === d.length && (c = !0);
      f.Za = 0;
      for (a = d.length; f.Za < a; f.Za++)
        (b = d[f.Za]), b.trigger || ((b = b.Ma()) && (c = !0));
      (f.Ue = c) && this.ij();
    } else {
      f.Za = 0;
      for (a = d.length; f.Za < a; f.Za++)
        if (((b = d[f.Za].Ma()), !b)) {
          f.Ue = !1;
          this.Nl && e.be && e.lb();
          return;
        }
      f.Ue = !0;
      this.ij();
    }
    this.cr(f);
  };
  t.prototype.cr = function (a) {
    a.Ue && this.$m && (a.dk = !0);
    this.Nl && this.b.be && this.b.lb();
  };
  t.prototype.tt = function (a) {
    this.b.tb().ic = this;
    this.$a[a].Ma() && (this.ij(), (this.b.tb().Ue = !0));
  };
  t.prototype.ij = function () {
    var a = this.b.tb(),
      b;
    a.fc = 0;
    for (b = this.rc.length; a.fc < b; a.fc++) if (this.rc[a.fc].Ma()) return;
    this.Bo();
  };
  t.prototype.ot = function () {
    var a = this.b.tb(),
      b;
    for (b = this.rc.length; a.fc < b; a.fc++) if (this.rc[a.fc].Ma()) return;
    this.Bo();
  };
  t.prototype.Bo = function () {
    if (this.yc.length) {
      var a,
        b,
        c,
        e,
        f = this.yc.length - 1;
      this.b.bj(this);
      if (this.Qo)
        for (a = 0, b = this.yc.length; a < b; a++)
          (c = this.yc[a]),
            (e = !this.yj || (!this.group && a < f)) && this.b.aj(c.Ca),
            c.Ma(),
            e ? this.b.df(c.Ca) : this.b.Xj(c.Ca);
      else for (a = 0, b = this.yc.length; a < b; a++) this.yc[a].Ma();
      this.b.Zi();
    }
  };
  t.prototype.ut = function () {
    var a = this.b.tb();
    a.ic = this;
    var b = !1,
      c;
    a.Za = 0;
    for (c = this.$a.length; a.Za < c; a.Za++)
      if (this.$a[a.Za].Ma()) b = !0;
      else if (!this.Nc) return !1;
    return this.Nc ? b : !0;
  };
  t.prototype.pt = function () {
    this.b.Le++;
    var a = this.b.tb().Za,
      b = this.b.bj(this);
    if (!this.Nc)
      for (b.Za = a + 1, a = this.$a.length; b.Za < a; b.Za++)
        if (!this.$a[b.Za].Ma()) {
          this.b.Zi();
          return;
        }
    this.ij();
    this.b.Zi();
  };
  t.prototype.Ls = function (a) {
    var b = a.index;
    if (0 === b) return !0;
    for (--b; 0 <= b; --b) if (this.$a[b].type === a.type) return !1;
    return !0;
  };
  Pb = t;
  a.prototype.Ia = function () {
    var a, b, c;
    a = 0;
    for (b = this.Q.length; a < b; a++)
      (c = this.Q[a]), c.Ia(), c.Ac && (this.lg = !0);
  };
  a.prototype.xt = function () {
    return !0;
  };
  a.prototype.yl = function () {
    var a, b;
    a = 0;
    for (b = this.Q.length; a < b; a++) this.Fa[a] = this.Q[a].get();
    return Ma(this.gb.apply(this.b.jd, this.Fa), this.Dk);
  };
  a.prototype.wt = function () {
    var a, b;
    a = 0;
    for (b = this.Q.length; a < b; a++) this.Fa[a] = this.Q[a].get();
    a = this.gb.apply(this.Ae ? this.Ae : this.type, this.Fa);
    this.type.Zc();
    return a;
  };
  a.prototype.xl = function () {
    var a,
      b,
      c,
      e,
      f,
      d,
      l,
      h,
      n = this.type,
      m = n.ba(),
      p = this.gc.Nc && !this.trigger;
    b = 0;
    var r = n.Jb,
      q = n.K,
      t = n.sd,
      x = this.Fc,
      A = -1 < x,
      z = this.lg,
      J = this.Q,
      v = this.Fa,
      y = this.Dk,
      B = this.gb,
      H;
    if (z)
      for (b = 0, f = J.length; b < f; ++b)
        (d = J[b]), d.Ac || (v[b] = d.get(0));
    else for (b = 0, f = J.length; b < f; ++b) v[b] = J[b].get(0);
    if (m.na) {
      V(m.j);
      V(m.Y);
      H = n.j;
      a = 0;
      for (e = H.length; a < e; ++a) {
        h = H[a];
        if (z)
          for (b = 0, f = J.length; b < f; ++b)
            (d = J[b]), d.Ac && (v[b] = d.get(a));
        A
          ? ((b = 0), q && (b = h.type.Af[t]), (b = B.apply(h.L[x + b], v)))
          : (b = B.apply(h, v));
        (l = Ma(b, y)) ? m.j.push(h) : p && m.Y.push(h);
      }
      n.finish && n.finish(!0);
      m.na = !1;
      n.Zc();
      return m.Ym();
    }
    c = 0;
    H = (l = p && !this.gc.Ls(this)) ? m.Y : m.j;
    var D = !1;
    a = 0;
    for (e = H.length; a < e; ++a) {
      h = H[a];
      if (z)
        for (b = 0, f = J.length; b < f; ++b)
          (d = J[b]), d.Ac && (v[b] = d.get(a));
      A
        ? ((b = 0), q && (b = h.type.Af[t]), (b = B.apply(h.L[x + b], v)))
        : (b = B.apply(h, v));
      if (Ma(b, y))
        if (((D = !0), l)) {
          if ((m.j.push(h), r))
            for (b = 0, f = h.siblings.length; b < f; b++)
              (d = h.siblings[b]), d.type.ba().j.push(d);
        } else {
          H[c] = h;
          if (r)
            for (b = 0, f = h.siblings.length; b < f; b++)
              (d = h.siblings[b]), (d.type.ba().j[c] = d);
          c++;
        }
      else if (l) {
        H[c] = h;
        if (r)
          for (b = 0, f = h.siblings.length; b < f; b++)
            (d = h.siblings[b]), (d.type.ba().Y[c] = d);
        c++;
      } else if (p && (m.Y.push(h), r))
        for (b = 0, f = h.siblings.length; b < f; b++)
          (d = h.siblings[b]), d.type.ba().Y.push(d);
    }
    xa(H, c);
    if (r)
      for (q = n.Qb, a = 0, e = q.length; a < e; a++)
        (h = q[a].ba()), l ? xa(h.Y, c) : xa(h.j, c);
    c = D;
    if (l && !D)
      for (a = 0, e = m.j.length; a < e; a++) {
        h = m.j[a];
        if (z)
          for (b = 0, f = J.length; b < f; b++)
            (d = J[b]), d.Ac && (v[b] = d.get(a));
        b = A ? B.apply(h.L[x], v) : B.apply(h, v);
        if (Ma(b, y)) {
          D = !0;
          break;
        }
      }
    n.finish && n.finish(c || p);
    return p ? D : m.Ym();
  };
  Kb = a;
  f.prototype.Ia = function () {
    var a, b, c;
    a = 0;
    for (b = this.Q.length; a < b; a++)
      (c = this.Q[a]), c.Ia(), c.Ac && (this.lg = !0);
  };
  f.prototype.yl = function () {
    var a = this.b,
      b,
      c,
      e = this.Q,
      f = this.Fa;
    b = 0;
    for (c = e.length; b < c; ++b) f[b] = e[b].get();
    return this.gb.apply(a.jd, f);
  };
  f.prototype.xl = function () {
    var a = this.type,
      b = this.Fc,
      c = a.sd,
      e = this.lg,
      f = this.Q,
      d = this.Fa,
      l = this.gb,
      h = a.ba().kc(),
      a = a.K,
      n = -1 < b,
      m,
      p,
      r,
      q,
      t,
      x;
    if (e)
      for (p = 0, q = f.length; p < q; ++p)
        (t = f[p]), t.Ac || (d[p] = t.get(0));
    else for (p = 0, q = f.length; p < q; ++p) d[p] = f[p].get(0);
    m = 0;
    for (r = h.length; m < r; ++m) {
      x = h[m];
      if (e)
        for (p = 0, q = f.length; p < q; ++p)
          (t = f[p]), t.Ac && (d[p] = t.get(m));
      n
        ? ((p = 0), a && (p = x.type.Af[c]), l.apply(x.L[b + p], d))
        : l.apply(x, d);
    }
    return !1;
  };
  Lb = f;
  var A = [],
    z = -1;
  e.prototype.Ia = function () {
    var a, b;
    if (11 === this.type) this.Ke = this.b.Tm(this.Jh, this.gc.parent);
    else if (13 === this.type)
      for (a = 0, b = this.kf.length; a < b; a++) this.kf[a].Ia();
    this.ad && this.ad.Ia();
  };
  e.prototype.Ts = function (a) {
    this.Ac || !a || a.Ba.pj || (this.Ac = !0);
  };
  e.prototype.Io = function () {
    this.Ac = !0;
  };
  e.prototype.ks = function (a) {
    this.Fd = a || 0;
    a = r();
    this.ad.get(a);
    z--;
    return a.data;
  };
  e.prototype.ls = function (a) {
    this.Fd = a || 0;
    a = r();
    this.ad.get(a);
    z--;
    return L(a.data) ? a.data : "";
  };
  e.prototype.us = function () {
    return this.object;
  };
  e.prototype.gs = function () {
    return this.Am;
  };
  e.prototype.rs = function (a) {
    this.Fd = a || 0;
    a = r();
    this.ad.get(a);
    z--;
    return a.ab() ? this.b.Yr(a.data) : this.b.Xr(a.data);
  };
  e.prototype.ts = function () {
    return this.Ra;
  };
  e.prototype.qs = function () {
    return this.key;
  };
  e.prototype.os = function () {
    return this.index;
  };
  e.prototype.ns = function (a) {
    a = a || 0;
    var b = this.uc.type,
      c = null,
      c = b.ba(),
      e = c.kc();
    if (e.length) c = e[a % e.length].type;
    else if (c.Y.length) c = c.Y[a % c.Y.length].type;
    else if (b.j.length) c = b.j[a % b.j.length].type;
    else return 0;
    return this.index + c.hi[b.sd];
  };
  e.prototype.hs = function () {
    return this.Ke;
  };
  e.prototype.es = function () {
    return this.Mm;
  };
  e.prototype.vs = function () {
    var a, b;
    a = 0;
    for (b = this.kf.length; a < b; a++) this.Ih[a] = this.kf[a].get();
    return this.Ih;
  };
  Mb = e;
  b.prototype.Ia = function () {
    this.Ca = q(this.Ca);
  };
  b.prototype.hf = function (a) {
    var b = this.b.Sm();
    this.parent && !this.Tg && b
      ? (this.Ye >= b.length && (b.length = this.Ye + 1), (b[this.Ye] = a))
      : (this.data = a);
  };
  b.prototype.Hg = function () {
    var a = this.b.Sm();
    return !this.parent || this.Tg || !a || this.yi
      ? this.data
      : this.Ye >= a.length || "undefined" === typeof a[this.Ye]
      ? this.zk
      : a[this.Ye];
  };
  b.prototype.Ma = function () {
    !this.parent || this.Tg || this.yi || this.hf(this.zk);
  };
  Gb = b;
  p.prototype.toString = function () {
    return "include:" + this.Mg.toString();
  };
  p.prototype.Ia = function () {
    this.Mg = this.b.gk[this.As];
    this.sheet.bn.add(this);
    this.Ca = q(this.Ca);
    for (var a = this.parent; a; ) a.group && a.$h.push(this), (a = a.parent);
    this.ep();
  };
  p.prototype.Ma = function () {
    this.parent && this.b.vh(this.b.C);
    this.Mg.wk || this.Mg.Ma(!0);
    this.parent && this.b.df(this.b.C);
  };
  p.prototype.ep = function () {
    for (var a = this.parent; a; ) {
      if (a.group && !a.Jg) {
        this.mb = !1;
        return;
      }
      a = a.parent;
    }
    this.mb = !0;
  };
  Qb = p;
  l.prototype.reset = function (a) {
    this.ic = a;
    this.fc = this.Za = 0;
    V(this.To);
    this.dk = this.Ue = !1;
  };
  Fb = l;
})();
(function () {
  function d(c, a) {
    this.uc = c;
    this.b = c.b;
    this.type = a[0];
    this.get = [
      this.ur,
      this.qr,
      this.Gr,
      this.Jr,
      this.fr,
      this.Hr,
      this.Br,
      this.nr,
      this.Ar,
      this.Fr,
      this.gr,
      this.Er,
      this.or,
      this.Cr,
      this.xr,
      this.yr,
      this.rr,
      this.sr,
      this.mr,
      this.Ir,
      this.Dr,
      this.tr,
      this.lr,
      this.pr,
    ][this.type];
    var f = null;
    this.xd =
      this.Q =
      this.Fa =
      this.gb =
      this.vj =
      this.second =
      this.first =
      this.value =
        null;
    this.Fc = -1;
    this.Ic = null;
    this.lp = -1;
    this.Ke = this.Jh = null;
    this.Xf = !1;
    switch (this.type) {
      case 0:
      case 1:
      case 2:
        this.value = a[1];
        break;
      case 3:
        this.first = new Ob(c, a[1]);
        break;
      case 18:
        this.first = new Ob(c, a[1]);
        this.second = new Ob(c, a[2]);
        this.vj = new Ob(c, a[3]);
        break;
      case 19:
        this.gb = this.b.Nd(a[1]);
        (this.gb !== Y.prototype.Fb.random && this.gb !== Y.prototype.Fb.vq) ||
          this.uc.Io();
        this.Fa = [];
        this.Q = [];
        3 === a.length
          ? ((f = a[2]), (this.Fa.length = f.length + 1))
          : (this.Fa.length = 1);
        break;
      case 20:
        this.xd = this.b.C[a[1]];
        this.Fc = -1;
        this.gb = this.b.Nd(a[2]);
        this.Xf = a[3];
        Tb.Function && this.gb === Tb.Function.prototype.Fb.Yt && this.uc.Io();
        a[4] ? (this.Ic = new Ob(c, a[4])) : (this.Ic = null);
        this.Fa = [];
        this.Q = [];
        6 === a.length
          ? ((f = a[5]), (this.Fa.length = f.length + 1))
          : (this.Fa.length = 1);
        break;
      case 21:
        this.xd = this.b.C[a[1]];
        this.Xf = a[2];
        a[3] ? (this.Ic = new Ob(c, a[3])) : (this.Ic = null);
        this.lp = a[4];
        break;
      case 22:
        this.xd = this.b.C[a[1]];
        this.xd.pi(a[2]);
        this.Fc = this.xd.ok(a[2]);
        this.gb = this.b.Nd(a[3]);
        this.Xf = a[4];
        a[5] ? (this.Ic = new Ob(c, a[5])) : (this.Ic = null);
        this.Fa = [];
        this.Q = [];
        7 === a.length
          ? ((f = a[6]), (this.Fa.length = f.length + 1))
          : (this.Fa.length = 1);
        break;
      case 23:
        (this.Jh = a[1]), (this.Ke = null);
    }
    this.uc.Ts(this.xd);
    4 <= this.type &&
      17 >= this.type &&
      ((this.first = new Ob(c, a[1])), (this.second = new Ob(c, a[2])));
    if (f) {
      var d, e;
      d = 0;
      for (e = f.length; d < e; d++) this.Q.push(new Ob(c, f[d]));
    }
  }
  function m() {
    ++t;
    n.length === t && n.push(new Nb());
    return n[t];
  }
  function q(c, a, f) {
    var d, e;
    d = 0;
    for (e = c.length; d < e; ++d) c[d].get(f), (a[d + 1] = f.data);
  }
  function h(c, a) {
    this.type = c || Rb.rf;
    this.data = a || 0;
    this.bf = null;
    this.type == Rb.rf && (this.data = Math.floor(this.data));
  }
  d.prototype.Ia = function () {
    23 === this.type && (this.Ke = this.uc.b.Tm(this.Jh, this.uc.gc.parent));
    this.first && this.first.Ia();
    this.second && this.second.Ia();
    this.vj && this.vj.Ia();
    this.Ic && this.Ic.Ia();
    if (this.Q) {
      var c, a;
      c = 0;
      for (a = this.Q.length; c < a; c++) this.Q[c].Ia();
    }
  };
  var n = [],
    t = -1;
  d.prototype.Ir = function (c) {
    var a = this.Q,
      f = this.Fa;
    f[0] = c;
    c = m();
    q(a, f, c);
    --t;
    this.gb.apply(this.b.jd, f);
  };
  d.prototype.Dr = function (c) {
    var a = this.xd,
      f = this.Fa,
      d = this.Q,
      e = this.Ic,
      b = this.gb,
      h = this.uc.Fd,
      l = a.ba(),
      n = l.kc();
    if (!n.length)
      if (l.Y.length) n = l.Y;
      else {
        this.Xf ? c.ac("") : c.oa(0);
        return;
      }
    f[0] = c;
    c.bf = a;
    c = m();
    q(d, f, c);
    e && (e.get(c), c.ab() && ((h = c.data), (n = a.j)));
    --t;
    a = n.length;
    if (h >= a || h <= -a) h %= a;
    0 > h && (h += a);
    b.apply(n[h], f);
  };
  d.prototype.lr = function (c) {
    var a = this.xd,
      f = this.Fa,
      d = this.Q,
      e = this.Ic,
      b = this.Fc,
      h = this.gb,
      l = this.uc.Fd,
      n = a.ba(),
      A = n.kc();
    if (!A.length)
      if (n.Y.length) A = n.Y;
      else {
        this.Xf ? c.ac("") : c.oa(0);
        return;
      }
    f[0] = c;
    c.bf = a;
    c = m();
    q(d, f, c);
    e && (e.get(c), c.ab() && ((l = c.data), (A = a.j)));
    --t;
    d = A.length;
    if (l >= d || l <= -d) l %= d;
    0 > l && (l += d);
    l = A[l];
    A = 0;
    a.K && (A = l.type.Af[a.sd]);
    h.apply(l.L[b + A], f);
  };
  d.prototype.tr = function (c) {
    var a = this.Ic,
      f = this.xd,
      d = this.lp,
      e = this.uc.Fd,
      b = f.ba(),
      h = b.kc();
    if (!h.length)
      if (b.Y.length) h = b.Y;
      else {
        this.Xf ? c.ac("") : c.oa(0);
        return;
      }
    if (a) {
      b = m();
      a.get(b);
      if (b.ab()) {
        e = b.data;
        h = f.j;
        0 !== h.length && ((e %= h.length), 0 > e && (e += h.length));
        e = f.rk(e);
        f = e.Pa[d];
        L(f) ? c.ac(f) : c.S(f);
        --t;
        return;
      }
      --t;
    }
    a = h.length;
    if (e >= a || e <= -a) e %= a;
    0 > e && (e += a);
    e = h[e];
    h = 0;
    f.K && (h = e.type.hi[f.sd]);
    f = e.Pa[d + h];
    L(f) ? c.ac(f) : c.S(f);
  };
  d.prototype.ur = function (c) {
    c.type = Rb.rf;
    c.data = this.value;
  };
  d.prototype.qr = function (c) {
    c.type = Rb.qf;
    c.data = this.value;
  };
  d.prototype.Gr = function (c) {
    c.type = Rb.String;
    c.data = this.value;
  };
  d.prototype.Jr = function (c) {
    this.first.get(c);
    c.ab() && (c.data = -c.data);
  };
  d.prototype.fr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.ab() && a.ab() && ((c.data += a.data), a.Hf() && c.Qf());
    --t;
  };
  d.prototype.Hr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.ab() && a.ab() && ((c.data -= a.data), a.Hf() && c.Qf());
    --t;
  };
  d.prototype.Br = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.ab() && a.ab() && ((c.data *= a.data), a.Hf() && c.Qf());
    --t;
  };
  d.prototype.nr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.ab() && a.ab() && ((c.data /= a.data), c.Qf());
    --t;
  };
  d.prototype.Ar = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.ab() && a.ab() && ((c.data %= a.data), a.Hf() && c.Qf());
    --t;
  };
  d.prototype.Fr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.ab() && a.ab() && ((c.data = Math.pow(c.data, a.data)), a.Hf() && c.Qf());
    --t;
  };
  d.prototype.gr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    a.Jf() || c.Jf() ? this.ir(c, a) : this.hr(c, a);
    --t;
  };
  d.prototype.ir = function (c, a) {
    c.Jf() && a.Jf() ? this.kr(c, a) : this.jr(c, a);
  };
  d.prototype.kr = function (c, a) {
    c.data += a.data;
  };
  d.prototype.jr = function (c, a) {
    c.Jf()
      ? (c.data += (Math.round(1e10 * a.data) / 1e10).toString())
      : c.ac(c.data.toString() + a.data);
  };
  d.prototype.hr = function (c, a) {
    c.oa(c.data && a.data ? 1 : 0);
  };
  d.prototype.Er = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.ab() && a.ab() && (c.data || a.data ? c.oa(1) : c.oa(0));
    --t;
  };
  d.prototype.mr = function (c) {
    this.first.get(c);
    c.data ? this.second.get(c) : this.vj.get(c);
  };
  d.prototype.or = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.oa(c.data === a.data ? 1 : 0);
    --t;
  };
  d.prototype.Cr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.oa(c.data !== a.data ? 1 : 0);
    --t;
  };
  d.prototype.xr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.oa(c.data < a.data ? 1 : 0);
    --t;
  };
  d.prototype.yr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.oa(c.data <= a.data ? 1 : 0);
    --t;
  };
  d.prototype.rr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.oa(c.data > a.data ? 1 : 0);
    --t;
  };
  d.prototype.sr = function (c) {
    this.first.get(c);
    var a = m();
    this.second.get(a);
    c.oa(c.data >= a.data ? 1 : 0);
    --t;
  };
  d.prototype.pr = function (c) {
    var a = this.Ke.Hg();
    ga(a) ? c.S(a) : c.ac(a);
  };
  Ob = d;
  h.prototype.Hf = function () {
    return this.type === Rb.qf;
  };
  h.prototype.ab = function () {
    return this.type === Rb.rf || this.type === Rb.qf;
  };
  h.prototype.Jf = function () {
    return this.type === Rb.String;
  };
  h.prototype.Qf = function () {
    this.Hf() ||
      (this.Jf() && (this.data = parseFloat(this.data)), (this.type = Rb.qf));
  };
  h.prototype.oa = function (c) {
    this.type = Rb.rf;
    this.data = Math.floor(c);
  };
  h.prototype.S = function (c) {
    this.type = Rb.qf;
    this.data = c;
  };
  h.prototype.ac = function (c) {
    this.type = Rb.String;
    this.data = c;
  };
  h.prototype.Ct = function (c) {
    ga(c)
      ? ((this.type = Rb.qf), (this.data = c))
      : L(c)
      ? ((this.type = Rb.String), (this.data = c.toString()))
      : ((this.type = Rb.rf), (this.data = 0));
  };
  Nb = h;
  Rb = { rf: 0, qf: 1, String: 2 };
})();
function Y(d) {
  this.b = d;
  this.ec = [];
}
Y.prototype.kb = function () {
  var d = {},
    m,
    q,
    h,
    n,
    t,
    c,
    a,
    f;
  d.waits = [];
  var r = d.waits,
    e;
  m = 0;
  for (q = this.ec.length; m < q; m++) {
    c = this.ec[m];
    e = { t: c.time, st: c.Dt, s: c.No, ev: c.zg.V, sm: [], sols: {} };
    c.zg.rc[c.fc] && (e.act = c.zg.rc[c.fc].V);
    h = 0;
    for (n = c.Ca.length; h < n; h++) e.sm.push(c.Ca[h].V);
    for (t in c.cc)
      if (c.cc.hasOwnProperty(t)) {
        a = this.b.C[parseInt(t, 10)];
        f = { sa: c.cc[t].Co, insts: [] };
        h = 0;
        for (n = c.cc[t].Pe.length; h < n; h++) f.insts.push(c.cc[t].Pe[h].uid);
        e.sols[a.V.toString()] = f;
      }
    r.push(e);
  }
  return d;
};
Y.prototype.Ab = function (d) {
  d = d.waits;
  var m, q, h, n, t, c, a, f, r, e, b;
  V(this.ec);
  m = 0;
  for (q = d.length; m < q; m++)
    if (((c = d[m]), (f = this.b.um[c.ev.toString()]))) {
      r = -1;
      h = 0;
      for (n = f.rc.length; h < n; h++)
        if (f.rc[h].V === c.act) {
          r = h;
          break;
        }
      if (-1 !== r) {
        a = { cc: {}, Ca: [], Fm: !1 };
        a.time = c.t;
        a.Dt = c.st || "";
        a.No = !!c.s;
        a.zg = f;
        a.fc = r;
        h = 0;
        for (n = c.sm.length; h < n; h++)
          (f = this.b.ri(c.sm[h])) && a.Ca.push(f);
        for (t in c.sols)
          if (c.sols.hasOwnProperty(t) && (f = this.b.ri(parseInt(t, 10)))) {
            r = c.sols[t];
            e = { Co: r.sa, Pe: [] };
            h = 0;
            for (n = r.insts.length; h < n; h++)
              (b = this.b.Fg(r.insts[h])) && e.Pe.push(b);
            a.cc[f.index.toString()] = e;
          }
        this.ec.push(a);
      }
    }
};
(function () {
  function d() {}
  function m() {}
  function q() {}
  var h = Y.prototype;
  d.prototype.vp = function () {
    return !0;
  };
  d.prototype.im = function () {
    return !0;
  };
  d.prototype.Np = function () {
    return !0;
  };
  d.prototype.tp = function (c, a, f) {
    return Sb(c.Hg(), a, f);
  };
  d.prototype.up = function () {
    var c = this.b.tb();
    return c.dk ? !1 : !c.Ue;
  };
  d.prototype.jm = function () {
    return !0;
  };
  d.prototype.Gp = function () {
    return !0;
  };
  d.prototype.Jj = function () {
    return !0;
  };
  d.prototype.km = function () {
    return !0;
  };
  d.prototype.Op = function () {
    return !0;
  };
  d.prototype.Oh = function () {
    return !0;
  };
  d.prototype.pp = function (c, a, f) {
    return Ia(W(c), W(f)) <= W(a);
  };
  d.prototype.zp = function (c, a) {
    return Ka(W(c), W(a));
  };
  d.prototype.xp = function (c, a, f) {
    c = Ga(c);
    a = Ga(a);
    f = Ga(f);
    return Ka(f, a) ? Ka(c, a) && !Ka(c, f) : !(!Ka(c, a) && Ka(c, f));
  };
  h.q = new d();
  m.prototype.wp = function (c) {
    this.b.Ug || this.b.rg || (this.b.rg = c);
  };
  m.prototype.Zp = function (c, a) {
    0 === c.Ej
      ? ga(a)
        ? c.hf(a)
        : c.hf(parseFloat(a))
      : 1 === c.Ej && c.hf(a.toString());
  };
  m.prototype.op = function (c, a) {
    0 === c.Ej
      ? ga(a)
        ? c.hf(c.Hg() + a)
        : c.hf(c.Hg() + parseFloat(a))
      : 1 === c.Ej && c.hf(c.Hg() + a.toString());
  };
  var n = [],
    t = [];
  h.R = new m();
  q.prototype["int"] = function (c, a) {
    L(a) ? (c.oa(parseInt(a, 10)), isNaN(c.data) && (c.data = 0)) : c.oa(a);
  };
  q.prototype["float"] = function (c, a) {
    L(a) ? (c.S(parseFloat(a)), isNaN(c.data) && (c.data = 0)) : c.S(a);
  };
  q.prototype.random = function (c, a, f) {
    void 0 === f ? c.S(Math.random() * a) : c.S(Math.random() * (f - a) + a);
  };
  q.prototype.sqrt = function (c, a) {
    c.S(Math.sqrt(a));
  };
  q.prototype.abs = function (c, a) {
    c.S(Math.abs(a));
  };
  q.prototype.round = function (c, a) {
    c.oa(Math.round(a));
  };
  q.prototype.floor = function (c, a) {
    c.oa(Math.floor(a));
  };
  q.prototype.ceil = function (c, a) {
    c.oa(Math.ceil(a));
  };
  q.prototype.sin = function (c, a) {
    c.S(Math.sin(W(a)));
  };
  q.prototype.cos = function (c, a) {
    c.S(Math.cos(W(a)));
  };
  q.prototype.tan = function (c, a) {
    c.S(Math.tan(W(a)));
  };
  q.prototype.asin = function (c, a) {
    c.S(Da(Math.asin(a)));
  };
  q.prototype.acos = function (c, a) {
    c.S(Da(Math.acos(a)));
  };
  q.prototype.atan = function (c, a) {
    c.S(Da(Math.atan(a)));
  };
  q.prototype.exp = function (c, a) {
    c.S(Math.exp(a));
  };
  q.prototype.log10 = function (c, a) {
    c.S(Math.log(a) / Math.LN10);
  };
  q.prototype.max = function (c) {
    var a = arguments[1];
    "number" !== typeof a && (a = 0);
    var f, d, e;
    f = 2;
    for (d = arguments.length; f < d; f++)
      (e = arguments[f]), "number" === typeof e && a < e && (a = e);
    c.S(a);
  };
  q.prototype.min = function (c) {
    var a = arguments[1];
    "number" !== typeof a && (a = 0);
    var f, d, e;
    f = 2;
    for (d = arguments.length; f < d; f++)
      (e = arguments[f]), "number" === typeof e && a > e && (a = e);
    c.S(a);
  };
  q.prototype.xf = function (c) {
    c.S(this.b.xf);
  };
  q.prototype.eg = function (c) {
    c.S(this.b.eg);
  };
  q.prototype.time = function (c) {
    c.S(this.b.Vb.wa);
  };
  q.prototype.kd = function (c) {
    c.oa(this.b.kd);
  };
  q.prototype.Vi = function (c) {
    c.oa(this.b.Vi);
  };
  q.prototype.lk = function (c) {
    c.oa(this.b.lk);
  };
  q.prototype.k = function (c, a, f, d, e) {
    c.S(Da(Ha(a, f, d, e)));
  };
  q.prototype.left = function (c, a, f) {
    c.ac(L(a) ? a.substr(0, f) : "");
  };
  q.prototype.right = function (c, a, f) {
    c.ac(L(a) ? a.substr(a.length - f) : "");
  };
  q.prototype.replace = function (c, a, f, d) {
    L(a) && L(f) && L(d)
      ? c.ac(a.replace(new RegExp(Ua(f), "gi"), d))
      : c.ac(L(a) ? a : "");
  };
  q.prototype.trim = function (c, a) {
    c.ac(L(a) ? a.trim() : "");
  };
  q.prototype.vq = function (c) {
    var a = O(Math.random() * (arguments.length - 1));
    c.Ct(arguments[a + 1]);
  };
  q.prototype.Zj = function (c) {
    c.S(this.b.Zj / 1e3);
  };
  q.prototype.Li = function (c) {
    c.S(this.b.Li);
  };
  h.Fb = new q();
  h.qt = function () {
    var c,
      a,
      f,
      d,
      e,
      b,
      h = this.b.tb();
    c = 0;
    for (f = this.ec.length; c < f; c++) {
      d = this.ec[c];
      if (-1 === d.time) {
        if (!d.No) continue;
      } else if (d.time > this.b.Vb.wa) continue;
      h.ic = d.zg;
      h.fc = d.fc;
      h.Za = 0;
      for (a in d.cc)
        d.cc.hasOwnProperty(a) &&
          ((e = this.b.C[parseInt(a, 10)].ba()),
          (b = d.cc[a]),
          (e.na = b.Co),
          ya(e.j, b.Pe),
          (e = b),
          V(e.Pe),
          t.push(e));
      d.zg.ot();
      this.b.Xj(d.Ca);
      d.Fm = !0;
    }
    a = c = 0;
    for (f = this.ec.length; c < f; c++)
      (d = this.ec[c]),
        (this.ec[a] = d),
        d.Fm ? (Oa(d.cc), V(d.Ca), n.push(d)) : a++;
    xa(this.ec, a);
  };
})();
(function () {
  eb = function (d, q) {
    var h = d[1],
      n = d[3],
      t = d[4],
      c = d[5],
      a = d[6],
      f = d[7],
      r = d[8];
    q.q || (q.q = {});
    q.R || (q.R = {});
    q.Fb || (q.Fb = {});
    var e = q.q,
      b = q.R,
      p = q.Fb;
    n &&
      ((e.cu = function (a, b) {
        return Sb(this.x, a, b);
      }),
      (e.du = function (a, b) {
        return Sb(this.y, a, b);
      }),
      (e.iu = function () {
        var a = this.n;
        this.qa();
        var b = this.xa;
        return !(
          b.right < a.da ||
          b.bottom < a.ea ||
          b.left > a.la ||
          b.top > a.ka
        );
      }),
      (e.Bp = function () {
        this.qa();
        var a = this.xa,
          b = this.b.ja;
        return (
          0 > a.right || 0 > a.bottom || a.left > b.width || a.top > b.height
        );
      }),
      (e.uu = function (a, b, c) {
        var e = this.ba(),
          f = e.kc();
        if (!f.length) return !1;
        var d = f[0],
          h = d,
          n = La(d.x, d.y, b, c),
          m,
          p,
          r;
        m = 1;
        for (p = f.length; m < p; m++)
          if (
            ((d = f[m]),
            (r = La(d.x, d.y, b, c)),
            (0 === a && r < n) || (1 === a && r > n))
          )
            (n = r), (h = d);
        e.qh(h);
        return !0;
      }),
      (b.aq = function (a) {
        this.x !== a && ((this.x = a), this.B());
      }),
      (b.Lu = function (a) {
        this.y !== a && ((this.y = a), this.B());
      }),
      (b.Iu = function (a, b) {
        if (this.x !== a || this.y !== b) (this.x = a), (this.y = b), this.B();
      }),
      (b.Xp = function (a, b) {
        var c = a.$r(this);
        if (c) {
          var e;
          c.Eg
            ? ((e = c.Eg(b, !0)), (c = c.Eg(b, !1)))
            : ((e = c.x), (c = c.y));
          if (this.x !== e || this.y !== c)
            (this.x = e), (this.y = c), this.B();
        }
      }),
      (b.nu = function (a) {
        0 !== a &&
          ((this.x += Math.cos(this.k) * a),
          (this.y += Math.sin(this.k) * a),
          this.B());
      }),
      (b.mu = function (a, b) {
        0 !== b &&
          ((this.x += Math.cos(W(a)) * b),
          (this.y += Math.sin(W(a)) * b),
          this.B());
      }),
      (p.Qu = function (a) {
        a.S(this.x);
      }),
      (p.Ru = function (a) {
        a.S(this.y);
      }),
      (p.xf = function (a) {
        a.S(this.b.Dg(this));
      }));
    t &&
      ((e.bu = function (a, b) {
        return Sb(this.width, a, b);
      }),
      (e.Zt = function (a, b) {
        return Sb(this.height, a, b);
      }),
      (b.$p = function (a) {
        this.width !== a && ((this.width = a), this.B());
      }),
      (b.Fu = function (a) {
        this.height !== a && ((this.height = a), this.B());
      }),
      (b.Ju = function (a, b) {
        if (this.width !== a || this.height !== b)
          (this.width = a), (this.height = b), this.B();
      }),
      (p.Pu = function (a) {
        a.S(this.width);
      }),
      (p.fu = function (a) {
        a.S(this.height);
      }),
      (p.Vt = function (a) {
        this.qa();
        a.S(this.xa.left);
      }),
      (p.Xt = function (a) {
        this.qa();
        a.S(this.xa.top);
      }),
      (p.Wt = function (a) {
        this.qa();
        a.S(this.xa.right);
      }),
      (p.Ut = function (a) {
        this.qa();
        a.S(this.xa.bottom);
      }));
    c &&
      ((e.pp = function (a, b) {
        return Ia(this.k, W(b)) <= W(a);
      }),
      (e.zp = function (a) {
        return Ka(this.k, W(a));
      }),
      (e.xp = function (a, b) {
        var c = Ga(a),
          e = Ga(b),
          f = X(this.k);
        return Ka(e, c) ? Ka(f, c) && !Ka(f, e) : !(!Ka(f, c) && Ka(f, e));
      }),
      (b.Cu = function (a) {
        a = W(Ea(a));
        isNaN(a) || this.k === a || ((this.k = a), this.B());
      }),
      (b.yu = function (a) {
        0 === a ||
          isNaN(a) ||
          ((this.k += W(a)), (this.k = X(this.k)), this.B());
      }),
      (b.zu = function (a) {
        0 === a ||
          isNaN(a) ||
          ((this.k -= W(a)), (this.k = X(this.k)), this.B());
      }),
      (b.Au = function (a, b) {
        var c = Ja(this.k, W(b), W(a));
        isNaN(c) || this.k === c || ((this.k = c), this.B());
      }),
      (b.Bu = function (a, b, c) {
        a = Ja(this.k, Math.atan2(c - this.y, b - this.x), W(a));
        isNaN(a) || this.k === a || ((this.k = a), this.B());
      }),
      (b.Ku = function (a, b) {
        var c = Math.atan2(b - this.y, a - this.x);
        isNaN(c) || this.k === c || ((this.k = c), this.B());
      }),
      (p.Tt = function (a) {
        a.S(Fa(this.k));
      }));
    h ||
      ((e.$t = function (a, b, c) {
        return Sb(this.Pa[a], b, c);
      }),
      (e.yp = function (a) {
        return this.Pa[a];
      }),
      (e.vu = function (a, b) {
        var c = this.ba(),
          e = c.kc();
        if (!e.length) return !1;
        var f = e[0],
          d = f,
          h = f.Pa[b],
          n,
          m,
          p;
        n = 1;
        for (m = e.length; n < m; n++)
          if (
            ((f = e[n]),
            (p = f.Pa[b]),
            (0 === a && p < h) || (1 === a && p > h))
          )
            (h = p), (d = f);
        c.qh(d);
        return !0;
      }),
      (e.tu = function (a) {
        var b, c, e, f, d;
        if (this.b.Rm().Dk) {
          d = this.ba();
          if (d.na)
            for (
              d.na = !1, V(d.j), V(d.Y), e = this.j, b = 0, c = e.length;
              b < c;
              b++
            )
              (f = e[b]), f.uid === a ? d.Y.push(f) : d.j.push(f);
          else {
            e = b = 0;
            for (c = d.j.length; b < c; b++)
              (f = d.j[b]), (d.j[e] = f), f.uid === a ? d.Y.push(f) : e++;
            xa(d.j, e);
          }
          this.Zc();
          return !!d.j.length;
        }
        f = this.b.Fg(a);
        if (!f) return !1;
        d = this.ba();
        if (!d.na && -1 === d.j.indexOf(f)) return !1;
        if (this.K)
          for (a = f.type.za, b = 0, c = a.length; b < c; b++) {
            if (a[b] === this) return d.qh(f), this.Zc(), !0;
          }
        else if (f.type === this) return d.qh(f), this.Zc(), !0;
        return !1;
      }),
      (e.Nh = function () {
        return !0;
      }),
      (e.Ip = function () {
        return !0;
      }),
      (b.Gu = function (a, b) {
        var c = this.Pa;
        ga(c[a])
          ? (c[a] = ga(b) ? b : parseFloat(b))
          : L(c[a]) && (c[a] = L(b) ? b : b.toString());
      }),
      (b.St = function (a, b) {
        var c = this.Pa;
        ga(c[a])
          ? (c[a] = ga(b) ? c[a] + b : c[a] + parseFloat(b))
          : L(c[a]) && (c[a] = L(b) ? c[a] + b : c[a] + b.toString());
      }),
      (b.Mu = function (a, b) {
        var c = this.Pa;
        ga(c[a]) && (c[a] = ga(b) ? c[a] - b : c[a] - parseFloat(b));
      }),
      (b.Up = function (a, b) {
        this.Pa[a] = b ? 1 : 0;
      }),
      (b.Nu = function (a) {
        this.Pa[a] = 1 - this.Pa[a];
      }),
      (b.cm = function () {
        this.b.pf(this);
      }),
      b.Dp ||
        (b.Dp = function (a) {
          var b, c;
          try {
            b = JSON.parse(a);
          } catch (e) {
            return;
          }
          this.b.Ji(this, b, !0);
          this.Xc && this.Xc();
          if (this.L)
            for (a = 0, b = this.L.length; a < b; ++a)
              (c = this.L[a]), c.Xc && c.Xc();
        }),
      (p.eu = function (a) {
        var b = a.bf.j.length,
          c,
          e,
          f;
        c = 0;
        for (e = this.b.Gc.length; c < e; c++)
          (f = this.b.Gc[c]),
            a.bf.K
              ? 0 <= f.type.za.indexOf(a.bf) && b++
              : f.type === a.bf && b++;
        a.oa(b);
      }),
      (p.xu = function (a) {
        a.oa(a.bf.ba().kc().length);
      }),
      (p.Ou = function (a) {
        a.oa(this.uid);
      }),
      (p.gu = function (a) {
        a.oa(this.Ig());
      }),
      p.qp ||
        (p.qp = function (a) {
          a.ac(JSON.stringify(this.b.zl(this, !0)));
        }));
    a &&
      ((e.ju = function () {
        return this.visible;
      }),
      (b.mm = function (a) {
        !a !== !this.visible && ((this.visible = !!a), (this.b.ca = !0));
      }),
      (e.au = function (a, b) {
        return Sb(ab(100 * this.opacity), a, b);
      }),
      (b.Hu = function (a) {
        a = a / 100;
        0 > a ? (a = 0) : 1 < a && (a = 1);
        a !== this.opacity && ((this.opacity = a), (this.b.ca = !0));
      }),
      (p.Opacity = function (a) {
        a.S(ab(100 * this.opacity));
      }));
    f &&
      ((e.hu = function (a) {
        return a ? this.n === a : !1;
      }),
      (e.wu = function (a) {
        var b = this.ba(),
          c = b.kc();
        if (!c.length) return !1;
        var e = c[0],
          f = e,
          d,
          h;
        d = 1;
        for (h = c.length; d < h; d++)
          if (((e = c[d]), 0 === a)) {
            if (
              e.n.index > f.n.index ||
              (e.n.index === f.n.index && e.bd() > f.bd())
            )
              f = e;
          } else if (
            e.n.index < f.n.index ||
            (e.n.index === f.n.index && e.bd() < f.bd())
          )
            f = e;
        b.qh(f);
        return !0;
      }),
      (b.qu = function () {
        var a = this.n,
          b = a.j;
        (b.length && b[b.length - 1] === this) ||
          (a.Wf(this, !1), a.ng(this, !1), (this.b.ca = !0));
      }),
      (b.ou = function () {
        var a = this.n,
          b = a.j;
        (b.length && b[0] === this) ||
          (a.Wf(this, !1), a.ht(this), (this.b.ca = !0));
      }),
      (b.pu = function (a) {
        a &&
          a != this.n &&
          (this.n.Wf(this, !0), (this.n = a), a.ng(this, !0), (this.b.ca = !0));
      }),
      (b.Tu = function (a, b) {
        var c = 0 === a;
        if (b) {
          var e = b.Um(this);
          e &&
            e.uid !== this.uid &&
            (this.n.index !== e.n.index &&
              (this.n.Wf(this, !0), (this.n = e.n), e.n.ng(this, !0)),
            this.n.Us(this, e, c),
            (this.b.ca = !0));
        }
      }),
      (p.lu = function (a) {
        a.oa(this.n.fo);
      }),
      (p.ku = function (a) {
        a.ac(this.n.name);
      }),
      (p.Su = function (a) {
        a.oa(this.bd());
      }));
    r &&
      ((b.Du = function (a, b) {
        if (this.b.A) {
          var c = this.type.qk(b);
          if (!(0 > c)) {
            var e = 1 === a;
            this.Od[c] !== e && ((this.Od[c] = e), this.Vc(), (this.b.ca = !0));
          }
        }
      }),
      (b.Eu = function (a, b, c) {
        if (this.b.A) {
          var e = this.type.qk(a);
          0 > e ||
            ((a = this.type.N[e]),
            (e = this.Ea[e]),
            (b = Math.floor(b)),
            0 > b ||
              b >= e.length ||
              (1 === this.b.A.cs(a.Na, b) && (c /= 100),
              e[b] !== c && ((e[b] = c), a.mb && (this.b.ca = !0))));
        }
      }));
  };
  xb = function () {
    this.Sj = this.Oj = !0;
    this.type.Ph = !0;
    this.b.ca = !0;
    var d,
      q,
      h = this.Pj;
    d = 0;
    for (q = h.length; d < q; ++d) h[d](this);
    this.n.qc && this.qa();
  };
  yb = function (d) {
    d && this.Pj.push(d);
  };
  Ab = function () {
    if (this.Oj) {
      var d = this.xa,
        q = this.zb;
      d.set(this.x, this.y, this.x + this.width, this.y + this.height);
      d.offset(-this.Hb * this.width, -this.Ib * this.height);
      this.k
        ? (d.offset(-this.x, -this.y),
          q.Ko(d, this.k),
          q.offset(this.x, this.y),
          q.vm(d))
        : q.Ah(d);
      d.normalize();
      this.Oj = !1;
      this.Mt();
    }
  };
  var d = new U(0, 0, 0, 0);
  Bb = function () {
    if (this.n.qc) {
      var m = this.n.jb,
        q = this.xa;
      d.set(m.Cb(q.left), m.Db(q.top), m.Cb(q.right), m.Db(q.bottom));
      this.Lb.yg(d) ||
        (this.Lb.right < this.Lb.left
          ? m.update(this, null, d)
          : m.update(this, this.Lb, d),
        this.Lb.tg(d),
        (this.n.Sc = !0));
    }
  };
  Cb = function () {
    if (this.Sj && this.nd) {
      this.qa();
      var m = this.type.Zh,
        q = this.xa;
      d.set(m.Cb(q.left), m.Db(q.top), m.Cb(q.right), m.Db(q.bottom));
      this.Wd.yg(d) ||
        (this.Wd.right < this.Wd.left
          ? m.update(this, null, d)
          : m.update(this, this.Wd, d),
        this.Wd.tg(d),
        (this.Sj = !1));
    }
  };
  zb = function (d, q) {
    return this.xa.Rb(d, q) && this.zb.Rb(d, q)
      ? this.dg
        ? this.hv(d, q)
        : this.Z && !this.Z.Gf()
        ? (this.Z.uf(this.width, this.height, this.k),
          this.Z.Rb(d - this.x, q - this.y))
        : !0
      : !1;
  };
  tb = function () {
    this.type.Bj();
    return this.Ne;
  };
  Db = function () {
    this.n.Wl();
    return this.Wc;
  };
  Eb = function () {
    V(this.X);
    var d,
      q,
      h,
      n = !0;
    d = 0;
    for (q = this.Od.length; d < q; d++)
      this.Od[d] && ((h = this.type.N[d]), this.X.push(h), h.Pc || (n = !1));
    this.kp = !!this.X.length;
    this.Ed = n;
  };
  ub = function () {
    return "Inst" + this.wo;
  };
  hb = function (d) {
    if (d && d.Jb && d.type != this) {
      var q, h, n;
      q = 0;
      for (h = d.siblings.length; q < h; q++)
        if (((n = d.siblings[q]), n.type == this)) return n;
    }
    d = this.ba().kc();
    return d.length ? d[0] : null;
  };
  ib = function (d) {
    var q = this.ba().kc();
    return q.length ? q[d.Ig() % q.length] : null;
  };
  gb = function () {
    if (this.ag && !this.K) {
      var d, q;
      d = 0;
      for (q = this.j.length; d < q; d++) this.j[d].Ne = d;
      var h = d,
        n = this.b.Gc;
      d = 0;
      for (q = n.length; d < q; ++d) n[d].type === this && (n[d].Ne = h++);
      this.ag = !1;
    }
  };
  rb = function (d) {
    if (d < this.j.length) return this.j[d];
    d -= this.j.length;
    var q = this.b.Gc,
      h,
      n;
    h = 0;
    for (n = q.length; h < n; ++h)
      if (q[h].type === this) {
        if (0 === d) return q[h];
        --d;
      }
    return null;
  };
  jb = function () {
    return this.Gd[this.$c];
  };
  kb = function () {
    this.$c++;
    this.$c === this.Gd.length
      ? this.Gd.push(new fb(this))
      : ((this.Gd[this.$c].na = !0), V(this.Gd[this.$c].Y));
  };
  lb = function () {
    this.$c++;
    this.$c === this.Gd.length && this.Gd.push(new fb(this));
    var d = this.Gd[this.$c],
      q = this.Gd[this.$c - 1];
    q.na ? (d.na = !0) : ((d.na = !1), ya(d.j, q.j));
    V(d.Y);
  };
  mb = function () {
    this.$c--;
  };
  nb = function (d) {
    var q,
      h,
      n,
      t,
      c,
      a = 0;
    if (!this.K)
      for (q = 0, h = this.za.length; q < h; q++)
        for (c = this.za[q], n = 0, t = c.Da.length; n < t; n++) {
          if (d === c.Da[n].name) return (this.M.lastBehIndex = a), c.Da[n];
          a++;
        }
    q = 0;
    for (h = this.Da.length; q < h; q++) {
      if (d === this.Da[q].name) return (this.M.lastBehIndex = a), this.Da[q];
      a++;
    }
    return null;
  };
  ob = function (d) {
    return this.pi(d) ? this.M.lastBehIndex : -1;
  };
  pb = function (d) {
    var q, h;
    q = 0;
    for (h = this.N.length; q < h; q++) if (this.N[q].name === d) return q;
    return -1;
  };
  qb = function () {
    if (this.Jb && !this.K) {
      var d, q, h, n, t, c, a;
      this.Bj();
      c = this.ba();
      var f = c.na,
        r = (d = this.b.tb()) && d.ic && d.ic.Nc;
      d = 0;
      for (q = this.Qb.length; d < q; d++)
        if (
          ((t = this.Qb[d]),
          t !== this && (t.Bj(), (a = t.ba()), (a.na = f), !f))
        ) {
          V(a.j);
          h = 0;
          for (n = c.j.length; h < n; ++h) a.j[h] = t.rk(c.j[h].Ne);
          if (r)
            for (V(a.Y), h = 0, n = c.Y.length; h < n; ++h)
              a.Y[h] = t.rk(c.Y[h].Ne);
        }
    }
  };
  sb = function () {
    return "Type" + this.V;
  };
  Sb = function (d, q, h) {
    if ("undefined" === typeof d || "undefined" === typeof h) return !1;
    switch (q) {
      case 0:
        return d === h;
      case 1:
        return d !== h;
      case 2:
        return d < h;
      case 3:
        return d <= h;
      case 4:
        return d > h;
      case 5:
        return d >= h;
      default:
        return !1;
    }
  };
})();
var ac = {};
function ec(d) {
  this.b = d;
}
(function () {
  function d() {}
  function m() {}
  var q = ec.prototype;
  q.ta = function (d) {
    this.Ba = d;
    this.b = d.b;
  };
  q.ta.prototype.U = function () {};
  q.fa = function (d) {
    this.type = d;
    this.b = d.b;
  };
  var h = q.fa.prototype;
  h.U = function () {
    this.b.Ha
      ? w(
          "[Construct 2] Button plugin not supported on this platform - the object will not be created"
        )
      : ((this.Ef = 1 === this.G[0]),
        (this.mc = document.createElement("input")),
        (this.aa = this.Ef ? document.createElement("label") : this.mc),
        (this.Yg = null),
        (this.mc.type = this.Ef ? "checkbox" : "button"),
        (this.mc.id = this.G[6]),
        jQuery(this.aa).appendTo(this.b.Yh ? this.b.Yh : "body"),
        this.Ef
          ? (jQuery(this.mc).appendTo(this.aa),
            (this.Yg = document.createTextNode(this.G[1])),
            jQuery(this.aa).append(this.Yg),
            (this.mc.checked = 0 !== this.G[7]),
            jQuery(this.aa).css("font-family", "sans-serif"),
            jQuery(this.aa).css("display", "inline-block"),
            jQuery(this.aa).css("color", "black"))
          : (this.mc.value = this.G[1]),
        (this.aa.title = this.G[2]),
        (this.mc.disabled = 0 === this.G[4]),
        (this.mq = 0 !== this.G[5]),
        (this.Ie = !1),
        0 === this.G[3] &&
          (jQuery(this.aa).hide(), (this.visible = !1), (this.Ie = !0)),
        (this.mc.onclick = (function (d) {
          return function (h) {
            h.stopPropagation();
            d.b.Ff = !0;
            d.b.trigger(ec.prototype.q.em, d);
            d.b.Ff = !1;
          };
        })(this)),
        this.aa.addEventListener(
          "touchstart",
          function (d) {
            d.stopPropagation();
          },
          !1
        ),
        this.aa.addEventListener(
          "touchmove",
          function (d) {
            d.stopPropagation();
          },
          !1
        ),
        this.aa.addEventListener(
          "touchend",
          function (d) {
            d.stopPropagation();
          },
          !1
        ),
        jQuery(this.aa).mousedown(function (d) {
          d.stopPropagation();
        }),
        jQuery(this.aa).mouseup(function (d) {
          d.stopPropagation();
        }),
        jQuery(this.aa).keydown(function (d) {
          d.stopPropagation();
        }),
        jQuery(this.aa).keyup(function (d) {
          d.stopPropagation();
        }),
        (this.Bn = this.Cn = this.un = this.yn = this.An = this.wn = 0),
        this.ip(!0),
        this.b.xj(this));
  };
  h.kb = function () {
    var d = { tooltip: this.aa.title, disabled: !!this.mc.disabled };
    this.Ef
      ? ((d.checked = !!this.mc.checked), (d.text = this.Yg.nodeValue))
      : (d.text = this.aa.value);
    return d;
  };
  h.Ab = function (d) {
    this.aa.title = d.tooltip;
    this.mc.disabled = d.disabled;
    this.Ef
      ? ((this.mc.checked = d.checked), (this.Yg.nodeValue = d.text))
      : (this.aa.value = d.text);
  };
  h.oe = function () {
    this.b.Ha || (jQuery(this.aa).remove(), (this.aa = null));
  };
  h.Ob = function () {
    this.ip();
  };
  h.ip = function (d) {
    if (!this.b.Ha) {
      var h = this.n.ia(this.x, this.y, !0),
        c = this.n.ia(this.x, this.y, !1),
        a = this.n.ia(this.x + this.width, this.y + this.height, !0),
        f = this.n.ia(this.x + this.width, this.y + this.height, !1),
        m = this.b.width / this.b.devicePixelRatio,
        e = this.b.height / this.b.devicePixelRatio;
      !this.visible || !this.n.visible || 0 >= a || 0 >= f || h >= m || c >= e
        ? (this.Ie || jQuery(this.aa).hide(), (this.Ie = !0))
        : (1 > h && (h = 1),
          1 > c && (c = 1),
          a >= m && (a = m - 1),
          f >= e && (f = e - 1),
          (m = window.innerWidth),
          (e = window.innerHeight),
          d ||
          this.wn !== h ||
          this.An !== c ||
          this.yn !== a ||
          this.un !== f ||
          this.Cn !== m ||
          this.Bn !== e
            ? ((this.wn = h),
              (this.An = c),
              (this.yn = a),
              (this.un = f),
              (this.Cn = m),
              (this.Bn = e),
              this.Ie && (jQuery(this.aa).show(), (this.Ie = !1)),
              (d = Math.round(h) + jQuery(this.b.canvas).offset().left),
              (m = Math.round(c) + jQuery(this.b.canvas).offset().top),
              jQuery(this.aa).css("position", "absolute"),
              jQuery(this.aa).offset({ left: d, top: m }),
              jQuery(this.aa).width(Math.round(a - h)),
              jQuery(this.aa).height(Math.round(f - c)),
              this.mq &&
                jQuery(this.aa).css(
                  "font-size",
                  this.n.Gb(!0) / this.b.devicePixelRatio - 0.2 + "em"
                ))
            : this.Ie && (jQuery(this.aa).show(), (this.Ie = !1)));
    }
  };
  h.Hc = function () {};
  h.Eb = function () {};
  d.prototype.em = function () {
    return !0;
  };
  q.q = new d();
  m.prototype.lm = function (d) {
    this.b.Ha || (this.Ef ? (this.Yg.nodeValue = d) : (this.aa.value = d));
  };
  m.prototype.mm = function (d) {
    this.b.Ha || (this.visible = 0 !== d);
  };
  m.prototype.ye = function (d) {
    this.b.Ha || (this.mc.disabled = 0 === d);
  };
  m.prototype.Vp = function (d, h) {
    this.b.Ha || jQuery(this.aa).css(d, h);
  };
  q.R = new m();
  q.Fb = new (function () {})();
})();
function fc(d) {
  this.b = d;
}
(function () {
  function d() {}
  var m = fc.prototype;
  m.ta = function (d) {
    this.Ba = d;
    this.b = d.b;
  };
  m.ta.prototype.U = function () {};
  m.fa = function (d) {
    this.type = d;
    this.b = d.b;
    this.Xg = Array(256);
    this.Hh = Array(256);
    this.Id = 0;
  };
  var q = m.fa.prototype;
  q.U = function () {
    var d = this;
    this.b.Ha ||
      (jQuery(document).keydown(function (h) {
        d.el(h);
      }),
      jQuery(document).keyup(function (h) {
        d.fl(h);
      }));
  };
  var h = [32, 33, 34, 35, 36, 37, 38, 39, 40, 44];
  q.el = function (d) {
    var m = !1;
    window != window.top &&
      -1 < h.indexOf(d.which) &&
      (d.preventDefault(), (m = !0), d.stopPropagation());
    if (this.Xg[d.which]) this.Hh[d.which] && !m && d.preventDefault();
    else {
      this.Xg[d.which] = !0;
      this.Id = d.which;
      this.b.Ff = !0;
      this.b.trigger(fc.prototype.q.dm, this);
      var c = this.b.trigger(fc.prototype.q.fm, this),
        a = this.b.trigger(fc.prototype.q.Lp, this);
      this.b.Ff = !1;
      if (c || a) (this.Hh[d.which] = !0), m || d.preventDefault();
    }
  };
  q.fl = function (d) {
    this.Xg[d.which] = !1;
    this.Id = d.which;
    this.b.Ff = !0;
    this.b.trigger(fc.prototype.q.Ij, this);
    var h = this.b.trigger(fc.prototype.q.hm, this),
      c = this.b.trigger(fc.prototype.q.gm, this);
    this.b.Ff = !1;
    if (h || c || this.Hh[d.which]) (this.Hh[d.which] = !0), d.preventDefault();
  };
  q.cf = function () {
    var d;
    for (d = 0; 256 > d; ++d)
      if (this.Xg[d]) {
        this.Xg[d] = !1;
        this.Id = d;
        this.b.trigger(fc.prototype.q.Ij, this);
        var h = this.b.trigger(fc.prototype.q.hm, this),
          c = this.b.trigger(fc.prototype.q.gm, this);
        if (h || c) this.Hh[d] = !0;
      }
  };
  q.kb = function () {
    return { triggerKey: this.Id };
  };
  q.Ab = function (d) {
    this.Id = d.triggerKey;
  };
  d.prototype.fm = function (d) {
    return d === this.Id;
  };
  d.prototype.dm = function () {
    return !0;
  };
  d.prototype.Ij = function () {
    return !0;
  };
  d.prototype.hm = function (d) {
    return d === this.Id;
  };
  d.prototype.Lp = function (d) {
    return d === this.Id;
  };
  d.prototype.gm = function (d) {
    return d === this.Id;
  };
  m.q = new d();
  m.R = new (function () {})();
  m.Fb = new (function () {})();
})();
function Z(d) {
  this.b = d;
}
(function () {
  function d() {
    if (0 === this.bk.length) {
      var a = document.createElement("canvas");
      a.width = this.width;
      a.height = this.height;
      var b = a.getContext("2d");
      this.$f
        ? b.drawImage(
            this.T,
            this.Uf,
            this.Vf,
            this.width,
            this.height,
            0,
            0,
            this.width,
            this.height
          )
        : b.drawImage(this.T, 0, 0, this.width, this.height);
      this.bk = a.toDataURL("image/png");
    }
    return this.bk;
  }
  function m() {}
  function q(a) {
    a[0] = 0;
    a[1] = 0;
    a[2] = 0;
    p.push(a);
  }
  function h(a, b) {
    return a < b ? "" + a + "," + b : "" + b + "," + a;
  }
  function n(a, b, c, e) {
    b = b.uid;
    c = c.uid;
    var f = h(b, c);
    if (a.hasOwnProperty(f)) a[f][2] = e;
    else {
      var d = p.length ? p.pop() : [0, 0, 0];
      d[0] = b;
      d[1] = c;
      d[2] = e;
      a[f] = d;
    }
  }
  function t(a, b, c) {
    b = h(b.uid, c.uid);
    a.hasOwnProperty(b) && (q(a[b]), delete a[b]);
  }
  function c(a, b, c) {
    b = h(b.uid, c.uid);
    if (a.hasOwnProperty(b)) return (l = a[b][2]), !0;
    l = -2;
    return !1;
  }
  function a() {}
  var f = Z.prototype;
  f.ta = function (a) {
    this.Ba = a;
    this.b = a.b;
  };
  var r = f.ta.prototype;
  r.U = function () {
    if (!this.K) {
      var a, b, c, e, f, h, l, n, p;
      this.Cc = [];
      this.Lg = !1;
      a = 0;
      for (b = this.Pb.length; a < b; a++) {
        f = this.Pb[a];
        l = {};
        l.name = f[0];
        l.speed = f[1];
        l.loop = f[2];
        l.ul = f[3];
        l.wl = f[4];
        l.po = f[5];
        l.V = f[6];
        l.frames = [];
        c = 0;
        for (e = f[7].length; c < e; c++)
          (h = f[7][c]),
            (n = {}),
            (n.Jl = h[0]),
            (n.Kl = h[1]),
            (n.Uf = h[2]),
            (n.Vf = h[3]),
            (n.width = h[4]),
            (n.height = h[5]),
            (n.duration = h[6]),
            (n.Hb = h[7]),
            (n.Ib = h[8]),
            (n.yk = h[9]),
            (n.Yi = h[10]),
            (n.qo = h[11]),
            (n.$f = 0 !== n.width),
            (n.bk = ""),
            (n.Xu = d),
            (p = { left: 0, top: 0, right: 1, bottom: 1 }),
            (n.El = p),
            (n.ra = null),
            (p = this.b.Or(h[0]))
              ? (n.T = p)
              : ((n.T = new Image()),
                (n.T.xq = h[0]),
                (n.T.Bm = h[1]),
                (n.T.rq = null),
                this.b.mp(n.T, h[0])),
            l.frames.push(n),
            this.Cc.push(n);
        this.Pb[a] = l;
      }
    }
  };
  r.gp = function () {
    var a, b, c;
    a = 0;
    for (b = this.j.length; a < b; a++) (c = this.j[a]), (c.bi = c.eb.ra);
  };
  r.Wi = function () {
    if (!this.K) {
      var a, b, c;
      a = 0;
      for (b = this.Cc.length; a < b; ++a)
        (c = this.Cc[a]), (c.T.rq = null), (c.ra = null);
      this.Lg = !1;
      this.gp();
    }
  };
  r.gl = function () {
    if (!this.K && this.j.length) {
      var a, b, c;
      a = 0;
      for (b = this.Cc.length; a < b; ++a)
        (c = this.Cc[a]), (c.ra = this.b.A.Ki(c.T, !1, this.b.Aa, c.qo));
      this.gp();
    }
  };
  r.Xk = function () {
    if (!this.K && !this.Lg && this.b.A) {
      var a, b, c;
      a = 0;
      for (b = this.Cc.length; a < b; ++a)
        (c = this.Cc[a]), (c.ra = this.b.A.Ki(c.T, !1, this.b.Aa, c.qo));
      this.Lg = !0;
    }
  };
  r.Tl = function () {
    if (!this.K && !this.j.length && this.Lg) {
      var a, b, c;
      a = 0;
      for (b = this.Cc.length; a < b; ++a)
        (c = this.Cc[a]), this.b.A.deleteTexture(c.ra), (c.ra = null);
      this.Lg = !1;
    }
  };
  var e = [];
  r.ml = function (a) {
    var b, c, f;
    V(e);
    b = 0;
    for (c = this.Cc.length; b < c; ++b)
      (f = this.Cc[b].T),
        -1 === e.indexOf(f) && (a.drawImage(f, 0, 0), e.push(f));
  };
  f.fa = function (a) {
    this.type = a;
    this.b = a.b;
    a = this.type.Pb[0].frames[0].Yi;
    this.Zb ? this.Z.Bh(a) : (this.Z = new Va(a));
  };
  var b = f.fa.prototype;
  b.U = function () {
    this.visible = 0 === this.G[0];
    this.Lk = !1;
    this.nd = 0 !== this.G[3];
    this.ua = this.Pm(this.G[1]) || this.type.Pb[0];
    this.H = this.G[2];
    0 > this.H && (this.H = 0);
    this.H >= this.ua.frames.length && (this.H = this.ua.frames.length - 1);
    var a = this.ua.frames[this.H];
    this.Z.Bh(a.Yi);
    this.Hb = a.Hb;
    this.Ib = a.Ib;
    this.De = this.ua.speed;
    this.Yd = this.ua.wl;
    (1 === this.type.Pb.length && 1 === this.type.Pb[0].frames.length) ||
      0 === this.De ||
      (this.b.xj(this), (this.Lk = !0));
    this.Zb ? this.Yc.reset() : (this.Yc = new Ta());
    this.td = this.Yc.wa;
    this.ze = !0;
    this.ld = 0;
    this.Qd = !0;
    this.Wj = this.pm = "";
    this.tq = 0;
    this.Vj = -1;
    this.type.Xk();
    var b,
      c,
      e,
      f,
      d,
      h,
      l,
      a = 0;
    for (b = this.type.Pb.length; a < b; a++)
      for (f = this.type.Pb[a], c = 0, e = f.frames.length; c < e; c++)
        (d = f.frames[c]),
          0 === d.width && ((d.width = d.T.width), (d.height = d.T.height)),
          d.$f &&
            ((l = d.T),
            (h = d.El),
            (h.left = d.Uf / l.width),
            (h.top = d.Vf / l.height),
            (h.right = (d.Uf + d.width) / l.width),
            (h.bottom = (d.Vf + d.height) / l.height),
            0 === d.Uf &&
              0 === d.Vf &&
              d.width === l.width &&
              d.height === l.height &&
              (d.$f = !1));
    this.eb = this.ua.frames[this.H];
    this.bi = this.eb.ra;
  };
  b.kb = function () {
    var a = {
      a: this.ua.V,
      f: this.H,
      cas: this.De,
      fs: this.td,
      ar: this.ld,
      at: this.Yc.wa,
      rt: this.Yd,
    };
    this.ze || (a.ap = this.ze);
    this.Qd || (a.af = this.Qd);
    return a;
  };
  b.Ab = function (a) {
    var b = this.Qr(a.a);
    b && (this.ua = b);
    this.H = a.f;
    0 > this.H && (this.H = 0);
    this.H >= this.ua.frames.length && (this.H = this.ua.frames.length - 1);
    this.De = a.cas;
    this.td = a.fs;
    this.ld = a.ar;
    this.Yc.reset();
    this.Yc.wa = a.at;
    this.ze = a.hasOwnProperty("ap") ? a.ap : !0;
    this.Qd = a.hasOwnProperty("af") ? a.af : !0;
    a.hasOwnProperty("rt") ? (this.Yd = a.rt) : (this.Yd = this.ua.wl);
    this.eb = this.ua.frames[this.H];
    this.bi = this.eb.ra;
    this.Z.Bh(this.eb.Yi);
    this.Hb = this.eb.Hb;
    this.Ib = this.eb.Ib;
  };
  b.Mj = function (a) {
    this.H = a ? 0 : this.ua.frames.length - 1;
    this.ze = !1;
    this.pm = this.ua.name;
    this.b.trigger(Z.prototype.q.Fp, this);
    this.b.trigger(Z.prototype.q.Ep, this);
    this.ld = 0;
  };
  b.Nj = function () {
    return this.Yc.wa;
  };
  b.Ob = function () {
    this.Yc.add(this.b.Dg(this));
    this.Wj.length && this.Dq();
    0 <= this.Vj && this.Eq();
    var a = this.Yc.wa,
      b = this.ua,
      c = b.frames[this.H],
      e = c.duration / this.De;
    this.ze &&
      a >= this.td + e &&
      (this.Qd ? this.H++ : this.H--,
      (this.td += e),
      this.H >= b.frames.length &&
        (b.po
          ? ((this.Qd = !1), (this.H = b.frames.length - 2))
          : b.loop
          ? (this.H = this.Yd)
          : (this.ld++, this.ld >= b.ul ? this.Mj(!1) : (this.H = this.Yd))),
      0 > this.H &&
        (b.po
          ? ((this.H = 1),
            (this.Qd = !0),
            b.loop || (this.ld++, this.ld >= b.ul && this.Mj(!0)))
          : b.loop
          ? (this.H = this.Yd)
          : (this.ld++, this.ld >= b.ul ? this.Mj(!0) : (this.H = this.Yd))),
      0 > this.H
        ? (this.H = 0)
        : this.H >= b.frames.length && (this.H = b.frames.length - 1),
      a > this.td + b.frames[this.H].duration / this.De && (this.td = a),
      (a = b.frames[this.H]),
      this.sf(c, a),
      (this.b.ca = !0));
  };
  b.Pm = function (a) {
    var b, c, e;
    b = 0;
    for (c = this.type.Pb.length; b < c; b++)
      if (((e = this.type.Pb[b]), bb(e.name, a))) return e;
    return null;
  };
  b.Qr = function (a) {
    var b, c, e;
    b = 0;
    for (c = this.type.Pb.length; b < c; b++)
      if (((e = this.type.Pb[b]), e.V === a)) return e;
    return null;
  };
  b.Dq = function () {
    var a = this.ua.frames[this.H],
      b = this.Pm(this.Wj);
    this.Wj = "";
    !b ||
      (bb(b.name, this.ua.name) && this.ze) ||
      ((this.ua = b),
      (this.De = b.speed),
      (this.Yd = b.wl),
      0 > this.H && (this.H = 0),
      this.H >= this.ua.frames.length && (this.H = this.ua.frames.length - 1),
      1 === this.tq && (this.H = 0),
      (this.ze = !0),
      (this.td = this.Yc.wa),
      (this.Qd = !0),
      this.sf(a, this.ua.frames[this.H]),
      (this.b.ca = !0));
  };
  b.Eq = function () {
    var a = this.ua.frames[this.H],
      b = this.H;
    this.H = O(this.Vj);
    0 > this.H && (this.H = 0);
    this.H >= this.ua.frames.length && (this.H = this.ua.frames.length - 1);
    b !== this.H &&
      (this.sf(a, this.ua.frames[this.H]),
      (this.td = this.Yc.wa),
      (this.b.ca = !0));
    this.Vj = -1;
  };
  b.sf = function (a, b) {
    var c = a.width,
      e = a.height,
      f = b.width,
      d = b.height;
    c != f && (this.width *= f / c);
    e != d && (this.height *= d / e);
    this.Hb = b.Hb;
    this.Ib = b.Ib;
    this.Z.Bh(b.Yi);
    this.B();
    this.eb = b;
    this.bi = b.ra;
    c = 0;
    for (e = this.L.length; c < e; c++) (f = this.L[c]), f.lo && f.lo(a, b);
    this.b.trigger(Z.prototype.q.sf, this);
  };
  b.Hc = function (a) {
    a.globalAlpha = this.opacity;
    var b = this.eb,
      c = b.$f,
      e = b.T,
      f = this.x,
      d = this.y,
      h = this.width,
      l = this.height;
    if (0 === this.k && 0 <= h && 0 <= l)
      (f -= this.Hb * h),
        (d -= this.Ib * l),
        this.b.Oc && ((f = Math.round(f)), (d = Math.round(d))),
        c
          ? a.drawImage(e, b.Uf, b.Vf, b.width, b.height, f, d, h, l)
          : a.drawImage(e, f, d, h, l);
    else {
      this.b.Oc && ((f = Math.round(f)), (d = Math.round(d)));
      a.save();
      var n = 0 < h ? 1 : -1,
        p = 0 < l ? 1 : -1;
      a.translate(f, d);
      (1 === n && 1 === p) || a.scale(n, p);
      a.rotate(this.k * n * p);
      f = 0 - this.Hb * ka(h);
      d = 0 - this.Ib * ka(l);
      c
        ? a.drawImage(e, b.Uf, b.Vf, b.width, b.height, f, d, ka(h), ka(l))
        : a.drawImage(e, f, d, ka(h), ka(l));
      a.restore();
    }
  };
  b.Fe = function (a) {
    this.Eb(a);
  };
  b.Eb = function (a) {
    a.$b(this.bi);
    a.ff(this.opacity);
    var b = this.eb,
      c = this.zb;
    if (this.b.Oc) {
      var e = Math.round(this.x) - this.x,
        f = Math.round(this.y) - this.y;
      b.$f
        ? a.fd(
            c.Xa + e,
            c.Ya + f,
            c.xb + e,
            c.yb + f,
            c.qb + e,
            c.rb + f,
            c.ob + e,
            c.pb + f,
            b.El
          )
        : a.wh(
            c.Xa + e,
            c.Ya + f,
            c.xb + e,
            c.yb + f,
            c.qb + e,
            c.rb + f,
            c.ob + e,
            c.pb + f
          );
    } else
      b.$f
        ? a.fd(c.Xa, c.Ya, c.xb, c.yb, c.qb, c.rb, c.ob, c.pb, b.El)
        : a.wh(c.Xa, c.Ya, c.xb, c.yb, c.qb, c.rb, c.ob, c.pb);
  };
  b.Ur = function (a) {
    var b = this.eb,
      c,
      e;
    c = 0;
    for (e = b.yk.length; c < e; c++) if (bb(a, b.yk[c][0])) return c;
    return -1;
  };
  b.Eg = function (a, b) {
    var c = this.eb,
      e = c.yk,
      f;
    L(a) ? (f = this.Ur(a)) : (f = a - 1);
    f = O(f);
    if (0 > f || f >= e.length) return b ? this.x : this.y;
    var d = (e[f][1] - c.Hb) * this.width,
      e = e[f][2],
      e = (e - c.Ib) * this.height,
      c = Math.cos(this.k);
    f = Math.sin(this.k);
    var h = d * c - e * f,
      e = e * c + d * f,
      d = h + this.x,
      e = e + this.y;
    return b ? d : e;
  };
  var p = [],
    l = -2,
    x = [];
  m.prototype.Hp = function (a) {
    if (!a) return !1;
    var b = this.b,
      e = b.Rm(),
      f = e.type,
      d = null;
    e.M.collmemory ? (d = e.M.collmemory) : ((d = {}), (e.M.collmemory = d));
    e.M.spriteCreatedDestroyCallback ||
      ((e.M.spriteCreatedDestroyCallback = !0),
      b.Kj(function (a) {
        var b = e.M.collmemory;
        a = a.uid;
        var c, f;
        for (c in b)
          b.hasOwnProperty(c) &&
            ((f = b[c]), f[0] === a || f[1] === a) &&
            (q(b[c]), delete b[c]);
      }));
    var h = f.ba(),
      p = a.ba(),
      h = h.kc(),
      m,
      r,
      A,
      z,
      R,
      K,
      G,
      N = this.b.kd,
      na = N - 1,
      ha = b.tb().ic;
    for (r = 0; r < h.length; r++) {
      A = h[r];
      p.na
        ? (A.qa(), this.b.Qm(A.n, a, A.xa, x), (m = x), this.b.iq(A, a, m))
        : (m = p.kc());
      for (z = 0; z < m.length; z++)
        (R = m[z]),
          b.Nb(A, R) || b.uq(A, R)
            ? ((K = c(d, A, R)),
              (K = !K || l < na),
              n(d, A, R, N),
              K &&
                (b.aj(ha.Ca),
                (K = f.ba()),
                (G = a.ba()),
                (K.na = !1),
                (G.na = !1),
                f === a
                  ? ((K.j.length = 2), (K.j[0] = A), (K.j[1] = R), f.Zc())
                  : ((K.j.length = 1),
                    (G.j.length = 1),
                    (K.j[0] = A),
                    (G.j[0] = R),
                    f.Zc(),
                    a.Zc()),
                ha.pt(),
                b.df(ha.Ca)))
            : t(d, A, R);
      V(x);
    }
    return !1;
  };
  var A = new ca(),
    z = !1;
  new U(0, 0, 0, 0);
  r.finish = function (a) {
    if (z) {
      if (a) {
        var b = this.b.tb().ic.Nc;
        a = null.ba();
        var c = A.Jd(),
          e,
          f;
        if (a.na) {
          a.na = !1;
          V(a.j);
          e = 0;
          for (f = c.length; e < f; ++e) a.j[e] = c[e];
          if (b)
            for (V(a.Y), e = 0, f = null.j.length; e < f; ++e)
              (c = null.j[e]), A.contains(c) || a.Y.push(c);
        } else if (b)
          for (b = a.j.length, e = 0, f = c.length; e < f; ++e)
            (a.j[b + e] = c[e]), Ba(a.Y, c[e]);
        else ya(a.j, c);
        null.Zc();
      }
      A.clear();
      z = !1;
    }
  };
  m.prototype.Ep = function (a) {
    return bb(this.pm, a);
  };
  m.prototype.Fp = function () {
    return !0;
  };
  m.prototype.sf = function () {
    return !0;
  };
  m.prototype.Ap = function () {
    return 0 > this.width;
  };
  f.q = new m();
  a.prototype.cq = function (a, b, c) {
    if (a && b && (b = this.b.Cm(a, b, this.Eg(c, !0), this.Eg(c, !1)))) {
      "undefined" !== typeof b.k && ((b.k = this.k), b.B());
      this.b.Lc++;
      var e, f, d;
      this.b.trigger(Object.getPrototypeOf(a.Ba).q.Nh, b);
      if (b.Jb)
        for (e = 0, f = b.siblings.length; e < f; e++)
          (d = b.siblings[e]),
            this.b.trigger(Object.getPrototypeOf(d.type.Ba).q.Nh, d);
      this.b.Lc--;
      e = this.b.Sr();
      c = !1;
      if (fa(e.M.Spawn_LastExec) || e.M.Spawn_LastExec < this.b.Le)
        (c = !0), (e.M.Spawn_LastExec = this.b.Le);
      if (
        a != this.type &&
        ((a = a.ba()),
        (a.na = !1),
        c ? (V(a.j), (a.j[0] = b)) : a.j.push(b),
        b.Jb)
      )
        for (e = 0, f = b.siblings.length; e < f; e++)
          (d = b.siblings[e]),
            (a = d.type.ba()),
            (a.na = !1),
            c ? (V(a.j), (a.j[0] = d)) : a.j.push(d);
    }
  };
  a.prototype.Tp = function (a) {
    this.De = ka(a);
    this.Qd = 0 <= a;
    this.Lk || (this.b.xj(this), (this.Lk = !0));
  };
  a.prototype.Wp = function (a) {
    a = ka(this.width) * (0 === a ? -1 : 1);
    this.width !== a && ((this.width = a), this.B());
  };
  f.R = new a();
  f.Fb = new (function () {})();
})();
function gc(d) {
  this.b = d;
}
(function () {
  function d() {
    return a.length ? a.pop() : {};
  }
  function m(c) {
    var d, e;
    d = 0;
    for (e = c.length; d < e; d++) a.push(c[d]);
    V(c);
  }
  function q(a) {
    return a.length && " " === a.charAt(a.length - 1)
      ? a.substring(0, a.length - 1)
      : a;
  }
  function h() {}
  var n = gc.prototype;
  n.U = function () {
    n.R.$p = function (a) {
      this.width !== a && ((this.width = a), (this.nf = !0), this.B());
    };
  };
  n.ta = function (a) {
    this.Ba = a;
    this.b = a.b;
  };
  var t = n.ta.prototype;
  t.U = function () {};
  t.Wi = function () {
    if (!this.K) {
      var a, c, e;
      a = 0;
      for (c = this.j.length; a < c; a++)
        (e = this.j[a]), (e.dd = null), (e.$e = null), (e.Xb = null);
    }
  };
  n.fa = function (a) {
    this.type = a;
    this.b = a.b;
    this.Zb ? V(this.ke) : (this.ke = []);
    this.nf = !0;
  };
  t = n.fa.prototype;
  t.U = function () {
    this.text = this.G[0];
    this.visible = 0 === this.G[1];
    this.font = this.G[2];
    this.color = this.G[3];
    this.ti = this.G[4];
    this.Dj = this.G[5];
    this.Zl = 0 === this.G[7];
    this.Sk = this.Di = this.width;
    this.Ci = this.height;
    this.Uk = this.G[8];
    this.mi = this.yf = "";
    this.Vo = this.Wo = this.uh = 0;
    this.$s();
    this.Xb = this.$e = this.dd = null;
    this.co = !1;
    this.ah = this.b.kd;
    this.Zb ? this.qe.set(0, 0, 1, 1) : (this.qe = new U(0, 0, 1, 1));
    this.b.A && this.b.xj(this);
  };
  t.$s = function () {
    var a = this.font.split(" "),
      c;
    for (c = 0; c < a.length; c++)
      if ("pt" === a[c].substr(a[c].length - 2, 2)) {
        this.uh = parseInt(a[c].substr(0, a[c].length - 2));
        this.ql = Math.ceil((this.uh / 72) * 96) + 4;
        0 < c && (this.mi = a[c - 1]);
        this.yf = a[c + 1];
        for (c += 2; c < a.length; c++) this.yf += " " + a[c];
        break;
      }
  };
  t.kb = function () {
    return {
      t: this.text,
      f: this.font,
      c: this.color,
      ha: this.ti,
      va: this.Dj,
      wr: this.Zl,
      lho: this.Uk,
      fn: this.yf,
      fs: this.mi,
      ps: this.uh,
      pxh: this.ql,
      tw: this.Wo,
      th: this.Vo,
      lrt: this.ah,
    };
  };
  t.Ab = function (a) {
    this.text = a.t;
    this.font = a.f;
    this.color = a.c;
    this.ti = a.ha;
    this.Dj = a.va;
    this.Zl = a.wr;
    this.Uk = a.lho;
    this.yf = a.fn;
    this.mi = a.fs;
    this.uh = a.ps;
    this.ql = a.pxh;
    this.Wo = a.tw;
    this.Vo = a.th;
    this.ah = a.lrt;
    this.nf = !0;
    this.Sk = this.Di = this.width;
    this.Ci = this.height;
  };
  t.Ob = function () {
    if (this.b.A && this.Xb && 300 <= this.b.kd - this.ah) {
      var a = this.n;
      this.qa();
      var c = this.xa;
      if (c.right < a.da || c.bottom < a.ea || c.left > a.la || c.top > a.ka)
        this.b.A.deleteTexture(this.Xb), (this.dd = this.$e = this.Xb = null);
    }
  };
  t.oe = function () {
    this.dd = this.$e = null;
    this.b.A && this.Xb && this.b.A.deleteTexture(this.Xb);
    this.Xb = null;
  };
  t.Nj = function () {
    this.font = this.mi + " " + this.uh.toString() + "pt " + this.yf;
    this.nf = !0;
    this.b.ca = !0;
  };
  t.Hc = function (a, c) {
    a.font = this.font;
    a.textBaseline = "top";
    a.fillStyle = this.color;
    a.globalAlpha = c ? 1 : this.opacity;
    var e = 1;
    c && ((e = Math.abs(this.n.Gb())), a.save(), a.scale(e, e));
    if (this.nf || this.width !== this.Sk)
      this.type.Ba.fq(this.text, this.ke, a, this.width, this.Zl),
        (this.nf = !1),
        (this.Sk = this.width);
    this.qa();
    var e = c ? 0 : this.zb.Xa,
      b = c ? 0 : this.zb.Ya;
    this.b.Oc && ((e = (e + 0.5) | 0), (b = (b + 0.5) | 0));
    0 === this.k ||
      c ||
      (a.save(), a.translate(e, b), a.rotate(this.k), (b = e = 0));
    var d = b + this.height,
      h = this.ql,
      h = h + this.Uk,
      n,
      m;
    1 === this.Dj
      ? (b += Math.max(this.height / 2 - (this.ke.length * h) / 2, 0))
      : 2 === this.Dj &&
        (b += Math.max(this.height - this.ke.length * h - 2, 0));
    for (
      m = 0;
      m < this.ke.length &&
      !((n = e),
      1 === this.ti
        ? (n = e + (this.width - this.ke[m].width) / 2)
        : 2 === this.ti && (n = e + (this.width - this.ke[m].width)),
      a.fillText(this.ke[m].text, n, b),
      (b += h),
      b >= d - h);
      m++
    );
    (0 !== this.k || c) && a.restore();
    this.ah = this.b.kd;
  };
  t.Eb = function (a) {
    if (!(1 > this.width || 1 > this.height)) {
      var c = this.nf || this.co;
      this.co = !1;
      var e = this.n.Gb(),
        b = this.n.Ub(),
        d = this.qe,
        h = e * this.width,
        n = e * this.height,
        m = Math.ceil(h),
        q = Math.ceil(n),
        g = Math.abs(m),
        k = Math.abs(q),
        t = this.b.J / 2,
        P = this.b.I / 2;
      this.$e ||
        ((this.dd = document.createElement("canvas")),
        (this.dd.width = g),
        (this.dd.height = k),
        (this.Di = g),
        (this.Ci = k),
        (c = !0),
        (this.$e = this.dd.getContext("2d")));
      if (g !== this.Di || k !== this.Ci)
        (this.dd.width = g),
          (this.dd.height = k),
          this.Xb && (a.deleteTexture(this.Xb), (this.Xb = null)),
          (c = !0);
      c &&
        (this.$e.clearRect(0, 0, g, k),
        this.Hc(this.$e, !0),
        this.Xb || (this.Xb = a.hc(g, k, this.b.Aa, this.b.Re)),
        a.Pt(this.dd, this.Xb, this.b.Re));
      this.Di = g;
      this.Ci = k;
      a.$b(this.Xb);
      a.ff(this.opacity);
      a.wc();
      a.translate(-t, -P);
      a.pc();
      var E = this.zb,
        c = this.n.ia(E.Xa, E.Ya, !0, !0),
        g = this.n.ia(E.Xa, E.Ya, !1, !0),
        k = this.n.ia(E.xb, E.yb, !0, !0),
        t = this.n.ia(E.xb, E.yb, !1, !0),
        P = this.n.ia(E.qb, E.rb, !0, !0),
        T = this.n.ia(E.qb, E.rb, !1, !0),
        F = this.n.ia(E.ob, E.pb, !0, !0),
        E = this.n.ia(E.ob, E.pb, !1, !0);
      if (this.b.Oc || (0 === this.k && 0 === b))
        var C = ((c + 0.5) | 0) - c,
          I = ((g + 0.5) | 0) - g,
          c = c + C,
          g = g + I,
          k = k + C,
          t = t + I,
          P = P + C,
          T = T + I,
          F = F + C,
          E = E + I;
      0 === this.k && 0 === b
        ? ((k = c + m),
          (t = g),
          (P = k),
          (T = g + q),
          (F = c),
          (E = T),
          (d.right = 1),
          (d.bottom = 1))
        : ((d.right = h / m), (d.bottom = n / q));
      a.fd(c, g, k, t, P, T, F, E, d);
      a.wc();
      a.scale(e, e);
      a.hj(-this.n.Ub());
      a.translate((this.n.da + this.n.la) / -2, (this.n.ea + this.n.ka) / -2);
      a.pc();
      this.ah = this.b.kd;
    }
  };
  var c = [];
  n.dq = function (a) {
    V(c);
    for (var d = "", e, b = 0; b < a.length; )
      if (((e = a.charAt(b)), "\n" === e))
        d.length && (c.push(d), (d = "")), c.push("\n"), ++b;
      else if (" " === e || "\t" === e || "-" === e) {
        do (d += a.charAt(b)), b++;
        while (b < a.length && (" " === a.charAt(b) || "\t" === a.charAt(b)));
        c.push(d);
        d = "";
      } else b < a.length && ((d += e), b++);
    d.length && c.push(d);
  };
  var a = [];
  n.fq = function (a, c, e, b, h) {
    if (a && a.length)
      if (2 >= b) m(c);
      else {
        if (100 >= a.length && -1 === a.indexOf("\n")) {
          var l = e.measureText(a).width;
          if (l <= b) {
            m(c);
            c.push(d());
            c[0].text = a;
            c[0].width = l;
            return;
          }
        }
        this.gq(a, c, e, b, h);
      }
    else m(c);
  };
  n.gq = function (f, h, e, b, n) {
    n && (this.dq(f), (f = c));
    var l = "",
      m,
      t,
      z,
      g = 0;
    for (z = 0; z < f.length; z++)
      "\n" === f[z]
        ? (g >= h.length && h.push(d()),
          (l = q(l)),
          (t = h[g]),
          (t.text = l),
          (t.width = e.measureText(l).width),
          g++,
          (l = ""))
        : ((m = l),
          (l += f[z]),
          (t = e.measureText(l).width),
          t >= b &&
            (g >= h.length && h.push(d()),
            (m = q(m)),
            (t = h[g]),
            (t.text = m),
            (t.width = e.measureText(m).width),
            g++,
            (l = f[z]),
            n || " " !== l || (l = "")));
    l.length &&
      (g >= h.length && h.push(d()),
      (l = q(l)),
      (t = h[g]),
      (t.text = l),
      (t.width = e.measureText(l).width),
      g++);
    for (z = g; z < h.length; z++) a.push(h[z]);
    h.length = g;
  };
  n.q = new (function () {})();
  h.prototype.lm = function (a) {
    ga(a) && 1e9 > a && (a = Math.round(1e10 * a) / 1e10);
    a = a.toString();
    this.text !== a && ((this.text = a), (this.nf = !0), (this.b.ca = !0));
  };
  n.R = new h();
  n.Fb = new (function () {})();
})();
function hc(d) {
  this.b = d;
}
(function () {
  var d = hc.prototype;
  d.ta = function (d) {
    this.Ba = d;
    this.b = d.b;
  };
  var m = d.ta.prototype;
  m.U = function () {
    this.K ||
      ((this.T = new Image()),
      (this.T.Bm = this.Kl),
      this.b.mp(this.T, this.Jl),
      (this.ra = this.pattern = null));
  };
  m.Wi = function () {
    this.K || (this.ra = null);
  };
  m.gl = function () {
    if (!this.K && this.j.length) {
      this.ra || (this.ra = this.b.A.Ki(this.T, !0, this.b.Aa, this.Ll));
      var d, h;
      d = 0;
      for (h = this.j.length; d < h; d++) this.j[d].ra = this.ra;
    }
  };
  m.Xk = function () {
    this.K ||
      this.ra ||
      !this.b.A ||
      (this.ra = this.b.A.Ki(this.T, !0, this.b.Aa, this.Ll));
  };
  m.Tl = function () {
    this.K ||
      this.j.length ||
      !this.ra ||
      (this.b.A.deleteTexture(this.ra), (this.ra = null));
  };
  m.ml = function (d) {
    d.drawImage(this.T, 0, 0);
  };
  d.fa = function (d) {
    this.type = d;
    this.b = d.b;
  };
  m = d.fa.prototype;
  m.U = function () {
    this.visible = 0 === this.G[0];
    this.qe = new U(0, 0, 0, 0);
    this.an = !1;
    this.T = this.type.T;
    this.b.A
      ? (this.type.Xk(), (this.ra = this.type.ra))
      : (this.type.pattern ||
          (this.type.pattern = this.b.ya.createPattern(this.type.T, "repeat")),
        (this.pattern = this.type.pattern));
  };
  m.Xc = function () {
    this.an = !1;
    this.T = this.type.T;
  };
  m.oe = function () {
    this.b.A &&
      this.an &&
      this.ra &&
      (this.b.A.deleteTexture(this.ra), (this.ra = null));
  };
  m.Hc = function (d) {
    d.globalAlpha = this.opacity;
    d.save();
    d.fillStyle = this.pattern;
    var h = this.x,
      n = this.y;
    this.b.Oc && ((h = Math.round(h)), (n = Math.round(n)));
    var m = -(this.Hb * this.width),
      c = -(this.Ib * this.height),
      a = m % this.T.width,
      f = c % this.T.height;
    0 > a && (a += this.T.width);
    0 > f && (f += this.T.height);
    d.translate(h, n);
    d.rotate(this.k);
    d.translate(a, f);
    d.fillRect(m - a, c - f, this.width, this.height);
    d.restore();
  };
  m.Fe = function (d) {
    this.Eb(d);
  };
  m.Eb = function (d) {
    d.$b(this.ra);
    d.ff(this.opacity);
    var h = this.qe;
    h.right = this.width / this.T.width;
    h.bottom = this.height / this.T.height;
    var n = this.zb;
    if (this.b.Oc) {
      var m = Math.round(this.x) - this.x,
        c = Math.round(this.y) - this.y;
      d.fd(
        n.Xa + m,
        n.Ya + c,
        n.xb + m,
        n.yb + c,
        n.qb + m,
        n.rb + c,
        n.ob + m,
        n.pb + c,
        h
      );
    } else d.fd(n.Xa, n.Ya, n.xb, n.yb, n.qb, n.rb, n.ob, n.pb, h);
  };
  d.q = new (function () {})();
  d.R = new (function () {})();
  d.Fb = new (function () {})();
})();
function ic(d) {
  this.b = d;
}
(function () {
  function d() {}
  var m = ic.prototype;
  m.ta = function (d) {
    this.behavior = d;
    this.b = d.b;
  };
  m.ta.prototype.U = function () {};
  m.fa = function (d, n) {
    this.type = d;
    this.behavior = d.behavior;
    this.d = n;
    this.b = d.b;
  };
  var q = m.fa.prototype;
  q.U = function () {
    var d = this.G[0];
    this.Bc = this.G[1];
    this.Tb = this.G[2];
    this.pq = 0 !== this.G[3];
    this.Dl = 0 !== this.G[4];
    this.u = Math.cos(this.d.k) * d;
    this.F = Math.sin(this.d.k) * d;
    this.Ei = this.d.x;
    this.Fi = this.d.y;
    this.nc = this.d.k;
    this.zj = 0;
    this.enabled = 0 !== this.G[5];
  };
  q.kb = function () {
    return {
      acc: this.Bc,
      g: this.Tb,
      dx: this.u,
      dy: this.F,
      lx: this.Ei,
      ly: this.Fi,
      lka: this.nc,
      t: this.zj,
      e: this.enabled,
    };
  };
  q.Ab = function (d) {
    this.Bc = d.acc;
    this.Tb = d.g;
    this.u = d.dx;
    this.F = d.dy;
    this.Ei = d.lx;
    this.Fi = d.ly;
    this.nc = d.lka;
    this.zj = d.t;
    this.enabled = d.e;
  };
  q.Ob = function () {
    if (this.enabled) {
      var d = this.b.Dg(this.d),
        n,
        m;
      this.d.k !== this.nc &&
        (this.Dl &&
          ((n = La(0, 0, this.u, this.F)),
          (this.u = Math.cos(this.d.k) * n),
          (this.F = Math.sin(this.d.k) * n)),
        (this.nc = this.d.k));
      0 !== this.Bc &&
        ((n = La(0, 0, this.u, this.F)),
        (m =
          0 === this.u && 0 === this.F ? this.d.k : Ha(0, 0, this.u, this.F)),
        (n += this.Bc * d),
        0 > n && (n = 0),
        (this.u = Math.cos(m) * n),
        (this.F = Math.sin(m) * n));
      0 !== this.Tb && (this.F += this.Tb * d);
      this.Ei = this.d.x;
      this.Fi = this.d.y;
      if (0 !== this.u || 0 !== this.F)
        if (
          ((this.d.x += this.u * d),
          (this.d.y += this.F * d),
          (this.zj += La(0, 0, this.u * d, this.F * d)),
          this.Dl &&
            ((this.d.k = Ha(0, 0, this.u, this.F)),
            this.d.B(),
            (this.nc = this.d.k)),
          this.d.B(),
          this.pq && (n = this.b.cb(this.d)))
        )
          this.b.Dd(this.d, n),
            (n = La(0, 0, this.u, this.F)),
            (m = this.b.sq(this.d, this.Ei, this.Fi)),
            (this.u = Math.cos(m) * n),
            (this.F = Math.sin(m) * n),
            (this.d.x += this.u * d),
            (this.d.y += this.F * d),
            this.d.B(),
            this.Dl && ((this.nc = this.d.k = m), this.d.B()),
            this.b.vc(
              this.d,
              this.u / n,
              this.F / n,
              Math.max(2.5 * n * d, 30)
            ) || this.b.xo(this.d, 100);
    }
  };
  m.q = new (function () {})();
  d.prototype.Yp = function (d) {
    var n = Ha(0, 0, this.u, this.F);
    this.u = Math.cos(n) * d;
    this.F = Math.sin(n) * d;
  };
  d.prototype.Sp = function (d) {
    d = W(d);
    var n = La(0, 0, this.u, this.F);
    this.u = Math.cos(d) * n;
    this.F = Math.sin(d) * n;
  };
  d.prototype.ye = function (d) {
    this.enabled = 1 === d;
  };
  m.R = new d();
  m.Fb = new (function () {})();
})();
function jc(d) {
  this.b = d;
}
(function () {
  function d() {}
  var m = jc.prototype;
  m.ta = function (d) {
    this.behavior = d;
    this.b = d.b;
  };
  m.ta.prototype.U = function () {};
  m.fa = function (d, n) {
    this.type = d;
    this.behavior = d.behavior;
    this.d = n;
    this.b = d.b;
  };
  var q = m.fa.prototype;
  q.U = function () {
    this.Va = null;
    this.Xi = -1;
    this.mode = this.nc = this.uj = this.Tf = this.Cd = this.rh = 0;
    var d = this;
    this.Zb ||
      (this.lh = function (n) {
        d.dl(n);
      });
    this.b.Kj(this.lh);
  };
  q.kb = function () {
    return {
      uid: this.Va ? this.Va.uid : -1,
      pa: this.rh,
      pd: this.Cd,
      msa: this.Tf,
      tsa: this.uj,
      lka: this.nc,
      m: this.mode,
    };
  };
  q.Ab = function (d) {
    this.Xi = d.uid;
    this.rh = d.pa;
    this.Cd = d.pd;
    this.Tf = d.msa;
    this.uj = d.tsa;
    this.nc = d.lka;
    this.mode = d.m;
  };
  q.Xc = function () {
    -1 === this.Xi ? (this.Va = null) : (this.Va = this.b.Fg(this.Xi));
    this.Xi = -1;
  };
  q.dl = function (d) {
    this.Va == d && (this.Va = null);
  };
  q.oe = function () {
    this.Va = null;
    this.b.zo(this.lh);
  };
  q.Ob = function () {};
  q.wj = function () {
    if (this.Va) {
      this.nc !== this.d.k && (this.Tf = X(this.Tf + (this.d.k - this.nc)));
      var d = this.d.x,
        n = this.d.y;
      if (3 === this.mode || 4 === this.mode) {
        var m = La(this.d.x, this.d.y, this.Va.x, this.Va.y);
        if (m > this.Cd || (4 === this.mode && m < this.Cd))
          (n = Ha(this.Va.x, this.Va.y, this.d.x, this.d.y)),
            (d = this.Va.x + Math.cos(n) * this.Cd),
            (n = this.Va.y + Math.sin(n) * this.Cd);
      } else
        (d = this.Va.x + Math.cos(this.Va.k + this.rh) * this.Cd),
          (n = this.Va.y + Math.sin(this.Va.k + this.rh) * this.Cd);
      this.nc = m = X(this.Tf + (this.Va.k - this.uj));
      (0 !== this.mode &&
        1 !== this.mode &&
        3 !== this.mode &&
        4 !== this.mode) ||
        (this.d.x === d && this.d.y === n) ||
        ((this.d.x = d), (this.d.y = n), this.d.B());
      (0 !== this.mode && 2 !== this.mode) ||
        this.d.k === m ||
        ((this.d.k = m), this.d.B());
    }
  };
  m.q = new (function () {})();
  d.prototype.Rp = function (d, n) {
    if (d) {
      var m = d.Um(this.d);
      m &&
        ((this.Va = m),
        (this.rh = Ha(m.x, m.y, this.d.x, this.d.y) - m.k),
        (this.Cd = La(m.x, m.y, this.d.x, this.d.y)),
        (this.nc = this.Tf = this.d.k),
        (this.uj = m.k),
        (this.mode = n));
    }
  };
  m.R = new d();
  m.Fb = new (function () {})();
})();
function kc(d) {
  this.b = d;
}
(function () {
  function d() {}
  function m() {}
  var q = kc.prototype;
  q.ta = function (d) {
    this.behavior = d;
    this.b = d.b;
  };
  q.ta.prototype.U = function () {};
  q.fa = function (d, h) {
    this.type = d;
    this.behavior = d.behavior;
    this.d = h;
    this.b = d.b;
    this.oj =
      this.Gl =
      this.Fl =
      this.xk =
      this.qg =
      this.xg =
      this.Kf =
      this.Lf =
      this.yh =
      this.bh =
        !1;
    this.hb = null;
    this.Vk = -1;
    this.ie = this.he = 0;
    this.Cg = !1;
    this.zf = this.Dc = 0;
    this.li = !0;
    this.F = this.u = 0;
  };
  var h = q.fa.prototype;
  h.hp = function () {
    this.Ja = Math.cos(this.Df);
    this.Ka = Math.sin(this.Df);
    this.re = Math.cos(this.Df - Math.PI / 2);
    this.te = Math.sin(this.Df - Math.PI / 2);
    this.Ja = ab(this.Ja);
    this.Ka = ab(this.Ka);
    this.re = ab(this.re);
    this.te = ab(this.te);
    this.nk = this.Tb;
    0 > this.Tb &&
      ((this.Ja *= -1), (this.Ka *= -1), (this.Tb = Math.abs(this.Tb)));
  };
  h.U = function () {
    this.Rf = this.G[0];
    this.Bc = this.G[1];
    this.wf = this.G[2];
    this.Wg = this.G[3];
    this.nk = this.Tb = this.G[4];
    this.Ri = this.G[5];
    this.ek = 0 !== this.G[6];
    this.Pk = this.G[7] / 1e3;
    this.Aq = 1 === this.G[8];
    this.enabled = 0 !== this.G[9];
    this.jg = !1;
    this.kg = this.b.we(this.d);
    this.Wk = -1;
    this.lf = 0;
    this.Df = W(90);
    this.hp();
    var d = this;
    this.Aq &&
      !this.b.Ha &&
      (jQuery(document).keydown(function (h) {
        d.el(h);
      }),
      jQuery(document).keyup(function (h) {
        d.fl(h);
      }));
    this.Zb ||
      (this.lh = function (h) {
        d.dl(h);
      });
    this.b.Kj(this.lh);
    this.d.M.isPlatformBehavior = !0;
  };
  h.kb = function () {
    return {
      ii: this.xk,
      lfx: this.he,
      lfy: this.ie,
      lfo: this.hb ? this.hb.uid : -1,
      am: this.Dc,
      en: this.enabled,
      fall: this.zf,
      ft: this.li,
      dx: this.u,
      dy: this.F,
      ms: this.Rf,
      acc: this.Bc,
      dec: this.wf,
      js: this.Wg,
      g: this.Tb,
      g1: this.nk,
      mf: this.Ri,
      wof: this.jg,
      woj: this.kg ? this.kg.uid : -1,
      ga: this.Df,
      edj: this.ek,
      cdj: this.qg,
      dj: this.xg,
      sus: this.Pk,
    };
  };
  h.Ab = function (d) {
    this.xk = d.ii;
    this.he = d.lfx;
    this.ie = d.lfy;
    this.Vk = d.lfo;
    this.Dc = d.am;
    this.enabled = d.en;
    this.zf = d.fall;
    this.li = d.ft;
    this.u = d.dx;
    this.F = d.dy;
    this.Rf = d.ms;
    this.Bc = d.acc;
    this.wf = d.dec;
    this.Wg = d.js;
    this.Tb = d.g;
    this.nk = d.g1;
    this.Ri = d.mf;
    this.jg = d.wof;
    this.Wk = d.woj;
    this.Df = d.ga;
    this.ek = d.edj;
    this.qg = d.cdj;
    this.xg = d.dj;
    this.Pk = d.sus;
    this.oj = this.Gl = this.Fl = this.Kf = this.Lf = this.yh = this.bh = !1;
    this.lf = 0;
    this.hp();
  };
  h.Xc = function () {
    -1 === this.Vk ? (this.hb = null) : (this.hb = this.b.Fg(this.Vk));
    -1 === this.Wk ? (this.kg = null) : (this.kg = this.b.Fg(this.Wk));
  };
  h.dl = function (d) {
    this.hb == d && (this.hb = null);
  };
  h.oe = function () {
    this.hb = null;
    this.b.zo(this.lh);
  };
  h.el = function (d) {
    switch (d.which) {
      case 38:
        d.preventDefault();
        this.Lf = !0;
        break;
      case 37:
        d.preventDefault();
        this.bh = !0;
        break;
      case 39:
        d.preventDefault(), (this.yh = !0);
    }
  };
  h.fl = function (d) {
    switch (d.which) {
      case 38:
        d.preventDefault();
        this.Kf = this.Lf = !1;
        break;
      case 37:
        d.preventDefault();
        this.bh = !1;
        break;
      case 39:
        d.preventDefault(), (this.yh = !1);
    }
  };
  h.cf = function () {
    this.Lf = this.yh = this.bh = !1;
  };
  h.Nj = function () {
    return 0 > this.Tb ? -1 : 1;
  };
  h.ln = function () {
    var d = null,
      h = null,
      c,
      a;
    c = this.d.x;
    a = this.d.y;
    this.d.x += this.Ja;
    this.d.y += this.Ka;
    this.d.B();
    if (
      this.hb &&
      this.b.Nb(this.d, this.hb) &&
      (!this.b.Ql(this.hb.type, cc) || this.hb.M.solidEnabled)
    )
      return (this.d.x = c), (this.d.y = a), this.d.B(), this.hb;
    (d = this.b.cb(this.d)) || 0 !== this.zf || (h = this.b.we(this.d, !0));
    this.d.x = c;
    this.d.y = a;
    this.d.B();
    if (d) {
      if (this.b.Nb(this.d, d)) return null;
      this.Cg = !1;
      return d;
    }
    if (h && h.length) {
      a = d = 0;
      for (c = h.length; d < c; d++)
        (h[a] = h[d]), this.b.Nb(this.d, h[d]) || a++;
      if (1 <= a) return (this.Cg = !0), h[0];
    }
    return null;
  };
  h.Ob = function () {};
  h.to = function () {
    var d = this.b.Dg(this.d),
      h,
      c,
      a,
      f,
      m,
      e,
      b,
      p,
      l;
    this.Lf || this.oj || (this.Kf = !1);
    var q = this.bh || this.Fl;
    a = this.yh || this.Gl;
    var A = (f = this.Lf || this.oj) && !this.Kf;
    this.oj = this.Gl = this.Fl = !1;
    if (this.enabled) {
      this.xk && (A = f = a = q = !1);
      f || (this.lf = 0);
      p = this.hb;
      l = !1;
      this.li &&
        ((this.b.cb(this.d) || this.b.we(this.d)) &&
          this.b.vc(this.d, -this.Ja, -this.Ka, 4, !0),
        (this.li = !1));
      !p ||
        0 !== this.F ||
        (p.y === this.ie && p.x === this.he) ||
        ((h = p.x - this.he),
        (c = p.y - this.ie),
        (this.d.x += h),
        (this.d.y += c),
        this.d.B(),
        (this.he = p.x),
        (this.ie = p.y),
        (l = !0),
        this.b.cb(this.d) &&
          this.b.vc(this.d, -h, -c, 2.5 * Math.sqrt(h * h + c * c)));
      var z = this.ln();
      if ((c = this.b.cb(this.d)))
        if (
          ((m = Math.abs(this.d.width)),
          (e = Math.abs(this.d.height)),
          this.d.M.inputPredicted)
        )
          this.b.vc(this.d, -this.Ja, -this.Ka, 10, !1);
        else if (this.b.pl(this.d, -this.Ja, -this.Ka, e / 8))
          this.b.Dd(this.d, c);
        else if (this.b.pl(this.d, this.re, this.te, m / 2))
          this.b.Dd(this.d, c);
        else if (this.b.pl(this.d, this.Ja, this.Ka, e / 2))
          this.b.Dd(this.d, c);
        else if (this.b.xo(this.d, Math.max(m, e) / 2)) this.b.Dd(this.d, c);
        else return;
      z
        ? ((this.qg = this.xg = !1),
          0 < this.F &&
            (this.jg ||
              (this.b.cj(this.d, -this.Ja, -this.Ka, z), (this.jg = !0)),
            (this.F = 0)),
          p != z)
          ? ((this.hb = z),
            (this.he = z.x),
            (this.ie = z.y),
            this.b.Dd(this.d, z))
          : l &&
            (c = this.b.cb(this.d)) &&
            (this.b.Dd(this.d, c),
            0 !== h &&
              (0 < h
                ? this.b.vc(this.d, -this.re, -this.te)
                : this.b.vc(this.d, this.re, this.te)),
            this.b.vc(this.d, -this.Ja, -this.Ka))
        : f || (this.qg = !0);
      if ((z && A) || (!z && this.ek && f && this.qg && !this.xg))
        (p = this.d.x),
          (l = this.d.y),
          (this.d.x -= this.Ja),
          (this.d.y -= this.Ka),
          this.d.B(),
          this.b.cb(this.d)
            ? (A = !1)
            : ((this.lf = this.Pk),
              this.b.trigger(kc.prototype.q.Kp, this.d),
              (this.Dc = 2),
              (this.F = -this.Wg),
              (A = !0),
              z ? (this.Kf = !0) : (this.xg = !0)),
          (this.d.x = p),
          (this.d.y = l),
          this.d.B();
      z ||
        (f && 0 < this.lf
          ? ((this.F = -this.Wg), (this.lf -= d))
          : ((this.hb = null),
            (this.F += this.Tb * d),
            this.F > this.Ri && (this.F = this.Ri)),
        A && (this.Kf = !0));
      this.jg = !!z;
      q == a &&
        (0 > this.u
          ? ((this.u += this.wf * d), 0 < this.u && (this.u = 0))
          : 0 < this.u &&
            ((this.u -= this.wf * d), 0 > this.u && (this.u = 0)));
      q &&
        !a &&
        (this.u =
          0 < this.u ? this.u - (this.Bc + this.wf) * d : this.u - this.Bc * d);
      a &&
        !q &&
        (this.u =
          0 > this.u ? this.u + (this.Bc + this.wf) * d : this.u + this.Bc * d);
      this.u > this.Rf
        ? (this.u = this.Rf)
        : this.u < -this.Rf && (this.u = -this.Rf);
      q = !1;
      0 !== this.u &&
        ((p = this.d.x),
        (l = this.d.y),
        (h = this.u * d * this.re),
        (c = this.u * d * this.te),
        (this.d.x += this.re * (1 < this.u ? 1 : -1) - this.Ja),
        (this.d.y += this.te * (1 < this.u ? 1 : -1) - this.Ka),
        this.d.B(),
        (f = !1),
        (m = this.b.cb(this.d)),
        (this.d.x = p + h),
        (this.d.y = l + c),
        this.d.B(),
        (a = this.b.cb(this.d)),
        !a &&
          z &&
          (a = this.b.we(this.d)) &&
          ((this.d.x = p),
          (this.d.y = l),
          this.d.B(),
          this.b.Nb(this.d, a) ? ((a = null), (f = !1)) : (f = !0),
          (this.d.x = p + h),
          (this.d.y = l + c),
          this.d.B()),
        a
          ? ((h = Math.abs(this.u * d) + 2),
            m || !this.b.vc(this.d, -this.Ja, -this.Ka, h, f, a))
            ? (this.b.Dd(this.d, a),
              (h = Math.max(Math.abs(this.u * d * 2.5), 30)),
              this.b.vc(
                this.d,
                this.re * (0 > this.u ? 1 : -1),
                this.te * (0 > this.u ? 1 : -1),
                h,
                !1
              )
                ? !z ||
                  f ||
                  this.Cg ||
                  ((p = this.d.x),
                  (l = this.d.y),
                  (this.d.x += this.Ja),
                  (this.d.y += this.Ka),
                  this.b.cb(this.d)
                    ? this.b.vc(this.d, -this.Ja, -this.Ka, 3, !1) ||
                      ((this.d.x = p), (this.d.y = l), this.d.B())
                    : ((this.d.x = p), (this.d.y = l), this.d.B()))
                : ((this.d.x = p), (this.d.y = l), this.d.B()),
              f || (this.u = 0))
            : !m &&
              !A &&
              Math.abs(this.F) < Math.abs(this.Wg / 4) &&
              ((this.F = 0), z || (q = !0))
          : ((p = this.ln()),
            z && !p
              ? ((c = Math.ceil(Math.abs(this.u * d)) + 2),
                (p = this.d.x),
                (l = this.d.y),
                (this.d.x += this.Ja * c),
                (this.d.y += this.Ka * c),
                this.d.B(),
                this.b.cb(this.d) || this.b.we(this.d)
                  ? this.b.vc(this.d, -this.Ja, -this.Ka, c + 2, !0)
                  : ((this.d.x = p), (this.d.y = l), this.d.B()))
              : p &&
                (!z &&
                  this.Cg &&
                  ((this.hb = p),
                  (this.he = p.x),
                  (this.ie = p.y),
                  (this.F = 0),
                  (q = !0)),
                0 === this.F && this.b.cj(this.d, -this.Ja, -this.Ka, p))));
      if (0 !== this.F) {
        p = this.d.x;
        l = this.d.y;
        this.d.x += this.F * d * this.Ja;
        this.d.y += this.F * d * this.Ka;
        h = this.d.x;
        a = this.d.y;
        this.d.B();
        c = this.b.cb(this.d);
        f = !1;
        if (!c && 0 < this.F && !z) {
          if ((f = 0 < this.zf ? null : this.b.we(this.d, !0)) && f.length) {
            if (this.kg) {
              this.d.x = p;
              this.d.y = l;
              this.d.B();
              b = m = 0;
              for (e = f.length; m < e; m++)
                (f[b] = f[m]), this.b.Nb(this.d, f[m]) || b++;
              f.length = b;
              this.d.x = h;
              this.d.y = a;
              this.d.B();
            }
            1 <= f.length && (c = f[0]);
          }
          f = !!c;
        }
        c &&
          (this.b.Dd(this.d, c),
          (this.lf = 0),
          (h = f
            ? Math.abs(this.F * d * 2.5 + 10)
            : Math.max(Math.abs(this.F * d * 2.5 + 10), 30)),
          this.b.vc(
            this.d,
            this.Ja * (0 > this.F ? 1 : -1),
            this.Ka * (0 > this.F ? 1 : -1),
            h,
            f,
            c
          )
            ? ((this.hb = c),
              (this.he = c.x),
              (this.ie = c.y),
              (this.Cg = f) && (q = !0),
              (this.F = 0))
            : ((this.d.x = p),
              (this.d.y = l),
              this.d.B(),
              (this.jg = !0),
              f || (this.F = 0)));
      }
      3 !== this.Dc &&
        0 < this.F &&
        !z &&
        (this.b.trigger(kc.prototype.q.Jp, this.d), (this.Dc = 3));
      (z || q) &&
        0 <= this.F &&
        (3 === this.Dc || q || (A && 0 === this.F)
          ? (this.b.trigger(kc.prototype.q.Mp, this.d),
            (this.Dc = 0 === this.u && 0 === this.F ? 0 : 1))
          : (0 !== this.Dc &&
              0 === this.u &&
              0 === this.F &&
              (this.b.trigger(kc.prototype.q.Qp, this.d), (this.Dc = 0)),
            1 === this.Dc ||
              (0 === this.u && 0 === this.F) ||
              A ||
              (this.b.trigger(kc.prototype.q.Pp, this.d), (this.Dc = 1))));
      0 < this.zf && this.zf--;
      this.kg = this.b.we(this.d);
    }
  };
  d.prototype.Kp = function () {
    return !0;
  };
  d.prototype.Jp = function () {
    return !0;
  };
  d.prototype.Qp = function () {
    return !0;
  };
  d.prototype.Pp = function () {
    return !0;
  };
  d.prototype.Mp = function () {
    return !0;
  };
  q.q = new d();
  m.prototype.ye = function (d) {
    this.enabled !== (1 === d) &&
      ((this.enabled = 1 === d), this.enabled || (this.hb = null));
  };
  q.R = new m();
  q.Fb = new (function () {})();
})();
function lc(d) {
  this.b = d;
}
(function () {
  var d = lc.prototype;
  d.ta = function (d) {
    this.behavior = d;
    this.b = d.b;
  };
  d.ta.prototype.U = function () {};
  d.fa = function (d, c) {
    this.type = d;
    this.behavior = d.behavior;
    this.d = c;
    this.b = d.b;
    this.Oa = 0;
  };
  var m = d.fa.prototype,
    q = 2 * Math.PI,
    h = Math.PI / 2,
    n = (3 * Math.PI) / 2;
  m.U = function () {
    this.mb = 1 === this.G[0];
    this.Sf = this.G[1];
    this.Yl = this.G[2];
    this.Bd = this.G[3];
    this.Bd += Math.random() * this.G[4];
    0 === this.Bd
      ? (this.Oa = 0)
      : ((this.Oa = (this.G[5] / this.Bd) * q),
        (this.Oa += ((Math.random() * this.G[6]) / this.Bd) * q));
    this.Kb = this.G[7];
    this.Kb += Math.random() * this.G[8];
    this.xh = this.Oe = this.W = 0;
    5 === this.Sf && (this.Kb = W(this.Kb));
    this.init();
  };
  m.kb = function () {
    return {
      i: this.Oa,
      a: this.mb,
      mv: this.Sf,
      w: this.Yl,
      p: this.Bd,
      mag: this.Kb,
      iv: this.W,
      iv2: this.Oe,
      r: this.xh,
      lkv: this.Wb,
      lkv2: this.Zg,
    };
  };
  m.Ab = function (d) {
    this.Oa = d.i;
    this.mb = d.a;
    this.Sf = d.mv;
    this.Yl = d.w;
    this.Bd = d.p;
    this.Kb = d.mag;
    this.W = d.iv;
    this.Oe = d.iv2 || 0;
    this.xh = d.r;
    this.Wb = d.lkv;
    this.Zg = d.lkv2 || 0;
  };
  m.init = function () {
    switch (this.Sf) {
      case 0:
        this.W = this.d.x;
        break;
      case 1:
        this.W = this.d.y;
        break;
      case 2:
        this.W = this.d.width;
        this.xh = this.d.height / this.d.width;
        break;
      case 3:
        this.W = this.d.width;
        break;
      case 4:
        this.W = this.d.height;
        break;
      case 5:
        this.W = this.d.k;
        break;
      case 6:
        this.W = this.d.opacity;
        break;
      case 7:
        this.W = 0;
        break;
      case 8:
        (this.W = this.d.x), (this.Oe = this.d.y);
    }
    this.Wb = this.W;
    this.Zg = this.Oe;
  };
  m.Ld = function (d) {
    d = d % q;
    switch (this.Yl) {
      case 0:
        return Math.sin(d);
      case 1:
        return d <= h
          ? d / h
          : d <= n
          ? 1 - (2 * (d - h)) / Math.PI
          : (d - n) / h - 1;
      case 2:
        return (2 * d) / q - 1;
      case 3:
        return (-2 * d) / q + 1;
      case 4:
        return d < Math.PI ? -1 : 1;
    }
    return 0;
  };
  m.Ob = function () {
    var d = this.b.Dg(this.d);
    this.mb &&
      0 !== d &&
      (0 === this.Bd
        ? (this.Oa = 0)
        : ((this.Oa += (d / this.Bd) * q), (this.Oa = this.Oa % q)),
      this.Kt());
  };
  m.Kt = function () {
    switch (this.Sf) {
      case 0:
        this.d.x !== this.Wb && (this.W += this.d.x - this.Wb);
        this.d.x = this.W + this.Ld(this.Oa) * this.Kb;
        this.Wb = this.d.x;
        break;
      case 1:
        this.d.y !== this.Wb && (this.W += this.d.y - this.Wb);
        this.d.y = this.W + this.Ld(this.Oa) * this.Kb;
        this.Wb = this.d.y;
        break;
      case 2:
        this.d.width = this.W + this.Ld(this.Oa) * this.Kb;
        this.d.height = this.d.width * this.xh;
        break;
      case 3:
        this.d.width = this.W + this.Ld(this.Oa) * this.Kb;
        break;
      case 4:
        this.d.height = this.W + this.Ld(this.Oa) * this.Kb;
        break;
      case 5:
        this.d.k !== this.Wb && (this.W = X(this.W + (this.d.k - this.Wb)));
        this.d.k = X(this.W + this.Ld(this.Oa) * this.Kb);
        this.Wb = this.d.k;
        break;
      case 6:
        this.d.opacity = this.W + (this.Ld(this.Oa) * this.Kb) / 100;
        0 > this.d.opacity
          ? (this.d.opacity = 0)
          : 1 < this.d.opacity && (this.d.opacity = 1);
        break;
      case 8:
        this.d.x !== this.Wb && (this.W += this.d.x - this.Wb),
          this.d.y !== this.Zg && (this.Oe += this.d.y - this.Zg),
          (this.d.x = this.W + Math.cos(this.d.k) * this.Ld(this.Oa) * this.Kb),
          (this.d.y =
            this.Oe + Math.sin(this.d.k) * this.Ld(this.Oa) * this.Kb),
          (this.Wb = this.d.x),
          (this.Zg = this.d.y);
    }
    this.d.B();
  };
  m.lo = function (d, c) {
    switch (this.Sf) {
      case 2:
        this.W *= c.width / d.width;
        this.xh = c.height / c.width;
        break;
      case 3:
        this.W *= c.width / d.width;
        break;
      case 4:
        this.W *= c.height / d.height;
    }
  };
  d.q = new (function () {})();
  d.R = new (function () {})();
  d.Fb = new (function () {})();
})();
function dc(d) {
  this.b = d;
}
(function () {
  function d() {}
  var m = dc.prototype;
  m.ta = function (d) {
    this.behavior = d;
    this.b = d.b;
  };
  m.ta.prototype.U = function () {};
  m.fa = function (d, m) {
    this.type = d;
    this.behavior = d.behavior;
    this.d = m;
    this.b = d.b;
  };
  var q = m.fa.prototype;
  q.U = function () {
    this.d.M.jumpthruEnabled = 0 !== this.G[0];
  };
  q.Ob = function () {};
  m.q = new (function () {})();
  d.prototype.ye = function (d) {
    this.d.M.jumpthruEnabled = !!d;
  };
  m.R = new d();
})();
function mc(d) {
  this.b = d;
  this.nj = this.Dh = this.jf = this.mj = 0;
}
(function () {
  function d() {}
  var m = mc.prototype;
  m.ta = function (d) {
    this.behavior = d;
    this.b = d.b;
  };
  m.ta.prototype.U = function () {};
  m.fa = function (d, m) {
    this.type = d;
    this.behavior = d.behavior;
    this.d = m;
    this.b = d.b;
  };
  var q = m.fa.prototype;
  q.U = function () {
    this.enabled = 0 !== this.G[0];
  };
  q.kb = function () {
    return {
      smg: this.behavior.mj,
      ss: this.behavior.jf,
      se: this.behavior.Dh,
      smd: this.behavior.nj,
    };
  };
  q.Ab = function (d) {
    this.behavior.mj = d.smg;
    this.behavior.jf = d.ss;
    this.behavior.Dh = d.se;
    this.behavior.nj = d.smd;
  };
  q.Ob = function () {};
  q.wj = function () {
    if (this.enabled) {
      var d = this.behavior.Si.Jd(),
        m = 0,
        q = 0,
        c,
        a,
        f,
        r = 0;
      c = 0;
      for (a = d.length; c < a; c++) {
        a: {
          f = d[c];
          for (
            var e = void 0, b = void 0, p = void 0, e = 0, b = f.L.length;
            e < b;
            ++e
          )
            if (((p = f.L[e]), p.behavior instanceof mc)) {
              f = p;
              break a;
            }
          f = null;
        }
        f && f.enabled && ((m += d[c].x), (q += d[c].y), ++r);
      }
      d = this.d.n.Ra;
      a = this.b.Vb.wa;
      f = c = 0;
      a >= this.behavior.jf &&
        a < this.behavior.Dh &&
        ((c = this.behavior.mj * Math.min(this.b.eg, 1)),
        0 === this.behavior.nj &&
          (c *=
            1 - (a - this.behavior.jf) / (this.behavior.Dh - this.behavior.jf)),
        (a = Math.random() * Math.PI * 2),
        (f = Math.random() * c),
        (c = Math.cos(a) * f),
        (f *= Math.sin(a)));
      d.Bl(m / r + c);
      d.Cl(q / r + f);
    }
  };
  d.prototype.bq = function (d, m, q) {
    this.behavior.mj = d;
    this.behavior.jf = this.b.Vb.wa;
    this.behavior.Dh = this.behavior.jf + m;
    this.behavior.nj = q;
  };
  d.prototype.ye = function (d) {
    this.enabled = 0 !== d;
  };
  m.R = new d();
})();
function cc(d) {
  this.b = d;
}
(function () {
  function d() {}
  var m = cc.prototype;
  m.ta = function (d) {
    this.behavior = d;
    this.b = d.b;
  };
  m.ta.prototype.U = function () {};
  m.fa = function (d, m) {
    this.type = d;
    this.behavior = d.behavior;
    this.d = m;
    this.b = d.b;
  };
  var q = m.fa.prototype;
  q.U = function () {
    this.d.M.solidEnabled = 0 !== this.G[0];
  };
  q.Ob = function () {};
  m.q = new (function () {})();
  d.prototype.ye = function (d) {
    this.d.M.solidEnabled = !!d;
  };
  m.R = new d();
})();
function bc() {
  return [
    ec,
    fc,
    gc,
    hc,
    Z,
    kc,
    mc,
    ic,
    cc,
    jc,
    lc,
    dc,
    Y.prototype.q.im,
    ec.prototype.R.Vp,
    ec.prototype.q.em,
    Y.prototype.R.wp,
    Y.prototype.q.vp,
    gc.prototype.R.lm,
    fc.prototype.q.dm,
    Z.prototype.R.Tp,
    fc.prototype.q.Ij,
    fc.prototype.q.fm,
    Z.prototype.R.Wp,
    Z.prototype.q.yp,
    Z.prototype.R.Xp,
    jc.prototype.R.Rp,
    Z.prototype.q.Hp,
    Z.prototype.R.Up,
    Z.prototype.q.Ap,
    Z.prototype.R.cq,
    ic.prototype.R.Sp,
    ic.prototype.R.Yp,
    Z.prototype.R.cm,
    Y.prototype.R.op,
    ic.prototype.R.ye,
    kc.prototype.R.ye,
    Z.prototype.R.aq,
    Z.prototype.q.Bp,
    mc.prototype.R.bq,
    Z.prototype.R.mm,
    Y.prototype.R.Zp,
    Y.prototype.q.tp,
    gc.prototype.R.cm,
  ];
}
